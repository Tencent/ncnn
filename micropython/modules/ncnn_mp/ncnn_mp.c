...
static mp_obj_t ncnn_mp_Mat_make_new(const mp_obj_type_t* type, size_t n_args, size_t n_kw, const mp_obj_t* args)
{
    ...
    if (data_ptr)
    {
        ...
        {
            if (d)
                mat = ncnn_mat_create_external_4d_elem(w, h, d, c, data_ptr, elemsize, elempack, allocator);
            else if (c)
                mat = ncnn_mat_create_external_3d_elem(w, h, c, data_ptr, elemsize, elempack, allocator);
            else if (h)
                mat = ncnn_mat_create_external_2d_elem(w, h, data_ptr, elemsize, elempack, allocator);
            else if (w)
                mat = ncnn_mat_create_external_1d_elem(w, data_ptr, elemsize, elempack, allocator);
        }
        else
        {
            if (d)
                mat = ncnn_mat_create_external_4d(w, h, d, c, data_ptr, allocator);
            else if (c)
                mat = ncnn_mat_create_external_3d(w, h, c, data_ptr, allocator);
            else if (h)
                mat = ncnn_mat_create_external_2d(w, h, data_ptr, allocator);
            else if (w)
                mat = ncnn_mat_create_external_1d(w, data_ptr, allocator);
        }
    }
    else
    {
        ...
        {
            if (d)
                mat = ncnn_mat_create_4d_elem(w, h, d, c, elemsize, elempack, allocator);
            else if (c)
                mat = ncnn_mat_create_3d_elem(w, h, c, elemsize, elempack, allocator);
            else if (h)
                mat = ncnn_mat_create_2d_elem(w, h, elemsize, elempack, allocator);
            else if (w)
                mat = ncnn_mat_create_1d_elem(w, elemsize, elempack, allocator);
        }
        else
        {
            if (d)
                mat = ncnn_mat_create_4d(w, h, d, c, allocator);
            else if (c)
                mat = ncnn_mat_create_3d(w, h, c, allocator);
            else if (h)
                mat = ncnn_mat_create_2d(w, h, allocator);
            else if (w)
                mat = ncnn_mat_create_1d(w, allocator);
            else
                mat = ncnn_mat_create();
        }
    }
