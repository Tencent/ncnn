...
TOML_NAMESPACE_START
{
    ...
    private:
        ...
        template<typename Func, typename Array, typename T>
        static constexpr bool can_for_each_nothrow = std::is_nothrow_invocable_v<Func, for_each_elem_ref<T, Array>, size_t>     //
        ...
        static void do_for_each(Func&& visitor, Array&& arr) noexcept(for_each_is_nothrow<Func&&, Array&&>)
        {
            ...
            for (size_t i = 0; i < arr.size(); i++)
            {
                ...
                const auto keep_going = static_cast<node_ref>(static_cast<Array&&>(arr)[i])
                                        .visit(
                                            [&](auto&& elem)
...
#endif
                {
                    ...
                    {
                        using return_type = decltype(static_cast<Func&&>(visitor)(static_cast<elem_ref>(elem), i));
                    ...
                    {
                        using return_type = decltype(static_cast<Func&&>(visitor)(i, static_cast<elem_ref>(elem)));
                    ...
                    {
                        using return_type = decltype(static_cast<Func&&>(visitor)(static_cast<elem_ref>(elem)));
