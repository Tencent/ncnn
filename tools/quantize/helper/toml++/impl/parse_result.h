...
TOML_NAMESPACE_START
{
    ...
    private:
        struct storage_t
        {
            static constexpr size_t size_ = (sizeof(toml::table) < sizeof(parse_error) ? sizeof(parse_error) : sizeof(toml::table));
            static constexpr size_t align_ = (alignof(toml::table) < alignof(parse_error) ? alignof(parse_error) : alignof(toml::table));
...
return err_ ? table_iterator{} :
        ...
        {
return err_ ? const_table_iterator{} :
            table().begin();
        }
...
return err_ ? node_view<node>{} :
        ...
        {
return err_ ? node_view<const node>{} :
            table()[key];
        }
