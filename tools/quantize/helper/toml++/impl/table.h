...
TOML_NAMESPACE_START
{
    ...
    private:
        ...
        template<typename Func, typename Table, typename T>
        static constexpr bool can_for_each_nothrow = std::is_nothrow_invocable_v<Func, const key&, for_each_value_ref<T, Table> > //
        ...
        static void do_for_each(Func&& visitor, Table&& tbl) noexcept(for_each_is_nothrow<Func&&, Table&&>)
        {
            ...
            for (kvp_type& kvp : tbl.map_)
            {
                ...
                const auto keep_going = static_cast<node_ref>(*kvp.second)
                                        .visit(
                                            [&](auto&& v)
...
#endif
                {
                    ...
                    {
                        using return_type = decltype(static_cast<Func&&>(visitor)(static_cast<value_ref>(v)));
