...
def test():
    ...
    # torchscript to pnnx
    import os
    os.system(os.path.join("..", "src", "pnnx") + " test_F_conv_transpose2d.pt inputshape=[1,12,22,32],[12,16,3,3],[16,8,5,5],[16],[1,6,5,6]")

    # pnnx inference
    import test_F_conv_transpose2d_pnnx
    b0, pnnx_b1 = test_F_conv_transpose2d_pnnx.test_inference()

    ts_ok = torch.equal(a0, b0) and torch.equal(a1, pnnx_b1)

    # pt2 path (torch.export fails, skip automatically)
    from pnnx_test_helper import test_pnnx
    pt2_ok = test_pnnx(net, (x, w0, w1, b1, y), ["[1,12,22,32]", "[12,16,3,3]", "[16,8,5,5]", "[16]", "[1,6,5,6]"], "test_F_conv_transpose2d")
