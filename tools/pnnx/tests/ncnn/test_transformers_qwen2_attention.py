...
class Model(nn.Module):
    def __init__(self):
        ...
        config = Qwen2Config(hidden_size=192, num_attention_heads=16, num_key_value_heads=16, q_lora_rank=64, kv_lora_rank=128, attn_implementation='sdpa')
        self.rotary_emb = Qwen2RotaryEmbedding(config)
        self.attn0 = Qwen2Attention(config, layer_idx=1)
...
def test():
    ...
    torch.manual_seed(0)
    x = torch.rand(1, 16, 192)

    mask0 = torch.rand(1, 1, 16, 16)

    a = net(x, mask0)

    # export torchscript
    mod = torch.jit.trace(net, (x, mask0))
    mod.save("test_transformers_qwen2_attention.pt")

    # torchscript to pnnx
    import os
    os.system("../../src/pnnx test_transformers_qwen2_attention.pt inputshape=[1,16,192],[1,1,16,16] fp16=0")
