...
def test():
    ...
    # torchscript to ncnn
    import os
    os.system("../../src/pnnx test_F_hardshrink.pt inputshape=[16],[2,16],[3,12,16],[5,7,9,11],[2,3,5,7]")

    # ncnn inference
    import test_F_hardshrink_ncnn
    b = test_F_hardshrink_ncnn.test_inference()

    ts_ok = all(torch.allclose(a0, b0, 1e-4, 1e-4) for a0, b0 in zip(a, b))

    # pt2 path (torch.export fails, skip automatically)
    from pnnx_test_helper import test_pnnx_ncnn
    pt2_ok = test_pnnx_ncnn(net, (x, y, z, w, q), ["[16]", "[2,16]", "[3,12,16]", "[5,7,9,11]", "[2,3,5,7]"], "test_F_hardshrink")
