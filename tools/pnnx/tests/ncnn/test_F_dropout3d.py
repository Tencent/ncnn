...
class Model(nn.Module):
    ...
    def forward(self, x, y, q):
        ...
        q = F.max_pool3d(q, 1)
        q = F.dropout3d(q, training=False)
        x = F.relu(x)
        y = F.relu(y)
        q = F.relu(q)
        return x, y, q
...
def test():
    ...
    # torchscript to pnnx
    import os
    os.system("../../src/pnnx test_F_dropout3d.pt inputshape=[12,4,2,16],[3,5,12,16],[2,3,4,5,6]")
    ...
    pt2_ok = test_pnnx_ncnn(net, (x, y, q), ["[12,4,2,16]", "[3,5,12,16]", "[2,3,4,5,6]"], "test_F_dropout3d")
