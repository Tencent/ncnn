# Copyright 2021 Tencent
# SPDX-License-Identifier: BSD-3-Clause

import torch
import torch.nn as nn
import torch.nn.functional as F

class Model(nn.Module):
    def __init__(self):
        super(Model, self).__init__()

        self.pool_0 = nn.AdaptiveAvgPool3d(output_size=(7,6,5))
        self.pool_1 = nn.AdaptiveAvgPool3d(output_size=1)
        self.pool_2 = nn.AdaptiveAvgPool3d(output_size=(None,4,3))
        self.pool_3 = nn.AdaptiveAvgPool3d(output_size=(5,None,None))

    def forward(self, x, q):
        out0 = self.pool_0(x)
        out1 = self.pool_1(x)
        out2 = self.pool_2(x)
        out3 = self.pool_3(x)
        q = self.pool_0(q)
        return out0, out1, out2, out3, q

def test():
    net = Model()
    net.eval()

    torch.manual_seed(0)
    x = torch.rand(1, 128, 13, 13, 13)
    q = torch.rand(2, 128, 13, 13, 13)

    a = net(x, q)

    # export torchscript
    mod = torch.jit.trace(net, (x, q))
    mod.save("test_nn_AdaptiveAvgPool3d.pt")

    # torchscript to pnnx
    import os
    os.system("../../src/pnnx test_nn_AdaptiveAvgPool3d.pt inputshape=[1,128,13,13,13],[2,128,13,13,13]")

    # ncnn inference
    import test_nn_AdaptiveAvgPool3d_ncnn
    b = test_nn_AdaptiveAvgPool3d_ncnn.test_inference()

    ts_ok = all(torch.allclose(a0, b0, 1e-4, 1e-4) for a0, b0 in zip(a, b))

    # pt2 path (torch.export fails, skip automatically)
    from pnnx_test_helper import test_pnnx_ncnn
    pt2_ok = test_pnnx_ncnn(net, (x, q), ["[1,128,13,13,13]", "[2,128,13,13,13]"], "test_nn_AdaptiveAvgPool3d")

    return ts_ok and (pt2_ok is not False)

if __name__ == "__main__":
    if test():
        exit(0)
    else:
        exit(1)
