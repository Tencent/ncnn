...
class ModelBatch(nn.Module):
    def __init__(self):
        ...
        self.rnn_0 = nn.RNN(input_size=8, hidden_size=6)
        self.rnn_1 = nn.RNN(input_size=7, hidden_size=5, batch_first=True)
...
def test():
    ...
    pt2_ok = test_pnnx_ncnn(net, (x, y), ["[1,10,32]", "[1,12,25]"], "test_nn_RNN")
...
def test_batch():
    ...
    # export torchscript
    mod = torch.jit.trace(net, (x, y))
    mod.save("test_nn_RNN_batch.pt")

    # torchscript to pnnx
    import os
    os.system("../../src/pnnx test_nn_RNN_batch.pt inputshape=[2,4,8],[2,5,7]")

    # ncnn inference
    import test_nn_RNN_batch_ncnn
    b = test_nn_RNN_batch_ncnn.test_inference()
    ...
    # pt2 path (torch.export fails, skip automatically)
    from pnnx_test_helper import test_pnnx_ncnn
    pt2_ok = test_pnnx_ncnn(net, (x, y), ["[2,4,8]", "[2,5,7]"], "test_nn_RNN_batch")
