...
class Model(nn.Module):
    ...
    def forward(self, x, y, z, w, q, r):
        ...
        q = F.max_pool2d(q, 1)
        q0, q_indices = torch.min(q, dim=1, keepdim=False)
        q1 = torch.min(q, r)
...
def test():
    ...
    # torchscript to pnnx
    import os
    os.system("../../src/pnnx test_torch_min.pt inputshape=[3,16],[5,9,11],[8,5,9,10],[5,9,10],[2,3,5,7],[3,5,7]")
    ...
    pt2_ok = test_pnnx_ncnn(net, (x, y, z, w, q, r), ["[3,16]", "[5,9,11]", "[8,5,9,10]", "[5,9,10]", "[2,3,5,7]", "[3,5,7]"], "test_torch_min")
