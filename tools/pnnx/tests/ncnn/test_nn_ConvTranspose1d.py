...
class ModelBatch(nn.Module):
    def __init__(self):
        ...
        self.deconv = nn.ConvTranspose1d(3, 4, kernel_size=3, padding=1)
...
def test():
    ...
    pt2_ok = test_pnnx_ncnn(net, (x,), ["[1,12,10]"], "test_nn_ConvTranspose1d")
...
def test_batch():
    ...
    # export torchscript
    mod = torch.jit.trace(net, x)
    mod.save("test_nn_ConvTranspose1d_batch.pt")

    # torchscript to pnnx
    import os
    os.system("../../src/pnnx test_nn_ConvTranspose1d_batch.pt inputshape=[2,3,9]")

    # ncnn inference
    import test_nn_ConvTranspose1d_batch_ncnn
    b = test_nn_ConvTranspose1d_batch_ncnn.test_inference()

    
    # pt2 path (torch.export fails, skip automatically)
    from pnnx_test_helper import test_pnnx_ncnn
    pt2_ok = test_pnnx_ncnn(net, (x,), ["[2,3,9]"], "test_nn_ConvTranspose1d_batch")
