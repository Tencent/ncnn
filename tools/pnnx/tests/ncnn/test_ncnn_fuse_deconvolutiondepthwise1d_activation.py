...
class Model(nn.Module):
    def __init__(self, act_type):
        ...
        self.conv_0 = nn.ConvTranspose1d(in_channels=12, out_channels=12, kernel_size=3, stride=2, padding=1, groups=12)
        if act_type == 'relu':
            self.act = nn.ReLU()
...
def test_act(act_type):
    ...
    pt_name = f"test_ncnn_fuse_deconvolutiondepthwise1d_{act_type}.pt"
    mod = torch.jit.trace(net, x)
    mod.save(pt_name)

    os.system(f"../../src/pnnx {pt_name} inputshape=[1,12,64]")

    ncnn_module = __import__(f"test_ncnn_fuse_deconvolutiondepthwise1d_{act_type}_ncnn")
