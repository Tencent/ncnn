...
class Model(nn.Module):
    ...
    def forward(self, x, w0, w1, b1, y, q):
        ...
        q = F.conv1d(q, w0, None, stride=2, padding=1)
        if version.parse(torch.__version__) < version.parse('1.9'):
            q = F.conv1d(q, w1, b1, stride=1, padding=1, dilation=2, groups=2)
        else:
            q = F.conv1d(q, w1, b1, stride=1, padding='same', dilation=2, groups=2)
        return x, y, q
...
def test():
    ...
    # torchscript to pnnx
    import os
    os.system("../../src/pnnx test_F_conv1d.pt inputshape=[1,12,52],[16,12,3],[16,8,5],[16],[1,6,25],[2,12,52]")
