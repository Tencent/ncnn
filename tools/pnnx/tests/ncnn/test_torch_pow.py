# Copyright 2022 Tencent
# SPDX-License-Identifier: BSD-3-Clause

import torch
import torch.nn as nn
import torch.nn.functional as F

class Model(nn.Module):
    def __init__(self):
        super(Model, self).__init__()

    def forward(self, x, y, z, w, q0, q1):
        out0 = torch.pow(x, y)
        out1 = torch.pow(y, y)
        out2 = torch.pow(z, torch.ones_like(z) + 0.5)
        out3 = torch.pow(w, torch.ones_like(w) + 0.5)
        out4 = torch.pow(w, w)
        q0 = F.max_pool2d(q0, 1)
        q1 = F.max_pool2d(q1, 1)
        out5 = torch.pow(q0, q1)
        return out0, out1, out2, out3, out4, out5

def test():
    net = Model()
    net.eval()

    torch.manual_seed(0)
    x = torch.rand(3, 16)
    y = torch.rand(3, 16)
    z = torch.rand(5, 9, 3)
    w = torch.rand(2, 3, 4, 5)
    q0 = torch.rand(2, 3, 4, 5)
    q1 = torch.rand(2, 3, 4, 5)

    a = net(x, y, z, w, q0, q1)

    # export torchscript
    mod = torch.jit.trace(net, (x, y, z, w, q0, q1))
    mod.save("test_torch_pow.pt")

    # torchscript to pnnx
    import os
    os.system("../../src/pnnx test_torch_pow.pt inputshape=[3,16],[3,16],[5,9,3],[2,3,4,5],[2,3,4,5],[2,3,4,5]")

    # ncnn inference
    import test_torch_pow_ncnn
    b = test_torch_pow_ncnn.test_inference()

    ts_ok = all(torch.allclose(a0, b0, 1e-4, 1e-4) for a0, b0 in zip(a, b))

    # pt2 path (torch.export fails, skip automatically)
    from pnnx_test_helper import test_pnnx_ncnn
    pt2_ok = test_pnnx_ncnn(net, (x, y, z, w, q0, q1), ["[3,16]", "[3,16]", "[5,9,3]", "[2,3,4,5]", "[2,3,4,5]", "[2,3,4,5]"], "test_torch_pow")

    return ts_ok and (pt2_ok is not False)

if __name__ == "__main__":
    if test():
        exit(0)
    else:
        exit(1)
