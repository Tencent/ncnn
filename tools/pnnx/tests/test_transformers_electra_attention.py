...
class Model(nn.Module):
    def __init__(self):
        ...
        config0 = ElectraConfig(hidden_size=192, num_attention_heads=16, attn_implementation='eager')
        self.attn0 = ElectraAttention(config0)

        config1 = ElectraConfig(hidden_size=66, num_attention_heads=6, attn_implementation='eager')
...
def test():
    ...
    mod.save("test_transformers_electra_attention.pt")

    # torchscript to pnnx
    import os
    os.system(os.path.join("..", "src", "pnnx") + " test_transformers_electra_attention.pt inputshape=[3,16,192],[1,5,66]")
    ...
    pt2_ok = test_pnnx(net, (x, y), ["[3,16,192]", "[1,5,66]"], "test_transformers_electra_attention")
