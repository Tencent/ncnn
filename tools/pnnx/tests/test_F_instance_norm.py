...
def test():
    ...
    # torchscript to pnnx
    import os
    os.system(os.path.join("..", "src", "pnnx") + " test_F_instance_norm.pt inputshape=[1,12,24],[2,3,12,16],[1,10,12,16,24],[12],[12],[12],[12],[3],[3],[3],[3],[10],[10],[10],[10]")

    # pnnx inference
    import test_F_instance_norm_pnnx
    pnnx_b0, pnnx_b1, pnnx_b2 = test_F_instance_norm_pnnx.test_inference()

    ts_ok = torch.equal(a0, pnnx_b0) and torch.equal(a1, pnnx_b1) and torch.equal(a2, pnnx_b2)

    # pt2 path (torch.export fails, skip automatically)
    from pnnx_test_helper import test_pnnx
    pt2_ok = test_pnnx(net, (x, y, z, m0, v0, w0, b0, m1, v1, w1, b1, m2, v2, w2, b2), ["[1,12,24]", "[2,3,12,16]", "[1,10,12,16,24]", "[12]", "[12]", "[12]", "[12]", "[3]", "[3]", "[3]", "[3]", "[10]", "[10]", "[10]", "[10]"], "test_F_instance_norm")
