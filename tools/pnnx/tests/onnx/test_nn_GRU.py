...
def test():
    ...
    if version.parse(torch.__version__) >= version.parse('2.9') and version.parse(torch.__version__) < version.parse('2.10'):
        torch.onnx.export(net, (x, y), "test_nn_GRU.onnx", dynamo=False)
    else:
        torch.onnx.export(net, (x, y), "test_nn_GRU.onnx")

    # onnx to pnnx
    import os
    os.system("../../src/pnnx test_nn_GRU.onnx inputshape=[10,1,32],[1,12,25]")
