...
class Model(nn.Module):
    def __init__(self):
        ...
        if version.parse(torch.__version__) < version.parse('1.12'):
            self.pool_4 = nn.MaxPool2d(kernel_size=(2,3), stride=1, padding=1, dilation=1, return_indices=False, ceil_mode=False)
        else:
...
def test():
    ...
    # export onnx
    torch.onnx.export(net, (x,), "test_nn_MaxPool2d.onnx")

    # onnx to pnnx
    import os
    os.system("../../src/pnnx test_nn_MaxPool2d.onnx inputshape=[1,12,64,64]")
