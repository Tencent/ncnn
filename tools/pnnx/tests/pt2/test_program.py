            ...
            "module_call_graph": [
                {
                    "fqn": "",
                    "signature": {
                        "out_spec": '[1,{"type":null,"context":null,"children_spec":[]}]'
                    },
                }
            ],
            "metadata": {},
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="pnnx-pt2-json-") as temporary_directory:
        ...
        value["graph_module"]["module_call_graph"][0]["signature"]["out_spec"] = "["
        run_case(args.tester, root, "invalid_out_spec", "--program-json", encoded(value), False, ("out_spec",))

        value = copy.deepcopy(base)
        value["graph_module"]["module_call_graph"][0]["signature"]["out_spec"] = '[1,{"type":"builtins.tuple","context":"null","children_spec":[{"type":null,"context":null,"children_spec":[]},{"type":null,"context":null,"children_spec":[]}]}]'
        run_case(args.tester, root, "out_spec_leaf_count", "--program-json", encoded(value), False, ("PyTree leaf count",))
        ...
        value = copy.deepcopy(base)
        value["graph_module"]["graph"]["nodes"].insert(
            0,
            {
                "target": "torch.ops.aten.item.default",
                "inputs": [{"name": "self", "arg": {"as_tensor": {"name": "x"}}, "kind": 1}],
                "outputs": [{"as_sym_float": {"as_name": "item"}}],
                "metadata": {},
            },
        )
        value["graph_module"]["graph"]["nodes"][1]["inputs"].append(
            {"name": "runtime", "arg": {"as_sym_float": {"as_name": "item"}}, "kind": 1}
        )
        value["graph_module"]["graph"]["nodes"][1]["inputs"].append(
            {"name": "literal", "arg": {"as_sym_float": {"as_float": 0.5}}, "kind": 1}
        )
        value["graph_module"]["graph"]["sym_float_values"] = {
            "item": {"as_expr": {"expr_str": "Symbol('zuf0')", "hint": None}}
        }
        value["graph_module"]["graph"]["nodes"][1]["inputs"].append(
            {"name": "float_list", "arg": {"as_sym_floats": [{"as_name": "item"}, {"as_float": 0.25}]}, "kind": 1}
        )
        value["graph_module"]["graph"]["nodes"][1]["inputs"].append(
            {"name": "bool_list", "arg": {"as_sym_bools": [{"as_bool": True}, {"as_bool": False}]}, "kind": 1}
        )
        run_case(args.tester, root, "program_symfloat", "--program-json", encoded(value), True)

        del value["graph_module"]["graph"]["sym_float_values"]["item"]
        run_case(args.tester, root, "missing_symfloat_value", "--program-json", encoded(value), False, ("invalid symbolic float output",))
        ...
        value = copy.deepcopy(base)
        value["graph_module"]["graph"]["sym_float_values"] = {
            "f0": {"as_expr": {"expr_str": "Symbol('zuf0')", "hint": {"as_int": 1}}}
        }
        cases.append(("symfloat_hint", value, "SymFloat hint must use as_float"))

        value = copy.deepcopy(base)
        value["graph_module"]["graph"]["sym_float_values"][""] = {"as_float": 1.0}
        cases.append(("empty_symfloat_value_name", value, "symbolic float value name is empty"))
