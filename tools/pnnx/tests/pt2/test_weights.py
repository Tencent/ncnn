...
class LegacyWeights(torch.nn.Module):
    def __init__(self):
        super(LegacyWeights, self).__init__()
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="pnnx-pt2-weights-") as directory:
        ...
        raw = zip_bytes(make_raw_archive([("weight", 7, [2], bytes(8)), ("bias", 7, [2], bytes(8))]))
        ...
        mutate_raw(raw, root, "unsafe_path", unsafe_path, "unsafe payload record name", args.tester)
        ...
        mutate_raw(raw, root, "pickled", pickled_payload, "pickled raw payload is unsupported", args.tester)
        ...
        mutate_raw(raw, root, "unknown_config_field", unknown_config_field, "future_required", args.tester)

        def truncated_storage(records):
            name = find_record(records, "data/weights/weight")
            records[name] = records[name][:-4]

        mutate_raw(raw, root, "truncated", truncated_storage, "tensor view is outside its storage", args.tester)
        ...
        mutate_raw(raw, root, "missing", missing_payload, "missing payload for bias", args.tester)

        records = make_raw_archive([("value", 7, [2], bytes(range(12)))])
        name = find_record(records, "model_weights_config.json")
        config = json.loads(records[name])
        config["config"]["value"]["tensor_meta"]["storage_offset"]["as_int"] = 1
        records[name] = json.dumps(config).encode()
        ...
        if tuple(int(x) for x in torch.__version__.split("+")[0].split(".")[:2]) < (2, 8):
            ...
            torch.export.save(torch.export.export(LegacyWeights(), (torch.ones(4),)), legacy)

            def unknown_global(records):
                name = find_record(records, "data.pkl")
                records[name] = records[name].replace(b"collections\nOrderedDict\n", b"collections\nOrderedDicx\n", 1)

            mutate_legacy(legacy, root, "global", unknown_global, "unsupported GLOBAL", args.tester)
            ...
            mutate_legacy(legacy, root, "opcode", unsupported_opcode, "unsupported pickle opcode", args.tester)

            def empty_string_at_eof(records):
                name = find_record(records, "data.pkl")
                records[name] = b"\x80\x02X\x00\x00\x00\x00"

            mutate_legacy(legacy, root, "empty_string", empty_string_at_eof, "pickle has no STOP opcode", args.tester)

            def short_storage(records):
                name = find_record(records, "data/0")
                records[name] = records[name][:-4]

            mutate_legacy(legacy, root, "storage", short_storage, "storage byte size does not match", args.tester)
