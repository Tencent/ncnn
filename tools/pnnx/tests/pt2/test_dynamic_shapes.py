...
def check_output(expected, actual, rtol=1e-5, atol=1e-6, reshape=False):
    if reshape and expected.numel() == actual.numel():
        actual = actual.reshape(expected.shape)
    if actual.shape != expected.shape or not torch.allclose(actual, expected, rtol=rtol, atol=atol):
        raise AssertionError("output differs")
...
def main():
    ...
    try:
        ...
        for shape in ((6, 3), (4, 3)):
            x = torch.randn(shape)
            expected = model(x)
            check_output(expected, converted(x))
            check_output(expected, run_ncnn(args.workdir, x), 1e-3, 1e-3, True)
    finally:
    ...
    for name, mutate_input, expression, valid, expected in cases:
