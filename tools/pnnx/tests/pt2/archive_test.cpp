...
static bool parse_expected_format(const std::string& name, pnnx::ModelFormat& format)
{
    if (name == "other")
        ...
        format = pnnx::TorchScript;
    else if (name == "pt2-legacy-exported-program" || name == "pt2-archive")
        format = pnnx::Pt2;
    else if (name == "unknown-zip")
        format = pnnx::UnknownZip;
    else
        return false;
    return true;
}
...
static bool check_f32(const pnnx::Pt2Weights& weights, const char* name, const std::vector<int>& shape, const std::vector<float>& expected)
{
    ...
    {
        float value;
        memcpy(&value, &it->second.data[i * sizeof(float)], sizeof(float));
        if (value != expected[i])
            return false;
    }
    return true;
...
static int check_weights(const pnnx::Pt2Weights& weights, const std::string& name)
{
    ...
    if (name == "big_endian")
    {
        if (weights.values.size() != 1)
            return -1;
        float value;
        const pnnx::Attribute& attribute = weights.values.begin()->second;
...
int main(int argc, char** argv)
{
    ...
    if (expected_format == pnnx::Pt2)
    {
        ...
        const pnnx::Pt2ContainerKind expected_container = std::string(argv[2]) == "pt2-archive" ? pnnx::Pt2ContainerArchive : pnnx::Pt2ContainerLegacyExportedProgram;
        if (archive.container_kind != expected_container || archive.archive_version.empty())
        {
            fprintf(stderr, "unexpected PT2 container or version\n");
            return 1;
        }
    }
