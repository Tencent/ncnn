...
class Model(nn.Module):
    ...
    def forward(self, x, w0, w1, b1, y):
        ...
        if version.parse(torch.__version__) < version.parse('2.1'):
            ...
            x = F.conv1d(x, w1, b1, stride=1, padding='same', dilation=1, groups=2)
...
def test():
    ...
    program = torch.export.export(net, (x, w0, w1, b1, y))
    torch.export.save(program, "test_F_conv1d.pt2")

    from pnnxutil import run_pnnx
    run_pnnx("test_F_conv1d.pt2", "inputshape=[1,12,52],[16,12,3],[16,8,5],[16],[1,6,25]")
