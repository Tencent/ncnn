...
class Model(nn.Module):
    def __init__(self):
        ...
        if version.parse(torch.__version__) < version.parse('2.1'):
            ...
            self.conv_4 = nn.Conv1d(in_channels=28, out_channels=32, kernel_size=3, stride=1, padding='same', dilation=1, groups=2, bias=False, padding_mode='zeros')
...
def test():
    ...
    torch.export.save(program, "test_nn_Conv1d.pt2")

    from pnnxutil import run_pnnx
    run_pnnx("test_nn_Conv1d.pt2", "inputshape=[1,12,64]")
