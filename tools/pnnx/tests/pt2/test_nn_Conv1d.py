...
class Model(nn.Module):
    def __init__(self):
        ...
        #self.conv_7 = nn.Conv1d(in_channels=28, out_channels=24, kernel_size=3, stride=2, padding=(5), dilation=2, groups=1, bias=True, padding_mode='circular')

        self.conv_8 = nn.Conv1d(in_channels=28, out_channels=24, kernel_size=3, stride=2, padding=(5), dilation=2, groups=1, bias=True)
...
def test():
    ...
    torch.export.save(mod, "test_nn_Conv1d.pt2")

    # pt2 to pnnx
    import os
    os.system(os.path.normpath("../../src/pnnx") + " test_nn_Conv1d.pt2 inputshape=[1,12,64]")
