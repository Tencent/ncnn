...
class Model(nn.Module):
    def __init__(self):
        ...
        config0 = BertConfig(hidden_size=192, num_attention_heads=16, attn_implementation='eager')
        self.attn0 = BertAttention(config0)

        config1 = BertConfig(hidden_size=66, num_attention_heads=6, attn_implementation='eager')
...
def test():
    ...
    # torchscript to pnnx
    import os
    os.system(os.path.join("..", "src", "pnnx") + " test_transformers_bert_attention.pt inputshape=[3,16,192],[1,5,66]")
    ...
    pt2_ok = test_pnnx(net, (x, y), ["[3,16,192]", "[1,5,66]"], "test_transformers_bert_attention")
