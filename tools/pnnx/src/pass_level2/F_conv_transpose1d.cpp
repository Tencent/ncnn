...
REGISTER_GLOBAL_PNNX_GRAPH_REWRITER_PASS(F_conv_transpose1d, 140)

class F_conv_transpose1d_bias : public GraphRewriterPass
{
public:
    const char* match_pattern_graph() const
    {
...
pnnx.Input              input_0     0 1 input
pnnx.Input              input_1     0 1 weight
F.conv_transpose1d      op_0        2 1 input weight a bias=None stride=%stride padding=%padding dilation=%dilation output_padding=%output_padding groups=%groups
...
F.conv_transpose1d      conv        3 1 input weight bias out stride=%stride padding=%padding dilation=%dilation output_padding=%output_padding groups=%groups
pnnx.Output             output      1 0 out
)PNNXIR";
    }
...
REGISTER_GLOBAL_PNNX_GRAPH_REWRITER_PASS(F_conv_transpose1d_bias, 141)
...
pnnx.Output             output      1 0 out
    ...
    bool match(const std::map<std::string, Parameter>& captured_params) const
    {
        ...
        {
            if (captured_params.at("op_0.output_padding").type != 5 || captured_params.at("op_0.output_padding").ai.size() != 1)
                return false;
        }
...
REGISTER_GLOBAL_PNNX_GRAPH_REWRITER_PASS(F_conv_transpose1d_onnx, 140)

class F_conv_transpose1d_onnx_1 : public F_conv_transpose1d_onnx
{
public:
    const char* match_pattern_graph() const
    {
...
pnnx.Output             output      1 0 out
    ...
    void write(Operator* op, const std::map<std::string, Parameter>& captured_params) const
    {
        F_conv_transpose1d_onnx::write(op, captured_params);
...
class F_conv_transpose1d_aten : public GraphRewriterPass
{
public:
    const char* match_pattern_graph() const
    {
...
aten::conv_transpose1d  op_0        8 1 input weight bias stride padding output_padding groups dilation out
pnnx.Output             output      1 0 out
)PNNXIR";
    }
...
REGISTER_GLOBAL_PNNX_GRAPH_REWRITER_PASS(F_conv_transpose1d_aten, 140)
