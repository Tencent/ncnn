...
pnnx.Output             output      1 0 out
    ...
    bool match(const std::map<std::string, Parameter>& captured_params, const std::map<std::string, Attribute>& captured_attrs) const
    {
        ...
        const std::map<std::string, Attribute>::const_iterator bias = captured_attrs.find("bias.data");
        if (bias != captured_attrs.end() && (bias->second.shape.size() != 1 || weight.shape.empty() || bias->second.shape[0] != weight.shape[0]))
            return false;
    ...
    void write(Operator* op, const std::map<std::string, Parameter>& captured_params, const std::map<std::string, Attribute>& captured_attrs) const
    {
        ...
        const bool has_bias = captured_attrs.find("bias.data") != captured_attrs.end();
...
class torchvision_DeformConv2d_nobias : public torchvision_DeformConv2d
{
public:
    const char* match_pattern_graph() const
    {
...
pnnx.Input              input_2     0 1 mask
pnnx.Attribute          weight      0 1 weight @data
prim::Constant          bias_size   0 1 bias_size value=%bias_size
torch.zeros             bias        1 1 bias_size bias dtype=*
prim::Constant          op_0        0 1 stride_h value=%stride_h
prim::Constant          op_1        0 1 stride_w value=%stride_w
prim::Constant          op_2        0 1 pad_h value=%pad_h
prim::Constant          op_3        0 1 pad_w value=%pad_w
...
pnnx.Output             output      1 0 out
    ...
    bool match(const std::map<std::string, Parameter>& captured_params, const std::map<std::string, Attribute>& captured_attrs) const
    {
        if (!torchvision_DeformConv2d::match(captured_params, captured_attrs))
            return false;

        const Parameter& bias_size = captured_params.at("bias_size");
        return bias_size.type == 5 && bias_size.ai.size() == 1 && bias_size.ai[0] == captured_attrs.at("weight.data").shape[0];
    }
};

REGISTER_GLOBAL_PNNX_GRAPH_REWRITER_PASS(torchvision_DeformConv2d_nobias, 21)
