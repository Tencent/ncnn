...
pnnx.Input              input_2     0 1 mask
pnnx.Attribute          weight      0 1 weight @data=(%out_channels,%weight_in_channels,%kernel_h,%kernel_w)f32
pnnx.Input              input_3     0 1 bias_size
torch.zeros             bias        1 1 bias_size bias %*=%*
prim::Constant          op_0        0 1 stride_h value=%stride_h
prim::Constant          op_1        0 1 stride_w value=%stride_w
prim::Constant          op_2        0 1 pad_h value=%pad_h
prim::Constant          op_3        0 1 pad_w value=%pad_w
...
pnnx.Output             output      1 0 out
    ...
    {
        torchvision_DeformConv2d::write(op, captured_params, captured_attrs);
        op->inputs.back()->remove_consumer(op);
        op->inputs.pop_back();
        if (!op->inputnames.empty())
...
REGISTER_GLOBAL_PNNX_GRAPH_REWRITER_PASS(torchvision_DeformConv2d_nobias, 140)

class torchvision_DeformConv2d_nomask_nobias : public torchvision_DeformConv2d_nobias
{
public:
    const char* match_pattern_graph() const
    {
...
pnnx.Output             output      1 0 out
    ...
    void write(Operator* op, const std::map<std::string, Parameter>& captured_params, const std::map<std::string, Attribute>& captured_attrs) const
    {
        torchvision_DeformConv2d_nobias::write(op, captured_params, captured_attrs);
        op->inputs[2]->remove_consumer(op);
        op->inputs.resize(2);
        if (!op->inputnames.empty())
            op->inputnames.resize(2);
...
REGISTER_GLOBAL_PNNX_GRAPH_REWRITER_PASS(torchvision_DeformConv2d_nomask_nobias, 140)
