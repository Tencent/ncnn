...
pnnx.Attribute          R           0 1 R @data
pnnx.Attribute          B           0 1 B @data
tnn.LSTMONNX            lstm        6 3 input W R B initial_h initial_c out outh outc %*=%*
pnnx.Output             output      3 0 out outh outc
    ...
    bool match(const std::map<std::string, Parameter>& captured_params, const std::map<std::string, Attribute>& captured_attrs) const
    {
        // arg0=0, arg1=512, arg2=2

        // captured_params.at("lstm.arg0"); // clip_threshold

        const int hidden_size = captured_params.at("lstm.arg1").i;
        const int direction = captured_params.at("lstm.arg2").i;

        const int num_directions = direction == 2 ? 2 : 1;

        const auto& W = captured_attrs.at("W.data"); // 2,2048,512
        const auto& R = captured_attrs.at("R.data"); // 2,2048,512
        const auto& B = captured_attrs.at("B.data"); // 2,4096
    ...
    void write(Operator* op, const std::map<std::string, Parameter>& captured_params, const std::map<std::string, Attribute>& captured_attrs) const
    {
        ...
        op->params["bidirectional"] = direction == 2 ? true : false;
        op->params["proj_size"] = 0;
...
REGISTER_GLOBAL_PNNX_GRAPH_REWRITER_PASS(nn_LSTM_tnn, 140)
