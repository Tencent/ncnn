...
int Graph::python(const std::string& pypath, const std::string& pnnxbinpath, const std::vector<std::vector<int64_t> >& input_shapes)
{
    ...
    for (const Operator* op : ops)
    {
        if (op->type != "GRU" || op->inputs.size() < 3)
            continue;
        ...
        for (const Operator* op : ops)
        {
            ...
            if (op->inputs.size() >= 4 && op->inputs[3]->producer && op->inputs[3]->producer->type == "pnnx.Attribute")
            {
                ...
                fprintf(pyfp, "        self.%s.bias_ih_l0 = nn.Parameter(_%s_bias[:%d])\n",
                    sanitize_identifier(op->name).c_str(), sanitize_identifier(op->name).c_str(), hidden_size * 3);
