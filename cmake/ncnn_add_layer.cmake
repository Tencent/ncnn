...
macro(ncnn_add_layer class)
    ...
    if(NCNN_RUNTIME_CPU AND NCNN_TARGET_ARCH STREQUAL "x86")
        if(CMAKE_CXX_COMPILER_ID MATCHES "MSVC" OR (CMAKE_CXX_COMPILER_ID MATCHES "Clang" AND CMAKE_CXX_SIMULATE_ID MATCHES "MSVC" AND CMAKE_CXX_COMPILER_FRONTEND_VARIANT MATCHES "MSVC"))
            if(NCNN_AVX512)
                ncnn_add_arch_opt_layer(${class} avx512 "/arch:AVX512 /D__FMA__")
            endif()
            if(NCNN_FMA)
                ncnn_add_arch_opt_layer(${class} fma "/arch:AVX /D__FMA__")
            endif()
            if(NCNN_AVX)
                ncnn_add_arch_opt_layer(${class} avx "/arch:AVX")
            endif()
            if(NCNN_AVX512VNNI)
                ncnn_add_arch_opt_source(${class} avx512vnni "/arch:AVX512 /D__FMA__ /D__AVX512VNNI__")
            endif()
            if(NCNN_AVXVNNI)
                ncnn_add_arch_opt_source(${class} avxvnni "/arch:AVX2 /D__FMA__ /D__AVXVNNI__")
            endif()
            if(NCNN_AVX2)
                ncnn_add_arch_opt_source(${class} avx2 "/arch:AVX2 /D__FMA__")
            endif()
            if(NCNN_XOP)
                ncnn_add_arch_opt_source(${class} xop "/arch:AVX /D__FMA__ /D__XOP__")
            endif()
        else()
            if(NCNN_AVX512)
                ncnn_add_arch_opt_layer(${class} avx512 "-mavx512f -mavx512vl -mfma -mf16c")
            endif()
            if(NCNN_FMA)
            ...
            if(NCNN_AVX512VNNI)
                ncnn_add_arch_opt_source(${class} avx512vnni "-mavx512f -mavx512vl -mfma -mf16c -mavx512vnni")
            endif()
            if(NCNN_AVXVNNI)
            ...
            if(NCNN_XOP)
                ncnn_add_arch_opt_source(${class} xop "-mavx -mfma -mxop")
            endif()
        endif()
    endif()
