...
macro(ncnn_add_arch_optbase_opt_source class NCNN_TARGET_ARCH_OPT_BASE NCNN_TARGET_ARCH_OPT NCNN_TARGET_ARCH_OPT_CFLAGS)
    ...
    if(WITH_LAYER_${name} AND EXISTS ${NCNN_${NCNN_TARGET_ARCH_OPT_BASE}_SOURCE})
        ...
        set(NCNN_${NCNN_TARGET_ARCH_OPT}_SOURCE ${CMAKE_CURRENT_BINARY_DIR}/layer/${NCNN_TARGET_ARCH}/${name}_${NCNN_TARGET_ARCH}_${NCNN_TARGET_ARCH_OPT_BASE}_${NCNN_TARGET_ARCH_OPT}.cpp)

        add_custom_command(
            OUTPUT ${NCNN_${NCNN_TARGET_ARCH_OPT}_SOURCE}
            COMMAND ${CMAKE_COMMAND} -DSRC=${NCNN_${NCNN_TARGET_ARCH_OPT_BASE}_SOURCE} -DDST=${NCNN_${NCNN_TARGET_ARCH_OPT}_SOURCE} -DCLASS=${class} -P "${CMAKE_CURRENT_SOURCE_DIR}/../cmake/ncnn_generate_${NCNN_TARGET_ARCH_OPT}_source.cmake"
    ...
    if(NCNN_RUNTIME_CPU AND NCNN_TARGET_ARCH STREQUAL "x86")
        if(CMAKE_CXX_COMPILER_ID MATCHES "MSVC" OR (CMAKE_CXX_COMPILER_ID MATCHES "Clang" AND CMAKE_CXX_SIMULATE_ID MATCHES "MSVC" AND CMAKE_CXX_COMPILER_FRONTEND_VARIANT MATCHES "MSVC"))
            if(NCNN_AVX512)
                ...
                ncnn_add_arch_opt_source(${class} avx2_avx512 "/arch:AVX512 /D__FMA__ /D__F16C__")
                ncnn_add_arch_opt_source(${class} f16c_avx512 "/arch:AVX512 /D__FMA__ /D__F16C__")
            endif()
            if(NCNN_FMA)
                ncnn_add_arch_opt_layer(${class} fma "/arch:AVX /D__FMA__ /D__F16C__")
                ncnn_add_arch_opt_source(${class} f16c_fma "/arch:AVX /D__FMA__ /D__F16C__")
            endif()
            if(NCNN_AVX)
            ...
            if(NCNN_AVX2)
                ...
                #if(NCNN_AVX512)
                    #ncnn_add_arch_optbase_opt_source(${class} avx2 avx512 "/arch:AVX512 /D__FMA__ /D__F16C__")
                #endif()
            endif()
            if(NCNN_XOP)
            ...
            if(NCNN_F16C)
                ...
                #if(NCNN_AVX512)
                    #ncnn_add_arch_optbase_opt_source(${class} f16c avx512 "/arch:AVX512 /D__FMA__ /D__F16C__")
                #endif()
                #if(NCNN_FMA)
                    #ncnn_add_arch_optbase_opt_source(${class} f16c fma "/arch:AVX /D__FMA__ /D__F16C__")
                #endif()
                #if(NCNN_AVX)
                    #ncnn_add_arch_optbase_opt_source(${class} f16c avx "/arch:AVX /D__F16C__")
                #endif()
            endif()
        else()
            if(NCNN_AVX512)
                ncnn_add_arch_opt_layer(${class} avx512 "-mavx512f -mavx512cd -mavx512bw -mavx512dq -mavx512vl -mfma -mf16c")
                ncnn_add_arch_opt_source(${class} avx2_avx512 "-mavx512f -mavx512cd -mavx512bw -mavx512dq -mavx512vl -mfma -mf16c")
                ncnn_add_arch_opt_source(${class} f16c_avx512 "-mavx512f -mavx512cd -mavx512bw -mavx512dq -mavx512vl -mfma -mf16c")
            endif()
            if(NCNN_FMA)
                ncnn_add_arch_opt_layer(${class} fma "-mavx -mfma -mf16c")
                ncnn_add_arch_opt_source(${class} f16c_fma "-mavx -mfma -mf16c")
            endif()
            if(NCNN_AVX)
            ...
            if(NCNN_AVX2)
                ...
                #if(NCNN_AVX512)
                    #ncnn_add_arch_optbase_opt_source(${class} avx2 avx512 "-mavx512f -mavx512cd -mavx512bw -mavx512dq -mavx512vl -mfma -mf16c")
                #endif()
            endif()
            if(NCNN_XOP)
            ...
            if(NCNN_F16C)
                ...
                #if(NCNN_AVX512)
                    #ncnn_add_arch_optbase_opt_source(${class} f16c avx512 "-mavx512f -mavx512cd -mavx512bw -mavx512dq -mavx512vl -mfma -mf16c")
                #endif()
                #if(NCNN_FMA)
                    #ncnn_add_arch_optbase_opt_source(${class} f16c fma "-mavx -mfma -mf16c")
                #endif()
                #if(NCNN_AVX)
                    #ncnn_add_arch_optbase_opt_source(${class} f16c avx "-mavx -mf16c")
                #endif()
            endif()
        endif()
    endif()
