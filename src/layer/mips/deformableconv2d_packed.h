    ...
    for (int h_col = 0; h_col < outh; h_col++)
    {
        for (int w_col = 0; w_col < outw; w_col++)
        {
            ...
            for (int oc = 0; oc < outch; oc++)
            {
...
#if __mips_msa
                v4f32 _sum_msa = (v4f32)__msa_fill_w(0);
#endif // __mips_msa
...
#if __mips_msa
                    if (out_elempack == 4)
                        _sum_msa = (v4f32)__msa_ld_w(bias_data_ptr + oc * out_elempack, 0);
#endif // __mips_msa
                    if (out_elempack == 1)
                ...
                for (int i = 0; i < kernel_h; i++)
                {
                    for (int j = 0; j < kernel_w; j++)
                    {
                        ...
                        for (int ic = 0; ic < inch; ic++)
                        {
                            ...
                            if (out_elempack == 4)
                            {
                                v4f32 _bw1 = __msa_fill_w_f32(bw1);
                                v4f32 _bw2 = __msa_fill_w_f32(bw2);
                                v4f32 _bw3 = __msa_fill_w_f32(bw3);
                                v4f32 _bw4 = __msa_fill_w_f32(bw4);
                                v4f32 _mask = __msa_fill_w_f32(mask_);
                                ...
                                for (int ez = 0; ez < elempack; ez++)
                                {
                                    ...
                                    if (cond)
                                    {
                                        v4f32 _v1 = v1_cond ? __msa_fill_w_f32(*(data_im_ptr + v1_pos * elempack + ez)) : (v4f32)__msa_fill_w(0);
                                        v4f32 _v2 = v2_cond ? __msa_fill_w_f32(*(data_im_ptr + v2_pos * elempack + ez)) : (v4f32)__msa_fill_w(0);
                ...
                if (out_elempack == 4)
                {
                    _sum_msa = activation_msa(_sum_msa, activation_type, activation_params);
                    __msa_st_w((v4i32)_sum_msa, outptr + (h_col * outw + w_col) * out_elempack, 0);
                }
#endif // __mips_msa
                if (out_elempack == 1)
                {
