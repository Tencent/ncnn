...
static void gridsample_2d_bicubic_compute_blob_mips(const Mat& src, const Mat& grid, Mat& offset_value, int permute_fusion)
{
    ...
    if (permute_fusion == 0)
    {
        for (int y = 0; y < outh; y++)
        {
            ...
            for (; x + 3 < outw; x += 4)
            {
                ...
                gridsample_store_2d_bicubic_msa(offset_value_ptr, _sample_x, _sample_y, src.w, src.h, src.elempack, padding_mode, align_corner);
            ...
            for (; x < outw; x++)
            {
                ...
                x0 = (int)compute_coord_mips((float)x0, src.w, padding_mode, align_corner);
                x1 = (int)compute_coord_mips((float)x1, src.w, padding_mode, align_corner);
                x2 = (int)compute_coord_mips((float)x2, src.w, padding_mode, align_corner);
                x3 = (int)compute_coord_mips((float)x3, src.w, padding_mode, align_corner);
                ...
                {
                    int gy = y1 + i - 1;
                    gy = (int)compute_coord_mips((float)gy, src.h, padding_mode, align_corner);
...
static void gridsample_2d_bicubic_compute_blob_mips(const Mat& src, const Mat& grid, Mat& offset_value, int padding_mode, int align_corner, int permute_fusion)
{
    GRIDSAMPLE_COMPUTE_BLOB_DISPATCH(gridsample_2d_bicubic_compute_blob_mips, src, grid, offset_value, padding_mode, align_corner, permute_fusion);
}
