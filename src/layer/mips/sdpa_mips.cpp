...
static int read_past_len_blob_mips(const Mat& past_len_blob, bool use_bf16_storage, int& past_seqlen)
{
...
int SDPA_mips::forward(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& _opt) const
{
    ...
    if (kv_cache == 2)
    {
        if (past_key.dims == 0 || past_value.dims == 0 || read_past_len_blob_mips(past_len_blob, opt.use_bf16_storage, past_seqlen) != 0)
            return -1;
    }
