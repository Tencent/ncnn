...
int Reshape_mips::forward(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const
{
    ...
    if (ndim == 1)
    {
        ...
        const int total_unpacked = bottom_blob_unpacked.w * bottom_blob_unpacked.h * bottom_blob_unpacked.d * bottom_blob_unpacked.c;
        top_blob = bottom_blob_unpacked.reshape(total_unpacked, opt.blob_allocator);
        if (top_blob.empty())
            return -100;
