...

        {
            ...
            if (out_elempack == 4)
            {
                ...
                for (int p = 0; p < out_channels; p++)
                {
                    ...
                    if (bias_data.empty())
                    {
                        outm.fill((v4f32)__msa_fill_w(0));
                    }
                    else
                    {
                        outm.fill((v4f32)__msa_ld_w((const float*)bias_data + p * 4, 0));
                    }
...
#if NCNN_BF16
int Deconvolution_mips::create_pipeline_bf16s(const Option& opt)
{
    const int maxk = kernel_w * kernel_h;
...
int Deconvolution_mips::forward_bf16s(const Mat& bottom_blob, Mat& top_blob, const Option& opt) const
{
    int w = bottom_blob.w;
    int h = bottom_blob.h;
    int elempack = bottom_blob.elempack;
