...
static void pack_A_tile_bf16(const Mat& A, Mat& AT, int i, int max_ii, int k, int max_kk)
{
    ...
    for (; ii + 1 < max_ii; ii += 2)
    {
        ...
        if (elempack == 4)
        {
            const int i_pack = (i + ii) / 4;
            const int i_lane = (i + ii) % 4;
            const unsigned short* p0 = (const unsigned short*)A + (size_t)i_pack * A_hstep * 4 + k * 4 + i_lane;
...
static void transpose_pack_B_tile_bf16(const Mat& B, Mat& BT, int j, int max_jj, int k, int max_kk)
{
    ...
    for (; jj + 7 < max_jj; jj += 8)
    {
        if (elempack == 8)
        {
            const int k_pack = k / 8;
            const int k_lane = k % 8;
            const unsigned short* p0 = (const unsigned short*)B + (size_t)k_pack * B_hstep * 8 + (j + jj) * 8 + k_lane;
    ...
    for (; ii + 1 < max_ii; ii += 2)
    {
        ...
        for (; jj + 7 < max_jj; jj += 8)
        {
            ...
            for (; kk + 1 < max_kk; kk += 2)
            {
                ...
                v4f32 _pB0 = (v4f32)__msa_ilvr_h(_pB01_bf16, _zero_bf16);
                v4f32 _pB1 = (v4f32)__msa_ilvl_h(_pB01_bf16, _zero_bf16);
                v8i16 _pB23_bf16 = (v8i16)__msa_ld_h(pB + 8, 0);
                v4f32 _pB2 = (v4f32)__msa_ilvr_h(_pB23_bf16, _zero_bf16);
                v4f32 _pB3 = (v4f32)__msa_ilvl_h(_pB23_bf16, _zero_bf16);
    ...
    for (; ii < max_ii; ii += 1)
    {
        ...
        for (; jj + 7 < max_jj; jj += 8)
        {
            ...
            for (; kk + 3 < max_kk; kk += 4)
            {
                ...
                _pA = (v4f32)__msa_fill_w((int)((unsigned int)pA[1] << 16));
                _pB01_bf16 = (v8i16)__msa_ld_h(pB + 8, 0);
                _pB0 = (v4f32)__msa_ilvr_h(_pB01_bf16, _zero_bf16);
                _pB1 = (v4f32)__msa_ilvl_h(_pB01_bf16, _zero_bf16);
                _sum1 = __ncnn_msa_fmadd_w(_sum1, _pA, _pB0);
                _sum9 = __ncnn_msa_fmadd_w(_sum9, _pA, _pB1);

                _pA = (v4f32)__msa_fill_w((int)((unsigned int)pA[2] << 16));
                _pB01_bf16 = (v8i16)__msa_ld_h(pB + 16, 0);
                _pB0 = (v4f32)__msa_ilvr_h(_pB01_bf16, _zero_bf16);
                _pB1 = (v4f32)__msa_ilvl_h(_pB01_bf16, _zero_bf16);
                _sum2 = __ncnn_msa_fmadd_w(_sum2, _pA, _pB0);
                _suma = __ncnn_msa_fmadd_w(_suma, _pA, _pB1);

                _pA = (v4f32)__msa_fill_w((int)((unsigned int)pA[3] << 16));
                _pB01_bf16 = (v8i16)__msa_ld_h(pB + 24, 0);
                _pB0 = (v4f32)__msa_ilvr_h(_pB01_bf16, _zero_bf16);
                _pB1 = (v4f32)__msa_ilvl_h(_pB01_bf16, _zero_bf16);
        ...
        for (; jj + 3 < max_jj; jj += 4)
        {
            ...
            for (; kk + 3 < max_kk; kk += 4)
            {
                ...
                _pA = (v4f32)__msa_fill_w((int)((unsigned int)pA[1] << 16));
                _pB = bfloat2float_msa(pB + 4);
                _sum1 = __ncnn_msa_fmadd_w(_sum1, _pA, _pB);

                _pA = (v4f32)__msa_fill_w((int)((unsigned int)pA[2] << 16));
                _pB = bfloat2float_msa(pB + 8);
                _sum2 = __ncnn_msa_fmadd_w(_sum2, _pA, _pB);

                _pA = (v4f32)__msa_fill_w((int)((unsigned int)pA[3] << 16));
                _pB = bfloat2float_msa(pB + 12);
...
static int gemm_BT_mips_m1_bf16s(const Mat& A, const Mat& BT, const Mat& C, Mat& top_blob, int broadcast_type_C, int N, int K, float alpha, float beta, int constant_TILE_M, int constant_TILE_N, int constant_TILE_K, int nT, int output_elemtype, const Option& opt)
{
    (void)opt;
    ...
    for (int ppj = 0; ppj < nn_N; ppj++)
    {
        ...
        for (; jj + 7 < max_jj; jj += 8)
        {
            ...
            if (pC0)
            {
                ...
                {
                    _sum0 = __ncnn_msa_fmadd_w(_sum0, _beta, (v4f32)__msa_ld_w(pC0 + jj, 0));
                    _sum8 = __ncnn_msa_fmadd_w(_sum8, _beta, (v4f32)__msa_ld_w(pC0 + jj + 4, 0));
                }
            }
            if (alpha != 1.f)
            {
                v4f32 _alpha = __msa_fill_w_f32(alpha);
                _sum0 = __msa_fmul_w(_sum0, _alpha);
                _sum8 = __msa_fmul_w(_sum8, _alpha);
            }

            if (output_elemtype == 1)
            {
                __msa_st_w((v4i32)_sum0, outptr0f + jj, 0);
                __msa_st_w((v4i32)_sum8, outptr0f + jj + 4, 0);
            }
            else
            {
                __msa_storel_d(float2bfloat_msa(_sum0), outptr0 + jj);
                __msa_storel_d(float2bfloat_msa(_sum8), outptr0 + jj + 4);
            }
        }
#endif // __mips_msa
        ...
        for (; jj + 3 < max_jj; jj += 4)
        {
            ...
            if (alpha != 1.f)
                _sum0 = __msa_fmul_w(_sum0, __msa_fill_w_f32(alpha));
            ...
            else
            {
                ...
                outptr0[jj + 2] = float32_to_bfloat16(sum2);
                outptr0[jj + 3] = float32_to_bfloat16(sum3);
            }
#endif // __mips_msa
        }
