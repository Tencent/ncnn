...
{
    v4f32 one = (v4f32)__lsx_vreplgr2vr_w(c_1.i);

    x = __lsx_vfmax_s(x, (v4f32)__lsx_vreplgr2vr_w(0)); /* force flush to zero on denormal values */
    v4i32 invalid_mask = __lsx_vfcmp_cle_s(x, (v4f32)__lsx_vreplgr2vr_w(0));

    v4i32 ux = (v4i32)(x);

    v4i32 emm0 = __lsx_vsrl_w(ux, (v4i32)__lsx_vreplgr2vr_w(23));

    /* keep only the fractional part */
    ux = (v4i32)__lsx_vand_v((v16u8)ux, (v16u8)__lsx_vreplgr2vr_w(c_inv_mant_mask));
    ux = (v4i32)__lsx_vor_v((v16u8)ux, (v16u8)__lsx_vreplgr2vr_w(c_0p5.i));
    x = (v4f32)(ux);

    emm0 = __lsx_vsub_w(emm0, (v4i32)__lsx_vreplgr2vr_w(0x7f));
    v4f32 e = __lsx_vffint_s_w(emm0);
     ...
     *     } else { x = x - 1.0; }
     */
    v4i32 mask = __lsx_vfcmp_clt_s(x, (v4f32)__lsx_vreplgr2vr_w(c_cephes_SQRTHF.i));
    v4f32 tmp = (v4f32)(__lsx_vand_v((v16u8)(x), (v16u8)mask));
    x = __lsx_vfsub_s(x, one);
    e = __lsx_vfsub_s(e, (v4f32)(__lsx_vand_v((v16u8)(one), (v16u8)mask)));
    x = __lsx_vfadd_s(x, tmp);

    v4f32 z = __lsx_vfmul_s(x, x);
    ...
    tmp = __lsx_vfmul_s(e, (v4f32)__lsx_vreplgr2vr_w(c_cephes_log_q1.i));
    y = __lsx_vfadd_s(y, tmp);

    tmp = __lsx_vfmul_s(z, (v4f32)__lsx_vreplgr2vr_w(c_0p5.i));
    y = __lsx_vfsub_s(y, tmp);

    tmp = __lsx_vfmul_s(e, (v4f32)__lsx_vreplgr2vr_w(c_cephes_log_q2.i));
    x = __lsx_vfadd_s(x, y);
    x = __lsx_vfadd_s(x, tmp);
    x = (v4f32)(__lsx_vor_v((v16u8)(x), (v16u8)invalid_mask)); // negative arg will be NAN
    return x;
}
...
static inline v4f32 exp_ps(v4f32 x)
{
    ...
    v4f32 one = (v4f32)__lsx_vreplgr2vr_w(c_1.i);
    x = __lsx_vfmin_s(x, (v4f32)__lsx_vreplgr2vr_w(c_exp_hi.i));
    x = __lsx_vfmax_s(x, (v4f32)__lsx_vreplgr2vr_w(c_exp_lo.i));

    /* express exp(x) as exp(g + n*log(2)) */
    fx = __lsx_vfmul_s(x, (v4f32)__lsx_vreplgr2vr_w(c_cephes_LOG2EF.i));
    fx = __lsx_vfadd_s(fx, (v4f32)__lsx_vreplgr2vr_w(c_0p5.i));

    /* perform a floorf */
    tmp = __lsx_vffint_s_w(__lsx_vftint_w_s(fx));

    /* if greater, substract 1 */
    v4i32_w mask = __lsx_vfcmp_clt_s(fx, tmp);
    mask = (v4i32_w)__lsx_vand_v((v16u8)mask, (v16u8)one);

    fx = __lsx_vfsub_s(tmp, (v4f32)mask);

    tmp = __lsx_vfmul_s(fx, (v4f32)__lsx_vreplgr2vr_w(c_cephes_exp_C1.i));
    v4f32 z = __lsx_vfmul_s(fx, (v4f32)__lsx_vreplgr2vr_w(c_cephes_exp_C2.i));
    x = __lsx_vfsub_s(x, tmp);
    x = __lsx_vfsub_s(x, z);

    v4f32 y = (v4f32)__lsx_vreplgr2vr_w(c_cephes_exp_p0.i);
...
{
    v4f32 x2 = (v4f32)__lsx_vbitclri_w((v4u32)x, 31);
    v4i32 tiny_mask = __lsx_vfcmp_clt_s(x2, (v4f32)__lsx_vreplgr2vr_w(c_tanh_tiny.i));

    // clamp the inputs to the range [-9, 9] since anything outside
    // this range is -/+1.0f in single-precision.
    x2 = (v4f32)__lsx_vbitsel_v((v16u8)__lsx_vfcmp_clt_s((v4f32)__lsx_vreplgr2vr_w(c_tanh_hi.i), x2), (v16u8)x2, (v16u8)__lsx_vreplgr2vr_w(c_tanh_hi.i));

    // since the polynomials are odd/even, we need x**2.
    v4f32 z = __lsx_vfmul_s(x2, x2);
    ...
    // reinstate the sign.
    y = (v4f32)__lsx_vbstrc21I_w((v4u32)y, (v4u32)x, 0);

    // when the argument is very small in magnitude it's more accurate to just return it.
    y = (v4f32)__lsx_vbitsel_v((v16u8)tiny_mask, (v16u8)y, (v16u8)x);
...
{
    v4f32 _one = __lsx_vreplgr2vr_w(1.f);
    _v = (v4f32)__lsx_vbitrevi_w((v4u32)_v, 31);
    _v = exp_ps(_v);
