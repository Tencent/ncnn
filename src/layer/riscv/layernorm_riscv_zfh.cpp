...
static void layernorm_fp16sa(__fp16* ptr, const float* gamma_data, const float* beta_data, float eps, int elemcount, int elempack)
{
#if __riscv_zvfh
    const int packn = csrr_vlenb() / 2; // fp16
    size_t vl = (elempack == packn) ? __riscv_vsetvl_e16m1(packn) : __riscv_vsetvlmax_e16m8();
#endif
    __fp16 mean = 0.f;
    __fp16 var = 0.f;
    __fp16 a = 0.f;
    ...
    if (elempack != 1)
    {
        for (; i < elemcount; i++) _sum0 = __riscv_vfadd_vv_f16m1(_sum0, __riscv_vle16_v_f16m1(ptr + vl * i, vl), vl);
        _sum0 = __riscv_vfdiv_vf_f16m1(_sum0, elemcount, vl);
    }
#endif // __riscv_zvfh
    ...
    if (elempack != 1)
    {
        ...
        _sqsum0 = __riscv_vfdiv_vf_f16m1(_sqsum0, elemcount, vl);
    ...
    if (gamma_data && beta_data)
    {
        ...
        if (elempack != 1)
        {
            for (; i < elemcount; i++)
            {
                vfloat16m1_t _p = __riscv_vle16_v_f16m1(ptr_store, vl);
                ...
                __riscv_vse16_v_f16m1(ptr_store, _p, vl);
                ptr_store += vl;
            }
        }
#endif // __riscv_zvfh
...
int LayerNorm_riscv::forward_inplace_fp16sa(Mat& bottom_top_blob, const Option& opt) const
{
    ...
    {
        __fp16* ptr = bottom_top_blob;
        layernorm_fp16sa(ptr, gamma_data, beta_data, eps, w * elempack, 1);
    }
