...
static void convolution_im2col_pack_A_tile_rvv(const Mat& kernel, Mat& AT, int i, int max_ii, int k, int max_kk, int maxk, int K, int elempack)
{
    ...
    for (; ii + (packn - 1) < max_ii; ii += packn)
    {
        ...
        for (int kk = 0; kk < max_kk; kk++)
        {
            ...
            vfloat32m1_t _r0 = __riscv_vlse32_v_f32m1(k00, K * sizeof(float), vl);
            ...
            for (int kk = 0; kk < max_kk / elempack; kk++)
            {
                ...
                if (elempack == 1)
                {
                    if (stride_w == 1 && dy15 == dy0 + 1)
                    {
                        ...
                        const float* sptr0 = img.row(y0) + dx0 + dilation_w * v;
                        const float* sptr1 = img.row(y1) + dilation_w * v;

                        int n = 0;
                        while (n < nn0)
                        {
                            const size_t vl1 = __riscv_vsetvl_e32m1(nn0 - n);
                            vfloat32m1_t _val = __riscv_vle32_v_f32m1(sptr0 + n, vl1);
                            __riscv_vse32_v_f32m1(pp + n, _val, vl1);
                            n += vl1;
                        }
                        while (n < 16)
                        {
                            const size_t vl1 = __riscv_vsetvl_e32m1(16 - n);
                            vfloat32m1_t _val = __riscv_vle32_v_f32m1(sptr1 + n - nn0, vl1);
                            __riscv_vse32_v_f32m1(pp + n, _val, vl1);
                            n += vl1;
                        }
                    }
...
static void convolution_im2col_gemm_transform_kernel_rvv(const Mat& kernel, Mat& AT, int inch, int outch, int kernel_w, int kernel_h, const Option& opt)
{
    ...
    for (int ppj = 0; ppj < nn_M; ppj++)
    {
        ...
        for (int k = 0; k < K; k += TILE_K)
        {
            ...
            convolution_im2col_pack_A_tile_rvv(kernel, AT_tile, i, max_ii, k, max_kk, maxk, K, elempack);
        }
    }
}
