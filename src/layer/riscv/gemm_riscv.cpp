...
int Gemm_riscv::create_pipeline_wq_int8(const Option& opt)
{
    ...
    Mat B_data_packed;
    Mat B_data_descales;
    int ret = pack_B_wq_int8(B_data, B_data_quantize_scales, B_data_packed, B_data_descales, constantN, constantK, block_size, opt);
    if (ret != 0)
        return ret;
    if (B_data_packed.empty() || B_data_descales.empty())
        return -100;

    BT_data_wq_int8 = B_data_packed;
    BT_data_wq_int8_descales = B_data_descales;
