...
int AbsVal_riscv::forward_inplace_fp16s(Mat& bottom_top_blob, const Option& opt) const
{
    ...
    for (int q = 0; q < channels; q++)
    {
        ...
        while (n > 0)
        {
            ...
            _p = __riscv_vfsgnjx_vv_f32m8(_p, _p, vl);
...
int AbsVal_riscv::forward_inplace_fp16sa(Mat& bottom_top_blob, const Option& opt) const
{
    const int w = bottom_top_blob.w;
    const int h = bottom_top_blob.h;
    const int d = bottom_top_blob.d;
    ...
    for (int q = 0; q < channels; q++)
    {
        ...
        while (n > 0)
        {
            ...
            _p = __riscv_vfsgnjx_vv_f16m8(_p, _p, vl);
