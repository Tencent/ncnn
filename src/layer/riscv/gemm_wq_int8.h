...
static void pack_B_tile_wq_int8(const Mat& B, const Mat& B_scales, Mat& BT_tile, Mat& BT_descales_tile, int j, int max_jj, int K, int block_size)
{
    ...
    const size_t B_scales_hstep = B_scales.w;
#if __riscv_vector
    ...
    for (; use_nr8 && jj + 7 < max_jj; jj += 8)
    {
        ...
        for (int g = 0; g < block_count; g++)
        {
            ...
            {
                __riscv_vse8_v_i8m1(pp, __riscv_vlse8_v_i8m1(p0, B_stride, vl8), vl8);
                __riscv_vse8_v_i8m1(pp + 8, __riscv_vlse8_v_i8m1(p0 + 1, B_stride, vl8), vl8);
                __riscv_vse8_v_i8m1(pp + 16, __riscv_vlse8_v_i8m1(p0 + 2, B_stride, vl8), vl8);
                __riscv_vse8_v_i8m1(pp + 24, __riscv_vlse8_v_i8m1(p0 + 3, B_stride, vl8), vl8);
                p0 += 4;
                pp += 32;
            }
            for (; kk < max_kk; kk++)
            ...
            vfloat32m1_t _scale = __riscv_vlse32_v_f32m1(ps, B_scales_stride, vl8);
            __riscv_vse32_v_f32m1(pd, __riscv_vfrdiv_vf_f32m1(_scale, 1.f, vl8), vl8);
            ps++;
            pd += 8;
        }
    }
...
#if __riscv_vector
            vfloat32m1_t _scale = __riscv_vlse32_v_f32m1(ps, B_scales_stride, vl4);
            __riscv_vse32_v_f32m1(pd, __riscv_vfrdiv_vf_f32m1(_scale, 1.f, vl4), vl4);
#else
            pd[0] = 1.f / ps[0];
            pd[1] = 1.f / ps[B_scales_hstep];
            pd[2] = 1.f / ps[B_scales_hstep * 2];
            pd[3] = 1.f / ps[B_scales_hstep * 3];
#endif // __riscv_vector
            ps++;
            pd += 4;
        }
    ...
    if (input_scales.empty())
    {
        ...
        for (; ii + (packn - 1) < max_ii; ii += packn)
        {
            for (int g = 0; g < local_block_count; g++)
            {
                ...
                const int k0_pack = k0 / packn;
                const int k0_lane = k0 % packn;
                const float* p0_base = (const float*)A + (size_t)k0_pack * A_hstep * packn + (i + ii) * packn + k0_lane;
                const int head_kk = std::min(max_kk0, packn - k0_lane);
                vfloat32m1_t _absmax = __riscv_vfmv_v_f_f32m1(0.f, vl);

                if (elempack == packn)
                {
                    const float* p0a = p0_base;
                    ...
                    if (kk == packn - k0_lane)
                        p0a += A_hstep * packn - packn;
                    for (; kk + (packn - 1) < max_kk0; kk += packn)
                    {
                        for (int l = 0; l < packn; l++)
                        {
                            vfloat32m1_t _v = __riscv_vlse32_v_f32m1(p0a + l, packn * sizeof(float), vl);
                            _absmax = __riscv_vfmax_vv_f32m1(_absmax, __riscv_vfabs_v_f32m1(_v, vl), vl);
                        }
                        p0a += A_hstep * packn;
                    }
                ...
                if (elempack == 1)
                {
                    const float* p0 = (const float*)A + (size_t)(k + g * block_size) * A_hstep + i + ii;
                    const float* p0a = p0;
        ...
        for (; ii + 1 < max_ii; ii += 2)
        {
            ...
            if (elempack == packn)
            {
                for (int g = 0; g < local_block_count; g++)
                {
                    ...
                    const int k0_pack = k0 / packn;
                    const int k0_lane = k0 % packn;
                    const float* p0 = (const float*)A + (size_t)k0_pack * A_hstep * packn + (i + ii) * packn + k0_lane;
                    const float* p0a = p0;
                    float absmax0 = 0.f;
                    float absmax1 = 0.f;
                    ...
                    int kk_lane = k0_lane;
                    const size_t vl = packn;
                    const size_t vlq = packn * 4;
                    vuint32m1_t _idx = __riscv_vid_v_u32m1(vl);
                    while (kk < max_kk0)
                    {
                        const int n = std::min(max_kk0 - kk, packn - kk_lane);
                        vbool32_t _mask = __riscv_vmsltu_vx_u32m1_b32(_idx, n, vl);
                        vfloat32m1_t _v0 = __riscv_vle32_v_f32m1_m(_mask, p0a, vl);
                        vfloat32m1_t _v1 = __riscv_vle32_v_f32m1_m(_mask, p0a + packn, vl);
                        _v0 = __riscv_vfmerge_vfm_f32m1(_v0, 0.f, __riscv_vmnot_m_b32(_mask, vl), vl);
                        _v1 = __riscv_vfmerge_vfm_f32m1(_v1, 0.f, __riscv_vmnot_m_b32(_mask, vl), vl);
                        absmax0 = __riscv_vfmv_f_s_f32m1_f32(__riscv_vfredmax_vs_f32m1_f32m1(__riscv_vfabs_v_f32m1(_v0, vl), __riscv_vfmv_s_f_f32m1(absmax0, 1), vl));
                        absmax1 = __riscv_vfmv_f_s_f32m1_f32(__riscv_vfredmax_vs_f32m1_f32m1(__riscv_vfabs_v_f32m1(_v1, vl), __riscv_vfmv_s_f_f32m1(absmax1, 1), vl));
                        p0a += n;
                        kk += n;
                        kk_lane += n;
                        if (kk_lane == packn)
            ...
            if (elempack == packn)
            {
                for (int g = 0; g < local_block_count; g++)
                {
                    ...
                    while (kk < max_kk0)
                    {
                        const int n = std::min(max_kk0 - kk, packn - kk_lane);
                        vbool32_t _mask = __riscv_vmsltu_vx_u32m1_b32(_idx, n, vl);
                        vfloat32m1_t _v = __riscv_vle32_v_f32m1_m(_mask, p0a, vl);
                        _v = __riscv_vfmerge_vfm_f32m1(_v, 0.f, __riscv_vmnot_m_b32(_mask, vl), vl);
                        absmax = __riscv_vfmv_f_s_f32m1_f32(__riscv_vfredmax_vs_f32m1_f32m1(__riscv_vfabs_v_f32m1(_v, vl), __riscv_vfmv_s_f_f32m1(absmax, 1), vl));
                        p0a += n;
                        kk += n;
                        kk_lane += n;
                        if (kk_lane == packn)
    ...
    for (; ii + 1 < max_ii; ii += 2)
    {
        ...
        if (elempack == packn)
        {
            for (int g = 0; g < local_block_count; g++)
            {
                ...
                while (kk < max_kk0)
                {
                    ...
                    vfloat32m1_t _v0 = __riscv_vle32_v_f32m1_m(_mask, p0a, vl);
                    vfloat32m1_t _v1 = __riscv_vle32_v_f32m1_m(_mask, p0a + packn, vl);
                    vfloat32m1_t _s = __riscv_vle32_v_f32m1_m(_mask, psa, vl);
                    _v0 = __riscv_vfmul_vv_f32m1(__riscv_vfabs_v_f32m1(_v0, vl), _s, vl);
                    _v1 = __riscv_vfmul_vv_f32m1(__riscv_vfabs_v_f32m1(_v1, vl), _s, vl);
                    _v0 = __riscv_vfmerge_vfm_f32m1(_v0, 0.f, __riscv_vmnot_m_b32(_mask, vl), vl);
                    _v1 = __riscv_vfmerge_vfm_f32m1(_v1, 0.f, __riscv_vmnot_m_b32(_mask, vl), vl);
                    absmax0 = __riscv_vfmv_f_s_f32m1_f32(__riscv_vfredmax_vs_f32m1_f32m1(_v0, __riscv_vfmv_s_f_f32m1(absmax0, 1), vl));
                    absmax1 = __riscv_vfmv_f_s_f32m1_f32(__riscv_vfredmax_vs_f32m1_f32m1(_v1, __riscv_vfmv_s_f_f32m1(absmax1, 1), vl));
                    p0a += n;
                    psa += n;
                    kk += n;
                    kk_lane += n;
        ...
        if (elempack == 1)
        {
            ...
            for (int g = 0; g < local_block_count; g++)
            {
                ...
                int kk = 0;
#if __riscv_vector
                const size_t vl_absmax = packn;
                vuint32m1_t _idx_absmax = __riscv_vid_v_u32m1(vl_absmax);
                while (kk < max_kk0)
                {
                    const int n = std::min(max_kk0 - kk, packn);
                    vbool32_t _mask = __riscv_vmsltu_vx_u32m1_b32(_idx_absmax, n, vl_absmax);
                    vfloat32m1_t _v0 = __riscv_vlse32_v_f32m1_m(_mask, p0a, (ptrdiff_t)A_hstep * sizeof(float), vl_absmax);
                    vfloat32m1_t _v1 = __riscv_vlse32_v_f32m1_m(_mask, p0a + 1, (ptrdiff_t)A_hstep * sizeof(float), vl_absmax);
                    vfloat32m1_t _s = __riscv_vle32_v_f32m1_m(_mask, psa, vl_absmax);
                    _v0 = __riscv_vfmul_vv_f32m1(__riscv_vfabs_v_f32m1(_v0, vl_absmax), _s, vl_absmax);
                    _v1 = __riscv_vfmul_vv_f32m1(__riscv_vfabs_v_f32m1(_v1, vl_absmax), _s, vl_absmax);
                    _v0 = __riscv_vfmerge_vfm_f32m1(_v0, 0.f, __riscv_vmnot_m_b32(_mask, vl_absmax), vl_absmax);
                    _v1 = __riscv_vfmerge_vfm_f32m1(_v1, 0.f, __riscv_vmnot_m_b32(_mask, vl_absmax), vl_absmax);
                    absmax0 = __riscv_vfmv_f_s_f32m1_f32(__riscv_vfredmax_vs_f32m1_f32m1(_v0, __riscv_vfmv_s_f_f32m1(absmax0, 1), vl_absmax));
                    absmax1 = __riscv_vfmv_f_s_f32m1_f32(__riscv_vfredmax_vs_f32m1_f32m1(_v1, __riscv_vfmv_s_f_f32m1(absmax1, 1), vl_absmax));
                    p0a += n * A_hstep;
                    psa += n;
                    kk += n;
                }
                ...
                while (kk < max_kk0)
                {
                    const int n = std::min(max_kk0 - kk, packn);
                    vbool32_t _mask32 = __riscv_vmsltu_vx_u32m1_b32(_idx, n, vl);
                    vfloat32m1_t _v0 = __riscv_vlse32_v_f32m1_m(_mask32, p0, (ptrdiff_t)A_hstep * sizeof(float), vl);
                    vfloat32m1_t _v1 = __riscv_vlse32_v_f32m1_m(_mask32, p0 + 1, (ptrdiff_t)A_hstep * sizeof(float), vl);
                    vfloat32m1_t _s = __riscv_vle32_v_f32m1_m(_mask32, ps, vl);
                    _v0 = __riscv_vfmul_vv_f32m1(_v0, _s, vl);
                    _v1 = __riscv_vfmul_vv_f32m1(_v1, _s, vl);
                    vint32m1_t _v32_0 = __riscv_vfcvt_x_f_v_i32m1_rm(__riscv_vfmul_vf_f32m1(_v0, scale0, vl), __RISCV_FRM_RMM, vl);
        ...
        if (elempack == packn)
        {
            for (int g = 0; g < local_block_count; g++)
            {
                ...
                while (kk < max_kk0)
                {
                    ...
                    vfloat32m1_t _v = __riscv_vle32_v_f32m1_m(_mask, p0a, vl);
                    vfloat32m1_t _s = __riscv_vle32_v_f32m1_m(_mask, psa, vl);
                    _v = __riscv_vfmul_vv_f32m1(__riscv_vfabs_v_f32m1(_v, vl), _s, vl);
                    _v = __riscv_vfmerge_vfm_f32m1(_v, 0.f, __riscv_vmnot_m_b32(_mask, vl), vl);
                    absmax = __riscv_vfmv_f_s_f32m1_f32(__riscv_vfredmax_vs_f32m1_f32m1(_v, __riscv_vfmv_s_f_f32m1(absmax, 1), vl));
                    p0a += n;
                    psa += n;
                    kk += n;
                    kk_lane += n;
    ...
    for (; ii + 1 < max_ii; ii += 2)
    {
        ...
        for (; jj < max_jj; jj++)
        {
            ...
            for (int kk0 = 0; kk0 < max_kk; kk0 += block_size)
            {
                ...
                for (; kk + 3 < max_kk0; kk += 4)
                {
                    s0 += pA[0] * pB[0] + pA[2] * pB[1] + pA[4] * pB[2] + pA[6] * pB[3];
                    s1 += pA[1] * pB[0] + pA[3] * pB[1] + pA[5] * pB[2] + pA[7] * pB[3];
                    pA += 8;
                    pB += 4;
                }
                for (; kk < max_kk0; kk++)
    ...
    for (; ii < max_ii; ii++)
    {
        ...
        for (; jj < max_jj; jj++)
        {
            ...
            for (int kk0 = 0; kk0 < max_kk; kk0 += block_size)
            {
                ...
                for (; kk + 3 < max_kk0; kk += 4)
                {
                    s += pA[0] * pB[0] + pA[1] * pB[1] + pA[2] * pB[2] + pA[3] * pB[3];
                    pA += 4;
                    pB += 4;
                }
                for (; kk < max_kk0; kk++)
...
static void unpack_output_tile_wq_int8(const Mat& topT, const Mat& C, Mat& top_blob, int broadcast_type_C, int i, int max_ii, int j, int max_jj, float alpha, float beta, int output_elemtype, int output_transpose)
{
    ...
    {
        if (output_transpose)
            outptr = (float*)top_blob + (size_t)j * out_hstep + i * out_elempack;
        else
            outptr = (float*)top_blob + (size_t)i * out_hstep + j * out_elempack;
    }
#if __riscv_zfh
    __fp16* outptr_fp16 = 0;
    if (output_elemtype == 2)
    {
        if (output_transpose)
            outptr_fp16 = (__fp16*)top_blob + (size_t)j / out_elempack * out_hstep * out_elempack + i * out_elempack + j % out_elempack;
        else
            outptr_fp16 = (__fp16*)top_blob + (size_t)i / out_elempack * out_hstep * out_elempack + j * out_elempack + i % out_elempack;
    }
#endif
    ...
    const ptrdiff_t out_stride_fp16 = (ptrdiff_t)out_hstep * sizeof(__fp16);
    bool outptr_high = false;
#endif
    int packn_out = packn;
#if __riscv_zvfh
    if (output_elemtype == 2)
        packn_out = packn_fp16;
#endif
    const float* pC3 = C;
    ...
    for (; ii + (packn - 1) < max_ii; ii += packn)
    {
        ...
        for (int jj = 0; jj < max_jj; jj++)
        {
            ...
            if (output_elemtype == 1)
            {
                if (output_transpose)
                {
                    ...
                    {
                        __riscv_vsse32_v_f32m2(out0, packn * sizeof(float), _sum, vl_packn);
                        if ((jj + 1) % packn == 0)
                            out0 += out_hstep * packn - packn + 1;
                        else
                            out0++;
                    }
                    if (out_elempack == 1)
                    {
                        __riscv_vse32_v_f32m2(out0, _sum, vl_packn);
                        out0 += out_hstep;
                    }
                }
                else
                {
                    if (out_elempack == packn)
                        __riscv_vse32_v_f32m2(out0, _sum, vl_packn);
                    if (out_elempack == 1)
                        __riscv_vsse32_v_f32m2(out0, out_stride, _sum, vl_packn);
                    out0 += out_elempack;
                }
            }
#if __riscv_zvfh
            else // if (output_elemtype == 2)
            {
                vfloat16m1_t _sum_fp16 = __riscv_vfncvt_f_f_w_f16m1(_sum, vl_packn);

                if (output_transpose)
                {
                    if (out_elempack == packn_fp16)
                    {
                        __riscv_vsse16_v_f16m1(out0_fp16, packn_fp16 * sizeof(__fp16), _sum_fp16, vl_packn);
                        if ((jj + 1) % packn_fp16 == 0)
                            out0_fp16 += out_hstep * packn_fp16 - packn_fp16 + 1;
                        else
                            out0_fp16++;
                    }
                    if (out_elempack == 1)
                    {
                        __riscv_vse16_v_f16m1(out0_fp16, _sum_fp16, vl_packn);
                        out0_fp16 += out_hstep;
                    }
                }
                else
                {
                    if (out_elempack == packn_fp16)
                        __riscv_vse16_v_f16m1(out0_fp16, _sum_fp16, vl_packn);
                    if (out_elempack == 1)
                        __riscv_vsse16_v_f16m1(out0_fp16, out_stride_fp16, _sum_fp16, vl_packn);
                    out0_fp16 += out_elempack;
                }
            }
#endif
            pp += packn;
        ...
        else // if (output_elemtype == 2)
        {
            if (output_transpose)
            {
                outptr_fp16 += packn * out_elempack;
            }
            else
            {
                if (out_elempack == packn_fp16)
                {
                    if (outptr_high)
                        outptr_fp16 += out_hstep * packn_fp16 - packn;
                    else
                        outptr_fp16 += packn;
                    outptr_high = !outptr_high;
                }
                if (out_elempack == 1)
                    outptr_fp16 += out_hstep * packn;
            }
        }
#endif
    }
    ...
    for (; ii + 1 < max_ii; ii += 2)
    {
        ...
        int jj = 0;
#if __riscv_vector
        if (output_transpose && out_elempack == packn_out)
        {
            for (; jj < max_jj; jj += packn_out)
            {
                const size_t vl = __riscv_vsetvl_e32m2(packn_out);
                ...
                if (output_elemtype == 1)
                {
                    __riscv_vse32_v_f32m2(out0, _sum0, vl);
                    __riscv_vse32_v_f32m2(out0 + packn_out, _sum1, vl);
                    out0 += out_hstep * packn_out;
                }
#if __riscv_zvfh
                else // if (output_elemtype == 2)
                {
                    vfloat16m1_t _sum0_fp16 = __riscv_vfncvt_f_f_w_f16m1(_sum0, vl);
                    vfloat16m1_t _sum1_fp16 = __riscv_vfncvt_f_f_w_f16m1(_sum1, vl);

                    __riscv_vse16_v_f16m1(out0_fp16, _sum0_fp16, vl);
                    __riscv_vse16_v_f16m1(out0_fp16 + packn_out, _sum1_fp16, vl);
                    out0_fp16 += out_hstep * packn_out;
                }
#endif
                pp += packn_out * 2;
            }
            ...
            while (jj + 3 < max_jj)
            {
                ...
                if (output_elemtype == 1)
                {
                    ...
                    else
                    {
                        ...
                        __riscv_vse32_v_f32m4(out0 + out_hstep, _sum1, vl);
                        out0 += vl;
                    }
                }
#if __riscv_zvfh
                ...
                {
                    vfloat16m2_t _sum0_fp16 = __riscv_vfncvt_f_f_w_f16m2(_sum0, vl);
                    vfloat16m2_t _sum1_fp16 = __riscv_vfncvt_f_f_w_f16m2(_sum1, vl);

                    if (output_transpose)
                    {
                        vfloat16m2x2_t _sum = __riscv_vcreate_v_f16m2x2(_sum0_fp16, _sum1_fp16);
                        if (out_hstep == 2)
                            __riscv_vsseg2e16_v_f16m2x2(out0_fp16, _sum, vl);
                        else
                            __riscv_vssseg2e16_v_f16m2x2(out0_fp16, out_stride_fp16, _sum, vl);
                        out0_fp16 += out_hstep * vl;
                    }
                    else
                    {
                        __riscv_vse16_v_f16m2(out0_fp16, _sum0_fp16, vl);
                        __riscv_vse16_v_f16m2(out0_fp16 + out_hstep, _sum1_fp16, vl);
                        out0_fp16 += vl;
                    }
                }
#endif
        ...
        for (; jj + 3 < max_jj; jj += 4)
        {
            ...
            if (output_elemtype == 1)
            {
                ...
                else
                {
                    ...
                    out0[2] = sum02;
                    out0[3] = sum03;
                    out0[out_hstep] = sum10;
                    out0[out_hstep + 1] = sum11;
                    out0[out_hstep + 2] = sum12;
                    out0[out_hstep + 3] = sum13;
                    out0 += 4;
                }
            }
#if __riscv_zfh
            ...
            {
                __fp16 sum00_fp16 = (__fp16)sum00;
                __fp16 sum10_fp16 = (__fp16)sum10;
                __fp16 sum01_fp16 = (__fp16)sum01;
                __fp16 sum11_fp16 = (__fp16)sum11;
                __fp16 sum02_fp16 = (__fp16)sum02;
                __fp16 sum12_fp16 = (__fp16)sum12;
                __fp16 sum03_fp16 = (__fp16)sum03;
                __fp16 sum13_fp16 = (__fp16)sum13;

                if (output_transpose)
                {
                    out0_fp16[0] = sum00_fp16;
                    out0_fp16[1] = sum10_fp16;
                    out0_fp16[out_hstep] = sum01_fp16;
                    out0_fp16[out_hstep + 1] = sum11_fp16;
                    out0_fp16[out_hstep * 2] = sum02_fp16;
                    out0_fp16[out_hstep * 2 + 1] = sum12_fp16;
                    out0_fp16[out_hstep * 3] = sum03_fp16;
                    out0_fp16[out_hstep * 3 + 1] = sum13_fp16;
                    out0_fp16 += out_hstep * 4;
                }
                else
                {
                    out0_fp16[0] = sum00_fp16;
                    out0_fp16[1] = sum01_fp16;
                    out0_fp16[2] = sum02_fp16;
                    out0_fp16[3] = sum03_fp16;
                    out0_fp16[out_hstep] = sum10_fp16;
                    out0_fp16[out_hstep + 1] = sum11_fp16;
                    out0_fp16[out_hstep + 2] = sum12_fp16;
                    out0_fp16[out_hstep + 3] = sum13_fp16;
                    out0_fp16 += 4;
                }
            }
#endif
        ...
        for (; jj < max_jj; jj++)
        {
            ...
            {
                __fp16 sum0_fp16 = (__fp16)sum0;
                __fp16 sum1_fp16 = (__fp16)sum1;

                if (output_transpose)
                {
                    out0_fp16[0] = sum0_fp16;
                    out0_fp16[1] = sum1_fp16;
                    out0_fp16 += out_hstep;
                }
                else
                {
                    out0_fp16[0] = sum0_fp16;
                    out0_fp16[out_hstep] = sum1_fp16;
                    out0_fp16++;
                }
            }
#endif
        ...
        {
            if (output_transpose)
                outptr_fp16 += 2 * out_elempack;
            else
                outptr_fp16 += out_hstep * 2;
        }
#endif
    }
    for (; ii < max_ii; ii++)
    {
        ...
        if (output_transpose && out_elempack == packn_out)
        {
            for (; jj < max_jj; jj += packn_out)
            {
                ...
                if (output_elemtype == 1)
                {
                    __riscv_vse32_v_f32m2(out0, _sum, vl);
                    out0 += out_hstep * packn_out;
                }
#if __riscv_zvfh
                else // if (output_elemtype == 2)
                {
                    vfloat16m1_t _sum_fp16 = __riscv_vfncvt_f_f_w_f16m1(_sum, vl);

                    __riscv_vse16_v_f16m1(out0_fp16, _sum_fp16, vl);
                    out0_fp16 += out_hstep * packn_out;
                }
#endif
                pp += packn_out;
        ...
        if (out_elempack == 1)
        {
            while (jj + 3 < max_jj)
            {
                ...
                if (output_elemtype == 1)
                {
                    if (!output_transpose || out_hstep == 1)
                ...
                {
                    vfloat16m2_t _sum_fp16 = __riscv_vfncvt_f_f_w_f16m2(_sum, vl);

                    if (!output_transpose || out_hstep == 1)
                        __riscv_vse16_v_f16m2(out0_fp16, _sum_fp16, vl);
                    else
                        __riscv_vsse16_v_f16m2(out0_fp16, out_stride_fp16, _sum_fp16, vl);
                    if (output_transpose)
                        out0_fp16 += out_hstep * vl;
                    else
                        out0_fp16 += vl;
        ...
        for (; jj + 3 < max_jj; jj += 4)
        {
            ...
            else // if (output_elemtype == 2)
            {
                __fp16 sum0_fp16 = (__fp16)sum0;
                __fp16 sum1_fp16 = (__fp16)sum1;
                __fp16 sum2_fp16 = (__fp16)sum2;
                __fp16 sum3_fp16 = (__fp16)sum3;

                if (output_transpose)
                {
                    out0_fp16[0] = sum0_fp16;
                    out0_fp16[out_hstep] = sum1_fp16;
                    out0_fp16[out_hstep * 2] = sum2_fp16;
                    out0_fp16[out_hstep * 3] = sum3_fp16;
                    out0_fp16 += out_hstep * 4;
                }
                else
                {
                    out0_fp16[0] = sum0_fp16;
                    out0_fp16[1] = sum1_fp16;
                    out0_fp16[2] = sum2_fp16;
                    out0_fp16[3] = sum3_fp16;
                    out0_fp16 += 4;
                }
            }
#endif
        ...
        {
            if (output_transpose)
                outptr_fp16 += out_elempack;
            else
                outptr_fp16 += out_hstep;
        }
#endif
    }
}
