...
#if NCNN_ZFH
int Softplus_riscv::forward_inplace_fp16s(Mat& bottom_top_blob, const Option& opt) const
{
...
int Softplus_riscv::forward_inplace_fp16sa(Mat& bottom_top_blob, const Option& opt) const
{
    const int w = bottom_top_blob.w;
    const int h = bottom_top_blob.h;
    const int d = bottom_top_blob.d;
    ...
    for (int q = 0; q < channels; q++)
    {
        ...
        while (n > 0)
        {
            ...
            _p = exp_ps(_p, vl);
            _p = __riscv_vfadd_vf_f16m8(_p, (__fp16)1.f, vl);
            _p = log_ps(_p, vl);
