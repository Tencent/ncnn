...
static void hardsigmoid_fp32(Mat& bottom_top_blob, float alpha, float beta, float lower, float upper, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_FMA && __AVX__ && !__FMA__ && !__FMA4__
    if (ncnn::cpu_support_x86_fma())
    {
        hardsigmoid_fp32_fma(bottom_top_blob, alpha, beta, lower, upper, opt);
        return;
    }
#endif
#if NCNN_RUNTIME_CPU && NCNN_FMA4 && __AVX__ && !__FMA__ && !__FMA4__
    ...
    {
        hardsigmoid_fp32_fma4(bottom_top_blob, alpha, beta, lower, upper, opt);
        return;
    }
#endif
