...
#if __AVX2__
static void cumulative_sum_prefix_sum_row_avx2(float* ptr, int w)
{
    int j = 0;
    float sum = 0.f;
...
static void cumulative_sum_add_avx2(const float* ptr, float* outptr, int size)
{
    int i = 0;
...
static int cumulative_sum_forward_inplace_avx2_impl(Mat& bottom_top_blob, int axis, const Option& opt)
{
    ...
    if (dims == 1)
    {
        cumulative_sum_prefix_sum_row_avx2(bottom_top_blob, bottom_top_blob.w);
        return 0;
    }
    ...
    if (dims == 2 && positive_axis == 0)
    {
        ...
        for (int i = 1; i < h; i++)
        {
            ...
            cumulative_sum_add_avx2(prev_row, this_row, w);
        }
        return 0;
    }
    ...
    if (dims == 2 && positive_axis == 1)
    {
        ...
        for (int i = 0; i < h; i++)
        {
            cumulative_sum_prefix_sum_row_avx2(bottom_top_blob.row(i), w);
        }
        return 0;
    }
    ...
    if (dims == 3 && positive_axis == 0)
    {
        ...
        for (int q = 1; q < c; q++)
        {
            ...
            cumulative_sum_add_avx2(prev, cur, size);
        }
        return 0;
    }
    ...
    if (dims == 3 && positive_axis == 2)
    {
        ...
        for (int idx = 0; idx < c * h; idx++)
        {
            const int q = idx / h;
            const int i = idx - q * h;
            cumulative_sum_prefix_sum_row_avx2(bottom_top_blob.channel(q).row(i), w);
        }
        return 0;
    }
