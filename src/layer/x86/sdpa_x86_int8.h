...
static void dynamic_quantize_blockwise_scalar(const float* src, signed char* dst, float* scales, int width)
{
...
#if __SSE2__
static inline void dynamic_quantize_blockwise_sse2(const float* src, signed char* dst, float* scales, int width)
{
    const int block_size = 32;
    int num_blocks = (width + block_size - 1) / block_size;
...
#if __AVX2__
inline void __attribute__((noinline)) dynamic_quantize_blockwise_avx2(const float* src, signed char* dst, float* scales, int width)
{
    const int block_size = 32;
    int num_blocks = (width + block_size - 1) / block_size;
...
#if __AVX512F__
static inline void dynamic_quantize_blockwise_avx512(const float* src, signed char* dst, float* scales, int width)
{
    const int block_size = 32;
    int num_blocks = (width + block_size - 1) / block_size;
...
static void dynamic_quantize_blockwise_dispatch(const float* src, signed char* dst, float* scales, int width)
{
#if __AVX512F__
    dynamic_quantize_blockwise_avx512(src, dst, scales, width);
#else
#if NCNN_RUNTIME_CPU && NCNN_AVX2 && __AVX__ && !__AVX2__
    ...
    dynamic_quantize_blockwise_avx2(src, dst, scales, width);
#elif __SSE2__
    dynamic_quantize_blockwise_sse2(src, dst, scales, width);
#else
    dynamic_quantize_blockwise_scalar(src, dst, scales, width);
#endif
#endif
}

static inline void dynamic_quantize_rowwise_scalar(const float* src, signed char* dst, float* scale, int width)
{
    float absmax = 0.f;
...
#if __SSE2__
static inline void dynamic_quantize_rowwise_sse2(const float* src, signed char* dst, float* scale, int width)
{
...
#if __AVX2__
inline void __attribute__((noinline)) dynamic_quantize_rowwise_avx2(const float* src, signed char* dst, float* scale, int width)
{
    __m256 sign_mask = _mm256_set1_ps(-0.f);
    __m256 vmax = _mm256_setzero_ps();
    int i = 0;
...
#if __AVX512F__
static inline void dynamic_quantize_rowwise_avx512(const float* src, signed char* dst, float* scale, int width)
{
    __m512 sign_mask = _mm512_set1_ps(-0.f);
    __m512 vmax = _mm512_setzero_ps();
    int i = 0;
...
static void dynamic_quantize_rowwise_dispatch(const float* src, signed char* dst, float* scale, int width)
{
#if __AVX512F__
    dynamic_quantize_rowwise_avx512(src, dst, scale, width);
#else
#if NCNN_RUNTIME_CPU && NCNN_AVX2 && __AVX__ && !__AVX2__
    if (ncnn::cpu_support_x86_avx2())
    {
    ...
    dynamic_quantize_rowwise_avx2(src, dst, scale, width);
#elif __SSE2__
    dynamic_quantize_rowwise_sse2(src, dst, scale, width);
#else
    dynamic_quantize_rowwise_scalar(src, dst, scale, width);
#endif
#endif
}
...
static inline int qk_int8_dot_block_scalar(const signed char* a, const signed char* b, int len)
{
    int sum = 0;
    for (int i = 0; i < len; i++)
        sum += a[i] * b[i];
...
#if __SSE2__
static inline int qk_int8_dot_block_sse2(const signed char* a, const signed char* b, int len)
{
...
#if __AVX2__
inline int __attribute__((noinline)) qk_int8_dot_block_avx2(const signed char* a, const signed char* b, int len)
{
    ...
    for (; i + 31 < len; i += 32)
    {
        ...
        sum = _mm256_add_epi32(sum, _mm256_madd_epi16(va_lo, vb_lo));
        sum = _mm256_add_epi32(sum, _mm256_madd_epi16(va_hi, vb_hi));
    }
    int sum_tail = 0;
    for (; i < len; i++)
        sum_tail += a[i] * b[i];
...
#if __AVX512F__
static inline int qk_int8_dot_block_avx512(const signed char* a, const signed char* b, int len)
{
    ...
    for (; i + 31 < len; i += 32)
    {
        ...
        sum = _mm512_add_epi32(sum, _mm512_madd_epi16(va, vb));
    }
    int sum_tail = 0;
    for (; i < len; i++)
        sum_tail += a[i] * b[i];
...
{
#if __AVX512F__
    return qk_int8_dot_block_avx512(a, b, len);
#else
#if NCNN_RUNTIME_CPU && NCNN_AVX2 && __AVX__ && !__AVX2__
    if (ncnn::cpu_support_x86_avx2())
    {
    ...
    return qk_int8_dot_block_avx2(a, b, len);
#elif __SSE2__
    return qk_int8_dot_block_sse2(a, b, len);
#else
    return qk_int8_dot_block_scalar(a, b, len);
#endif
#endif
}
...
static inline void decode_qk_dot_int8_scalar(float* s, const signed char* q,
        ...
        int n_start, int block_n, int d, float scale)
{
    ...
    for (int j = 0; j < block_n; j++)
    {
        ...
        const float* ks = kscales + (n_start + j) * num_blocks;
        float sum = 0.f;
        for (int b = 0; b < num_blocks; b++)
        {
            int off = b * 32;
            ...
            if (len <= 0) continue;
            int block_sum = qk_int8_dot_block_scalar(q + off, kptr + off, len);
            sum += (float)block_sum / (qscales[b] * ks[b]);
        }
        s[j] = sum * scale;
    }
}
...
#if __SSE2__
static inline void decode_qk_dot_int8_sse2(float* s, const signed char* q,
        const signed char* K, const float* qscales, const float* kscales,
        int n_start, int block_n, int d, float scale)
{
    const int num_blocks = (d + 31) / 32;
    ...
    for (; j + 1 < block_n; j += 2)
    {
        ...
        const float* ks0 = kscales + (n_start + j + 0) * num_blocks;
        const float* ks1 = kscales + (n_start + j + 1) * num_blocks;

        float sum0 = 0.f, sum1 = 0.f;
        for (int b = 0; b < num_blocks; b++)
        {
            float descale = qscales[b] * ks0[b];
            int off = b * 32;
            int len = std::min(32, d - off);
            if (len <= 0) continue;
            int block_sum = qk_int8_dot_block_sse2(q + off, k0 + off, len);
            sum0 += (float)block_sum * descale;
        }
        for (int b = 0; b < num_blocks; b++)
        {
            float descale = qscales[b] * ks1[b];
            int off = b * 32;
            int len = std::min(32, d - off);
            if (len <= 0) continue;
            int block_sum = qk_int8_dot_block_sse2(q + off, k1 + off, len);
...
#if __AVX2__
inline void __attribute__((noinline)) decode_qk_dot_int8_avx2(float* s, const signed char* q,
        const signed char* K, const float* qscales, const float* kscales,
        int n_start, int block_n, int d, float scale)
{
    const int num_blocks = (d + 31) / 32;
    ...
    for (; j + 1 < block_n; j += 2)
    {
        ...
        for (int b = 0; b < num_blocks; b++)
        {
            ...
            int len = std::min(32, d - off);
            if (len <= 0) continue;
            int block_sum0 = qk_int8_dot_block_avx2(q + off, k0 + off, len);
            int block_sum1 = qk_int8_dot_block_avx2(q + off, k1 + off, len);
            sum0 += (float)block_sum0 / (qscales[b] * ks0[b]);
            sum1 += (float)block_sum1 / (qscales[b] * ks1[b]);
        }
    ...
    for (; j < block_n; j++)
    {
        ...
        for (int b = 0; b < num_blocks; b++)
        {
            ...
            int len = std::min(32, d - off);
            if (len <= 0) continue;
            int block_sum = qk_int8_dot_block_avx2(q + off, kptr + off, len);
            float descale = qscales[b] * ks[b];
            sum += (float)block_sum * descale;
        }
        s[j] = sum * scale;
...
#if __AVX512F__
static inline void decode_qk_dot_int8_avx512(float* s, const signed char* q,
        const signed char* K, const float* qscales, const float* kscales,
        int n_start, int block_n, int d, float scale)
{
    const int num_blocks = (d + 31) / 32;
    ...
    for (; j + 3 < block_n; j += 4)
    {
        ...
        const float* ks0 = kscales + (n_start + j + 0) * num_blocks;
        const float* ks1 = kscales + (n_start + j + 1) * num_blocks;
        const float* ks2 = kscales + (n_start + j + 2) * num_blocks;
        const float* ks3 = kscales + (n_start + j + 3) * num_blocks;
        ...
        for (int b = 0; b < num_blocks; b++)
        {
            ...
            if (len <= 0) continue;
            int bs;
            bs = qk_int8_dot_block_avx512(q + off, k0 + off, len);
            sum0 += (float)bs / (qscales[b] * ks0[b]);
            bs = qk_int8_dot_block_avx512(q + off, k1 + off, len);
            sum1 += (float)bs / (qscales[b] * ks1[b]);
            bs = qk_int8_dot_block_avx512(q + off, k2 + off, len);
            sum2 += (float)bs / (qscales[b] * ks2[b]);
            bs = qk_int8_dot_block_avx512(q + off, k3 + off, len);
            sum3 += (float)bs / (qscales[b] * ks3[b]);
        }
    ...
    for (; j < block_n; j++)
    {
        ...
        for (int b = 0; b < num_blocks; b++)
        {
            ...
            int len = std::min(32, d - off);
            if (len <= 0) continue;
            int block_sum = qk_int8_dot_block_avx512(q + off, kptr + off, len);
            float descale = qscales[b] * ks[b];
            sum += (float)block_sum * descale;
        }
        s[j] = sum * scale;
...
static void decode_qk_dot_int8_dispatch(float* s, const signed char* q,
        const signed char* K, const float* qscales, const float* kscales,
        int n_start, int block_n, int d, float scale)
{
#if __AVX512F__
    decode_qk_dot_int8_avx512(s, q, K, qscales, kscales, n_start, block_n, d, scale);
#else
#if NCNN_RUNTIME_CPU && NCNN_AVX2 && __AVX__ && !__AVX2__
    if (ncnn::cpu_support_x86_avx2())
    {
    ...
    decode_qk_dot_int8_avx2(s, q, K, qscales, kscales, n_start, block_n, d, scale);
#elif __SSE2__
    decode_qk_dot_int8_sse2(s, q, K, qscales, kscales, n_start, block_n, d, scale);
#else
    decode_qk_dot_int8_scalar(s, q, K, qscales, kscales, n_start, block_n, d, scale);
#endif
#endif
}
...
static inline void qk_int8_gemm_row_scalar(float* s_row,
        ...
        int n, int d, float scale)
{
    ...
    for (int j = 0; j < n; j++)
    {
        ...
        for (int b = 0; b < num_blocks; b++)
        {
            ...
            int len = std::min(32, d - off);
            if (len <= 0) continue;
            sum += qk_int8_dot_block_scalar(q_row + off, kptr + off, len);
        }
...
#if __SSE2__
static inline void qk_int8_gemm_row_sse2(float* s_row,
        const signed char* q_row, const signed char* K, float qscale, const float* kscales,
        int n, int d, float scale)
{
    const int num_blocks = (d + 31) / 32;
    ...
    for (; j + 1 < n; j += 2)
    {
        ...
        for (int b = 0; b < num_blocks; b++)
        {
            ...
            int len = std::min(32, d - off);
            if (len <= 0) continue;
            sum0 += qk_int8_dot_block_sse2(q_row + off, k0 + off, len);
            sum1 += qk_int8_dot_block_sse2(q_row + off, k1 + off, len);
        }
...
#if __AVX2__
inline void __attribute__((noinline)) qk_int8_gemm_row_avx2(float* s_row,
        const signed char* q_row, const signed char* K, float qscale, const float* kscales,
        int n, int d, float scale)
{
    const int num_blocks = (d + 31) / 32;
    ...
    for (; j + 1 < n; j += 2)
    {
        ...
        for (int b = 0; b < num_blocks; b++)
        {
            ...
            int len = std::min(32, d - off);
            if (len <= 0) continue;
            sum0 += qk_int8_dot_block_avx2(q_row + off, k0 + off, len);
            sum1 += qk_int8_dot_block_avx2(q_row + off, k1 + off, len);
        }
        s_row[j] = (float)sum0 * qscale * kscales[j] * scale;
        s_row[j + 1] = (float)sum1 * qscale * kscales[j + 1] * scale;
    }
    for (; j < n; j++)
    {
        ...
        for (int b = 0; b < num_blocks; b++)
        {
            ...
            int len = std::min(32, d - off);
            if (len <= 0) continue;
            sum += qk_int8_dot_block_avx2(q_row + off, kptr + off, len);
        }
        s_row[j] = (float)sum * qscale * kscales[j] * scale;
    }
}
...
#if __AVX512F__
static inline void qk_int8_gemm_row_avx512(float* s_row,
        const signed char* q_row, const signed char* K, float qscale, const float* kscales,
        int n, int d, float scale)
{
    const int num_blocks = (d + 31) / 32;
    ...
    for (; j + 3 < n; j += 4)
    {
        ...
        for (int b = 0; b < num_blocks; b++)
        {
            ...
            if (len == 32)
            {
                ...
                acc0 = _mm512_add_epi32(acc0, _mm512_madd_epi16(q_512, k0_512));
                acc1 = _mm512_add_epi32(acc1, _mm512_madd_epi16(q_512, k1_512));
                acc2 = _mm512_add_epi32(acc2, _mm512_madd_epi16(q_512, k2_512));
                acc3 = _mm512_add_epi32(acc3, _mm512_madd_epi16(q_512, k3_512));
            }
            else
            {
                int bs;
                bs = qk_int8_dot_block_avx512(q_row + off, k0 + off, len);
                scalar0 += bs;
                bs = qk_int8_dot_block_avx512(q_row + off, k1 + off, len);
                scalar1 += bs;
                bs = qk_int8_dot_block_avx512(q_row + off, k2 + off, len);
                scalar2 += bs;
                bs = qk_int8_dot_block_avx512(q_row + off, k3 + off, len);
                scalar3 += bs;
            }
        }
    ...
    for (; j < n; j++)
    {
        ...
        for (int b = 0; b < num_blocks; b++)
        {
            ...
            if (len == 32)
            {
                ...
                acc = _mm512_add_epi32(acc, _mm512_madd_epi16(q_512, k_512));
            }
            else
            {
                scalar_sum += qk_int8_dot_block_avx512(q_row + off, kptr + off, len);
            }
        }
...
static void qk_int8_gemm_row_dispatch(float* s_row,
        const signed char* q_row, const signed char* K, float qscale, const float* kscales,
        int n, int d, float scale)
{
#if __AVX512F__
    qk_int8_gemm_row_avx512(s_row, q_row, K, qscale, kscales, n, d, scale);
#else
#if NCNN_RUNTIME_CPU && NCNN_AVX2 && __AVX__ && !__AVX2__
    if (ncnn::cpu_support_x86_avx2())
    {
    ...
    qk_int8_gemm_row_avx2(s_row, q_row, K, qscale, kscales, n, d, scale);
#elif __SSE2__
    qk_int8_gemm_row_sse2(s_row, q_row, K, qscale, kscales, n, d, scale);
#else
    qk_int8_gemm_row_scalar(s_row, q_row, K, qscale, kscales, n, d, scale);
#endif
#endif
}
...
static inline void qk_int8_gemm_tiled_scalar(float* S,
        ...
        int m, int n, int d, float scale)
{
    ...
    for (; i + 2 <= m; i += 2)
    {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            for (int b = 0; b < num_blocks; b++)
            {
                ...
                int len = std::min(32, d - off);
                if (len <= 0) continue;
                sum0 += qk_int8_dot_block_scalar(q0 + off, kptr + off, len);
                sum1 += qk_int8_dot_block_scalar(q1 + off, kptr + off, len);
            }
    ...
    for (; i < m; i++)
    {
        qk_int8_gemm_row_scalar(S + i * n, Q + i * d, K, qscales[i], kscales, n, d, scale);
    }
}

#if __SSE2__
static inline void qk_int8_gemm_tiled_sse2(float* S,
        const signed char* Q, const signed char* K,
        const float* qscales, const float* kscales,
        int m, int n, int d, float scale)
{
    ...
    for (; i + 2 <= m; i += 2)
    {
        ...
        for (; j + 1 < n; j += 2)
        {
            ...
            for (int b = 0; b < num_blocks; b++)
            {
                ...
                if (len <= 0) continue;
                int bs;
                bs = qk_int8_dot_block_sse2(q0 + off, k0 + off, len);
                sum00 += bs;
                bs = qk_int8_dot_block_sse2(q0 + off, k1 + off, len);
                sum01 += bs;
                bs = qk_int8_dot_block_sse2(q1 + off, k0 + off, len);
                sum10 += bs;
                bs = qk_int8_dot_block_sse2(q1 + off, k1 + off, len);
                sum11 += bs;
            }
        ...
        for (; j < n; j++)
        {
            ...
            for (int b = 0; b < num_blocks; b++)
            {
                ...
                int len = std::min(32, d - off);
                if (len <= 0) continue;
                sum0 += qk_int8_dot_block_sse2(q0 + off, kptr + off, len);
                sum1 += qk_int8_dot_block_sse2(q1 + off, kptr + off, len);
            }
            S[(i + 0) * n + j] = (float)sum0 * qs0 * kscales[j] * scale;
            S[(i + 1) * n + j] = (float)sum1 * qs1 * kscales[j] * scale;
        }
    ...
    for (; i < m; i++)
    {
        qk_int8_gemm_row_sse2(S + i * n, Q + i * d, K, qscales[i], kscales, n, d, scale);
    }
}
#endif // __SSE2__
...
inline void __attribute__((noinline)) qk_int8_gemm_tiled_avx2(float* S,
        const signed char* Q, const signed char* K,
        const float* qscales, const float* kscales,
        int m, int n, int d, float scale)
{
    ...
    for (; i + 2 <= m; i += 2)
    {
        ...
        for (; j + 3 < n; j += 4)
        {
            ...
            for (int b = 0; b < num_blocks; b++)
            {
                ...
                if (len == 32)
                {
                    ...
                    acc00 = _mm256_add_epi32(acc00, _mm256_madd_epi16(q0_lo, k0_lo));
                    acc00 = _mm256_add_epi32(acc00, _mm256_madd_epi16(q0_hi, k0_hi));
                    acc10 = _mm256_add_epi32(acc10, _mm256_madd_epi16(q1_lo, k0_lo));
                    acc10 = _mm256_add_epi32(acc10, _mm256_madd_epi16(q1_hi, k0_hi));
                    ...
                    acc01 = _mm256_add_epi32(acc01, _mm256_madd_epi16(q0_lo, k1_lo));
                    acc01 = _mm256_add_epi32(acc01, _mm256_madd_epi16(q0_hi, k1_hi));
                    acc11 = _mm256_add_epi32(acc11, _mm256_madd_epi16(q1_lo, k1_lo));
                    acc11 = _mm256_add_epi32(acc11, _mm256_madd_epi16(q1_hi, k1_hi));
                    ...
                    acc02 = _mm256_add_epi32(acc02, _mm256_madd_epi16(q0_lo, k2_lo));
                    acc02 = _mm256_add_epi32(acc02, _mm256_madd_epi16(q0_hi, k2_hi));
                    acc12 = _mm256_add_epi32(acc12, _mm256_madd_epi16(q1_lo, k2_lo));
                    acc12 = _mm256_add_epi32(acc12, _mm256_madd_epi16(q1_hi, k2_hi));
                    ...
                    acc03 = _mm256_add_epi32(acc03, _mm256_madd_epi16(q0_lo, k3_lo));
                    acc03 = _mm256_add_epi32(acc03, _mm256_madd_epi16(q0_hi, k3_hi));
                    acc13 = _mm256_add_epi32(acc13, _mm256_madd_epi16(q1_lo, k3_lo));
                    acc13 = _mm256_add_epi32(acc13, _mm256_madd_epi16(q1_hi, k3_hi));
                }
                else
                {
                    int bs;
                    bs = qk_int8_dot_block_avx2(q0 + off, k0 + off, len);
                    scalar00 += bs;
                    bs = qk_int8_dot_block_avx2(q0 + off, k1 + off, len);
                    scalar01 += bs;
                    bs = qk_int8_dot_block_avx2(q0 + off, k2 + off, len);
                    scalar02 += bs;
                    bs = qk_int8_dot_block_avx2(q0 + off, k3 + off, len);
                    scalar03 += bs;
                    bs = qk_int8_dot_block_avx2(q1 + off, k0 + off, len);
                    scalar10 += bs;
                    bs = qk_int8_dot_block_avx2(q1 + off, k1 + off, len);
                    scalar11 += bs;
                    bs = qk_int8_dot_block_avx2(q1 + off, k2 + off, len);
                    scalar12 += bs;
                    bs = qk_int8_dot_block_avx2(q1 + off, k3 + off, len);
                    scalar13 += bs;
                }
            }
        ...
        for (; j < n; j++)
        {
            ...
            for (int b = 0; b < num_blocks; b++)
            {
                ...
                int len = std::min(32, d - off);
                if (len <= 0) continue;
                sum0 += qk_int8_dot_block_avx2(q0 + off, kptr + off, len);
                sum1 += qk_int8_dot_block_avx2(q1 + off, kptr + off, len);
            }
            S[(i + 0) * n + j] = (float)sum0 * qs0 * kscales[j] * scale;
            S[(i + 1) * n + j] = (float)sum1 * qs1 * kscales[j] * scale;
        }
    ...
    for (; i < m; i++)
    {
        qk_int8_gemm_row_avx2(S + i * n, Q + i * d, K, qscales[i], kscales, n, d, scale);
    }
}
#endif // __AVX2__

#if __AVX512F__
static inline void qk_int8_gemm_tiled_avx512(float* S,
        const signed char* Q, const signed char* K,
        const float* qscales, const float* kscales,
        int m, int n, int d, float scale)
{
        ...
        for (; j + 3 < n; j += 4)
        {
            ...
            for (int b = 0; b < num_blocks; b++)
            {
                ...
                if (len == 32)
                {
                    ...
                    acc00 = _mm512_add_epi32(acc00, _mm512_madd_epi16(q0_512, k0_512));
                    acc01 = _mm512_add_epi32(acc01, _mm512_madd_epi16(q0_512, k1_512));
                    acc02 = _mm512_add_epi32(acc02, _mm512_madd_epi16(q0_512, k2_512));
                    acc03 = _mm512_add_epi32(acc03, _mm512_madd_epi16(q0_512, k3_512));
                    acc10 = _mm512_add_epi32(acc10, _mm512_madd_epi16(q1_512, k0_512));
                    acc11 = _mm512_add_epi32(acc11, _mm512_madd_epi16(q1_512, k1_512));
                    acc12 = _mm512_add_epi32(acc12, _mm512_madd_epi16(q1_512, k2_512));
                    acc13 = _mm512_add_epi32(acc13, _mm512_madd_epi16(q1_512, k3_512));
                    acc20 = _mm512_add_epi32(acc20, _mm512_madd_epi16(q2_512, k0_512));
                    acc21 = _mm512_add_epi32(acc21, _mm512_madd_epi16(q2_512, k1_512));
                    acc22 = _mm512_add_epi32(acc22, _mm512_madd_epi16(q2_512, k2_512));
                    acc23 = _mm512_add_epi32(acc23, _mm512_madd_epi16(q2_512, k3_512));
                    acc30 = _mm512_add_epi32(acc30, _mm512_madd_epi16(q3_512, k0_512));
                    acc31 = _mm512_add_epi32(acc31, _mm512_madd_epi16(q3_512, k1_512));
                    acc32 = _mm512_add_epi32(acc32, _mm512_madd_epi16(q3_512, k2_512));
                    acc33 = _mm512_add_epi32(acc33, _mm512_madd_epi16(q3_512, k3_512));
                }
                else
                {
                    int bs;
                    bs = qk_int8_dot_block_avx512(q0 + off, k0 + off, len);
                    scalar00 += bs;
                    bs = qk_int8_dot_block_avx512(q0 + off, k1 + off, len);
                    scalar01 += bs;
                    bs = qk_int8_dot_block_avx512(q0 + off, k2 + off, len);
                    scalar02 += bs;
                    bs = qk_int8_dot_block_avx512(q0 + off, k3 + off, len);
                    scalar03 += bs;
                    bs = qk_int8_dot_block_avx512(q1 + off, k0 + off, len);
                    scalar10 += bs;
                    bs = qk_int8_dot_block_avx512(q1 + off, k1 + off, len);
                    scalar11 += bs;
                    bs = qk_int8_dot_block_avx512(q1 + off, k2 + off, len);
                    scalar12 += bs;
                    bs = qk_int8_dot_block_avx512(q1 + off, k3 + off, len);
                    scalar13 += bs;
                    bs = qk_int8_dot_block_avx512(q2 + off, k0 + off, len);
                    scalar20 += bs;
                    bs = qk_int8_dot_block_avx512(q2 + off, k1 + off, len);
                    scalar21 += bs;
                    bs = qk_int8_dot_block_avx512(q2 + off, k2 + off, len);
                    scalar22 += bs;
                    bs = qk_int8_dot_block_avx512(q2 + off, k3 + off, len);
                    scalar23 += bs;
                    bs = qk_int8_dot_block_avx512(q3 + off, k0 + off, len);
                    scalar30 += bs;
                    bs = qk_int8_dot_block_avx512(q3 + off, k1 + off, len);
                    scalar31 += bs;
                    bs = qk_int8_dot_block_avx512(q3 + off, k2 + off, len);
                    scalar32 += bs;
                    bs = qk_int8_dot_block_avx512(q3 + off, k3 + off, len);
                    scalar33 += bs;
                }
            }
            float descale00 = qs0 * kscales[j];
        ...
        for (; j < n; j++)
        {
            ...
            for (int b = 0; b < num_blocks; b++)
            {
                ...
                if (len == 32)
                {
                    ...
                    acc0 = _mm512_add_epi32(acc0, _mm512_madd_epi16(q0_512, k_512));
                    acc1 = _mm512_add_epi32(acc1, _mm512_madd_epi16(q1_512, k_512));
                    acc2 = _mm512_add_epi32(acc2, _mm512_madd_epi16(q2_512, k_512));
                    acc3 = _mm512_add_epi32(acc3, _mm512_madd_epi16(q3_512, k_512));
                }
                else
                {
                    int bs;
                    bs = qk_int8_dot_block_avx512(q0 + off, kptr + off, len);
                    scalar0 += bs;
                    bs = qk_int8_dot_block_avx512(q1 + off, kptr + off, len);
                    scalar1 += bs;
                    bs = qk_int8_dot_block_avx512(q2 + off, kptr + off, len);
                    scalar2 += bs;
                    bs = qk_int8_dot_block_avx512(q3 + off, kptr + off, len);
                    scalar3 += bs;
                }
            }
    ...
    for (; i < m; i++)
    {
        qk_int8_gemm_row_avx512(S + i * n, Q + i * d, K, qscales[i], kscales, n, d, scale);
    }
}
#endif // __AVX512F__

static void qk_int8_gemm_tiled_dispatch(float* S,
        const signed char* Q, const signed char* K,
        const float* qscales, const float* kscales,
        int m, int n, int d, float scale)
{
#if __AVX512F__
    qk_int8_gemm_tiled_avx512(S, Q, K, qscales, kscales, m, n, d, scale);
#else
#if NCNN_RUNTIME_CPU && NCNN_AVX2 && __AVX__ && !__AVX2__
    if (ncnn::cpu_support_x86_avx2())
    {
    ...
    qk_int8_gemm_tiled_avx2(S, Q, K, qscales, kscales, m, n, d, scale);
#elif __SSE2__
    qk_int8_gemm_tiled_sse2(S, Q, K, qscales, kscales, m, n, d, scale);
#else
    qk_int8_gemm_tiled_scalar(S, Q, K, qscales, kscales, m, n, d, scale);
#endif
#endif
}
...
static inline void decode_pv_gemv_int8_scalar(float* out, const float* s,
...
#if __SSE2__
static inline void decode_pv_gemv_int8_sse2(float* out, const float* s,
        const signed char* V, const float* vscales,
        int n_start, int block_n, int out_d)
{
    const int num_blocks = (out_d + 31) / 32;
...
#if __AVX2__
inline void __attribute__((noinline)) decode_pv_gemv_int8_avx2(float* out, const float* s,
        const signed char* V, const float* vscales,
        int n_start, int block_n, int out_d)
{
    const int num_blocks = (out_d + 31) / 32;
...
#if __AVX512F__
static inline void decode_pv_gemv_int8_avx512(float* out, const float* s,
        const signed char* V, const float* vscales,
        int n_start, int block_n, int out_d)
{
    const int num_blocks = (out_d + 31) / 32;
    for (int vb = 0; vb < num_blocks; vb++)
    {
        ...
        for (; k + 15 < k_end; k += 16)
        {
            ...
            for (int j = 0; j < block_n; j++)
            {
                ...
                __m128i v8 = _mm_loadu_si128((const __m128i*)(V + (n_start + j) * out_d + k));
...
static void decode_pv_gemv_int8_dispatch(float* out, const float* s,
        const signed char* V, const float* vscales,
        int n_start, int block_n, int out_d)
{
#if __AVX512F__
    decode_pv_gemv_int8_avx512(out, s, V, vscales, n_start, block_n, out_d);
#else
#if NCNN_RUNTIME_CPU && NCNN_AVX2 && __AVX__ && !__AVX2__
    if (ncnn::cpu_support_x86_avx2())
    {
    ...
    decode_pv_gemv_int8_avx2(out, s, V, vscales, n_start, block_n, out_d);
#elif __SSE2__
    decode_pv_gemv_int8_sse2(out, s, V, vscales, n_start, block_n, out_d);
#else
    decode_pv_gemv_int8_scalar(out, s, V, vscales, n_start, block_n, out_d);
#endif
#endif
}
...
static inline void pv_float_int8_gemm_row_scalar(float* out, const float* p_row,
        const signed char* V, const float* vscales,
        int n, int out_d)
{
    const int num_blocks = (out_d + 31) / 32;
...
#if __SSE2__
static inline void pv_float_int8_gemm_row_sse2(float* out, const float* p_row,
        const signed char* V, const float* vscales,
        int n, int out_d)
{
    const int num_blocks = (out_d + 31) / 32;
...
#if __AVX2__
inline void __attribute__((noinline)) pv_float_int8_gemm_row_avx2(float* out, const float* p_row,
        const signed char* V, const float* vscales,
        int n, int out_d)
{
    const int num_blocks = (out_d + 31) / 32;
...
#if __AVX512F__
static inline void pv_float_int8_gemm_row_avx512(float* out, const float* p_row,
        const signed char* V, const float* vscales,
        int n, int out_d)
{
    const int num_blocks = (out_d + 31) / 32;
...
static void pv_float_int8_gemm_row_dispatch(float* out, const float* p_row,
        const signed char* V, const float* vscales,
        int n, int out_d)
{
#if __AVX512F__
    pv_float_int8_gemm_row_avx512(out, p_row, V, vscales, n, out_d);
#else
#if NCNN_RUNTIME_CPU && NCNN_AVX2 && __AVX__ && !__AVX2__
    if (ncnn::cpu_support_x86_avx2())
    {
    ...
    pv_float_int8_gemm_row_avx2(out, p_row, V, vscales, n, out_d);
#elif __SSE2__
    pv_float_int8_gemm_row_sse2(out, p_row, V, vscales, n, out_d);
#else
    pv_float_int8_gemm_row_scalar(out, p_row, V, vscales, n, out_d);
#endif
#endif
}
...
static inline void pv_float_int8_fma_block_scalar(float* out, float p_invscale, const signed char* v, int len)
{
    for (int k = 0; k < len; k++)
...
#if __SSE2__
static inline void pv_float_int8_fma_block_sse2(float* out, float p_invscale, const signed char* v, int len)
{
    __m128 pvec = _mm_set1_ps(p_invscale);
    int k = 0;
    for (; k + 3 < len; k += 4)
...
#if __AVX2__
inline void __attribute__((noinline)) pv_float_int8_fma_block_avx2(float* out, float p_invscale, const signed char* v, int len)
{
    __m256 pvec = _mm256_set1_ps(p_invscale);
    int k = 0;
    for (; k + 7 < len; k += 8)
...
#if __AVX512F__
static inline void pv_float_int8_fma_block_avx512(float* out, float p_invscale, const signed char* v, int len)
{
    __m512 pvec = _mm512_set1_ps(p_invscale);
    int k = 0;
    for (; k + 15 < len; k += 16)
...
static void pv_float_int8_fma_block_dispatch(float* out, float p_invscale, const signed char* v, int len)
{
#if __AVX512F__
    pv_float_int8_fma_block_avx512(out, p_invscale, v, len);
#else
#if NCNN_RUNTIME_CPU && NCNN_AVX2 && __AVX__ && !__AVX2__
    if (ncnn::cpu_support_x86_avx2())
    {
    ...
    pv_float_int8_fma_block_avx2(out, p_invscale, v, len);
#elif __SSE2__
    pv_float_int8_fma_block_sse2(out, p_invscale, v, len);
#else
    pv_float_int8_fma_block_scalar(out, p_invscale, v, len);
#endif
#endif
}

#if __AVX2__
inline void __attribute__((noinline)) pv_float_int8_gemm_tile_avx2(float* O, const float* P,
                                    const signed char* V, const float* vscales,
