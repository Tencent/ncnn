...
void convolution1d_packed_bf16s_fma(const Mat& bottom_blob, Mat& top_blob, const Mat& weight_data_tm, const Mat& bias_data, int kernel_w, int dilation_w, int stride_w, int activation_type, const Mat& activation_params, const Option& opt);
#endif
...
static void convolution1d_packed_bf16s(const Mat& bottom_blob, Mat& top_blob, const Mat& weight_data_tm, const Mat& bias_data, int kernel_w, int dilation_w, int stride_w, int activation_type, const Mat& activation_params, const Option& opt)
{
    ...
    {
        convolution1d_packed_bf16s_fma(bottom_blob, top_blob, weight_data_tm, bias_data, kernel_w, dilation_w, stride_w, activation_type, activation_params, opt);
        return;
    }
#endif
#if NCNN_RUNTIME_CPU && NCNN_FMA4 && __AVX__ && !__FMA__ && !__FMA4__
    ...
    {
        convolution1d_packed_bf16s_fma4(bottom_blob, top_blob, weight_data_tm, bias_data, kernel_w, dilation_w, stride_w, activation_type, activation_params, opt);
        return;
    }
#endif
    ...
    for (int pp = 0; pp < nn_outh; pp++)
    {
        ...
        for (int j = 0; j < outw; j++)
        {
            ...
            if (out_elempack == 4)
            {
                __m128i _bf01 = float2bfloat_avx(_sum0);
                _mm_storel_epi64((__m128i*)outptr, _bf01);
                _mm_storel_epi64((__m128i*)(outptr + M), _mm_srli_si128(_bf01, 8));
                outptr += 4;
            }
            if (out_elempack == 1)
            {
