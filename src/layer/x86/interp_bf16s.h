...
void interp_forward_bf16s_sse_avx512bf16(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt, int resize_type, int align_corner, float height_scale, float width_scale, int output_height, int output_width, int has_dynamic_output_size);
#endif

static void interp_forward_bf16s_sse(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt, int resize_type, int align_corner, float height_scale, float width_scale, int output_height, int output_width, int has_dynamic_output_size);
...
static void interp_forward_bf16s_sse(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt, int resize_type, int align_corner, float height_scale, float width_scale, int output_height, int output_width, int has_dynamic_output_size)
{
    ...
    {
        interp_forward_bf16s_sse_avx512bf16(bottom_blobs, top_blobs, opt, resize_type, align_corner, height_scale, width_scale, output_height, output_width, has_dynamic_output_size);
        return;
    }
#endif
    ...
    if (dims == 2)
    {
        ...
        {
            const float ws = (output_width || has_dynamic_output_size) ? w / (float)outw : 1.f / width_scale;
        ...
        if (resize_type == 2) // bilinear
        {
            ...
            linear_coeffs_impl(w, outw, xofs, alpha, align_corner, (output_width || has_dynamic_output_size) ? w / (float)outw : 1.f / width_scale);
        ...
        if (resize_type == 3) // bicubic
        {
            ...
            cubic_coeffs_impl(w, outw, xofs, alpha, align_corner, (output_width || has_dynamic_output_size) ? w / (float)outw : 1.f / width_scale);
    ...
    if (resize_type == 1) // nearest
    {
        const float hs = (output_height || has_dynamic_output_size) ? h / (float)outh : 1.f / height_scale;
        const float ws = (output_width || has_dynamic_output_size) ? w / (float)outw : 1.f / width_scale;
    ...
    if (resize_type == 2) // bilinear
    {
        ...
        linear_coeffs_impl(w, outw, xofs, alpha, align_corner, (output_width || has_dynamic_output_size) ? w / (float)outw : 1.f / width_scale);
        linear_coeffs_impl(h, outh, yofs, beta, align_corner, (output_height || has_dynamic_output_size) ? h / (float)outh : 1.f / height_scale);
    ...
    if (resize_type == 3) // bicubic
    {
        ...
        cubic_coeffs_impl(w, outw, xofs, alpha, align_corner, (output_width || has_dynamic_output_size) ? w / (float)outw : 1.f / width_scale);
        cubic_coeffs_impl(h, outh, yofs, beta, align_corner, (output_height || has_dynamic_output_size) ? h / (float)outh : 1.f / height_scale);
