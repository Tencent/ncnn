...
static inline float yolov3_sigmoid(float x)
{
...
static int yolov3detectionoutput_fp32(const Yolov3DetectionOutput& self, const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt)
{
    ...
    const int num_class = self.num_class;
    const int num_box = self.num_box;
    const float confidence_threshold = self.confidence_threshold;
    const float nms_threshold = self.nms_threshold;
    const Mat& biases = self.biases;
    const Mat& mask = self.mask;
    const Mat& anchors_scale = self.anchors_scale;
...
//printf("%d %d %d\n", w, h, channels);
...
#endif
                    //yolov3_sigmoid(box_score) * yolov3_sigmoid(class_score)
                    float confidence = 1.f / ((1.f + expf(-box_score_ptr[0]) * (1.f + expf(-class_score))));
                    if (confidence >= confidence_threshold)
                    {
                        // region box
                        float bbox_cx = (j + yolov3_sigmoid(xptr[0])) / w;
                        float bbox_cy = (i + yolov3_sigmoid(yptr[0])) / h;
    ...
    // global sort inplace
    self.qsort_descent_inplace(all_bbox_rects);

    // apply nms
    std::vector<size_t> picked;
    self.nms_sorted_bboxes(all_bbox_rects, picked, nms_threshold);
