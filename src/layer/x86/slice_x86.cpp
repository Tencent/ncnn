...
int Slice_x86::forward_bf16s_fp16s(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const
{
    const Mat& bottom_blob = bottom_blobs[0];
    int dims = bottom_blob.dims;
    size_t elemsize = bottom_blob.elemsize;
        ...
        for (size_t i = 0; i < top_blobs.size(); i++)
        {
            ...
            if (out_elempack == 8 && top_blob.elempack == 16)
            {
                for (int j = 0; j < top_blob.h; j++)
                {
                    ...
                    const unsigned short* r1 = ptr + w * 8;
            ...
            if (out_elempack == 4 && top_blob.elempack == 16)
            {
                for (int j = 0; j < top_blob.h; j++)
                {
                    ...
                    const unsigned short* r2 = ptr + w * 8;
                    const unsigned short* r3 = ptr + w * 12;
            ...
            if (out_elempack == 1 && top_blob.elempack == 16)
            {
                for (int j = 0; j < top_blob.h; j++)
                {
                    ...
                    const unsigned short* r8 = ptr + w * 8;
                    const unsigned short* r9 = ptr + w * 9;
                    const unsigned short* ra = ptr + w * 10;
                    const unsigned short* rb = ptr + w * 11;
                    const unsigned short* rc = ptr + w * 12;
                    const unsigned short* rd = ptr + w * 13;
                    const unsigned short* re = ptr + w * 14;
                    const unsigned short* rf = ptr + w * 15;
    ...
    if ((dims == 3 || dims == 4) && positive_axis == 0)
    {
        ...
        for (size_t i = 0; i < top_blobs.size(); i++)
        {
            ...
            if (out_elempack == 8 && top_blob.elempack == 16)
            {
                ...
                for (int q = 0; q < top_blob.c; q++)
                {
                    const unsigned short* r0 = (const unsigned short*)bottom_blob_unpacked.channel(p);
                    const unsigned short* r1 = (const unsigned short*)bottom_blob_unpacked.channel(p + 1);
            ...
            if (out_elempack == 4 && top_blob.elempack == 16)
            {
                ...
                for (int q = 0; q < top_blob.c; q++)
                {
                    const unsigned short* r0 = (const unsigned short*)bottom_blob_unpacked.channel(p);
                    const unsigned short* r1 = (const unsigned short*)bottom_blob_unpacked.channel(p + 1);
                    const unsigned short* r2 = (const unsigned short*)bottom_blob_unpacked.channel(p + 2);
                    const unsigned short* r3 = (const unsigned short*)bottom_blob_unpacked.channel(p + 3);
            ...
            if (out_elempack == 1 && top_blob.elempack == 16)
            {
                ...
                for (int q = 0; q < top_blob.c; q++)
                {
                    ...
                    const unsigned short* r2 = (const unsigned short*)bottom_blob_unpacked.channel(p + 2);
                    const unsigned short* r3 = (const unsigned short*)bottom_blob_unpacked.channel(p + 3);
                    const unsigned short* r4 = (const unsigned short*)bottom_blob_unpacked.channel(p + 4);
                    const unsigned short* r5 = (const unsigned short*)bottom_blob_unpacked.channel(p + 5);
                    const unsigned short* r6 = (const unsigned short*)bottom_blob_unpacked.channel(p + 6);
                    const unsigned short* r7 = (const unsigned short*)bottom_blob_unpacked.channel(p + 7);
                    const unsigned short* r8 = (const unsigned short*)bottom_blob_unpacked.channel(p + 8);
                    const unsigned short* r9 = (const unsigned short*)bottom_blob_unpacked.channel(p + 9);
                    const unsigned short* ra = (const unsigned short*)bottom_blob_unpacked.channel(p + 10);
                    const unsigned short* rb = (const unsigned short*)bottom_blob_unpacked.channel(p + 11);
                    const unsigned short* rc = (const unsigned short*)bottom_blob_unpacked.channel(p + 12);
                    const unsigned short* rd = (const unsigned short*)bottom_blob_unpacked.channel(p + 13);
                    const unsigned short* re = (const unsigned short*)bottom_blob_unpacked.channel(p + 14);
                    const unsigned short* rf = (const unsigned short*)bottom_blob_unpacked.channel(p + 15);
            ...
            if (out_elempack == top_blob.elempack)
            {
                ...
                const unsigned short* ptr = (const unsigned short*)bottom_blob_unpacked.channel(p);
                unsigned short* outptr = top_blob;
                memcpy(outptr, ptr, size * top_blob.elemsize);
    ...
    if ((dims == 3 && positive_axis == 1) || (dims == 4 && positive_axis == 2))
    {
        ...
        for (int p = 0; p < channels; p++)
        {
            const unsigned short* ptr = (const unsigned short*)bottom_blob.channel(p);
