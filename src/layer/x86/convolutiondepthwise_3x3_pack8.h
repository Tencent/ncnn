...
static void convdw3x3s1_pack8_avx(const Mat& bottom_blob, Mat& top_blob, const Mat& kernel, const Mat& _bias, const Option& opt)
{
    ...
    {
        convdw3x3s1_pack8_avx_fma(bottom_blob, top_blob, kernel, _bias, opt);
        return;
    }
#endif
#if NCNN_RUNTIME_CPU && NCNN_FMA4 && __AVX__ && !__FMA__ && !__FMA4__
    ...
    {
        convdw3x3s1_pack8_avx_fma4(bottom_blob, top_blob, kernel, _bias, opt);
        return;
    }
#endif
...
static void convdw3x3s2_pack8_avx(const Mat& bottom_blob, Mat& top_blob, const Mat& kernel, const Mat& _bias, const Option& opt)
{
    ...
    if (ncnn::cpu_support_x86_fma())
    {
        convdw3x3s2_pack8_avx_fma(bottom_blob, top_blob, kernel, _bias, opt);
        return;
    }
#endif
#if NCNN_RUNTIME_CPU && NCNN_FMA4 && __AVX__ && !__FMA__ && !__FMA4__
    if (ncnn::cpu_support_x86_fma4())
    {
        convdw3x3s2_pack8_avx_fma4(bottom_blob, top_blob, kernel, _bias, opt);
        return;
    }
#endif
