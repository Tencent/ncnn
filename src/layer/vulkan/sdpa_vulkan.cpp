...
int SDPA_vulkan::create_pipeline(const Option& opt)
{
    ...
    if (use_cooperative_matrix)
    {
        ...
        else
        {
            ...
            const size_t shared_a = 16 * coopmat_M * (coopmat_K / 8);
            const size_t shared_b = 16 * std::max(coopmat_K * (coopmat_N / 8), coopmat_N * (coopmat_K / 8));
            const size_t shared_o = 16 * coopmat_M * (coopmat_N / 8);
        ...
        if (use_cooperative_matrix)
        {
            ...
            else
            {
                ...
                for (;;)
                {
                    ...
                    const size_t shared_bytes = 16 * (rows + UNROLL_P_N * FA_coopmat_N) * (FA_coopmat_K / 8)
                                                + 4 * rows * ((UNROLL_P_N + 1) * FA_coopmat_N + 3);
