...
int Interp_vulkan::forward(const std::vector<VkMat>& bottom_blobs, std::vector<VkMat>& top_blobs, VkCompute& cmd, const Option& opt) const
{
    ...
    if (dims == 1)
    {
        ...
        constants[10].f = (output_width || dynamic_target_size) ? w / (float)outw : 1.f / width_scale;
        constants[11].f = (output_height || dynamic_target_size) ? h / (float)outh : 1.f / height_scale;
    ...
    if (dims == 2)
    {
        ...
        if (resize_type == 1 || resize_type == 2) // nearest or bilinear
        {
            ...
            constants[8].i = top_blob.c;
            constants[9].i = top_blob.cstep;
            constants[10].f = (output_width || dynamic_target_size || !size_expr.empty()) ? w / (float)outw : 1.f / width_scale;
            constants[11].f = 1.f;
        ...
        if (resize_type == 3) // bicubic
        {
...

            {
                ...
                constants[1].i = outw;
                constants[2].f = (output_width || dynamic_target_size || !size_expr.empty()) ? (float)bottom_blob.w / outw : 1.f / width_scale;
    ...
    if (resize_type == 1 || resize_type == 2) // nearest or bilinear
    {
        ...
        constants[9].i = top_blob.cstep;
        constants[10].f = (output_width || dynamic_target_size || !size_expr.empty()) ? w / (float)outw : 1.f / width_scale;
        constants[11].f = (output_height || dynamic_target_size || !size_expr.empty()) ? h / (float)outh : 1.f / height_scale;
    ...
    else if (resize_type == 3) // bicubic
    {
...

        {
            ...
            constants[1].i = outh;
            constants[2].f = (output_height || dynamic_target_size || !size_expr.empty()) ? (float)bottom_blob.h / outh : 1.f / height_scale;
