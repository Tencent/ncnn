...
int Interp_vulkan::forward(const std::vector<VkMat>& bottom_blobs, std::vector<VkMat>& top_blobs, VkCompute& cmd, const Option& opt) const
{
    ...
    if (dims == 1)
    {
        ...
        constants[10].f = get_resize_scale_x(w, outw);
        constants[11].f = get_resize_scale_y(h, outh);
    ...
    if (dims == 2)
    {
        ...
        if (resize_type == 3) // bicubic
        {
...

            {
                ...
                constants[1].i = outw;
                constants[2].f = get_resize_scale_x(w, outw);
    ...
    else if (resize_type == 3) // bicubic
    {
...

        {
            ...
            constants[1].i = outh;
            constants[2].f = get_resize_scale_y(h, outh);
