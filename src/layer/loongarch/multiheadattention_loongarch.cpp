...
#include "multiheadattention_loongarch.h"
...
MultiHeadAttention_loongarch::MultiHeadAttention_loongarch()
{
#if __loongarch_sx
...
int MultiHeadAttention_loongarch::create_pipeline_wq_int8(const Option& _opt)
{
    if (q_gemm)
        return 0;
...
int MultiHeadAttention_loongarch::create_pipeline(const Option& _opt)
{
#if NCNN_WEIGHT_QUANT
...
int MultiHeadAttention_loongarch::destroy_pipeline(const Option& _opt)
{
    if (weight_block_quantize)
    {
        int weight_bits;
...
int MultiHeadAttention_loongarch::forward(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& _opt) const
{
    ...
    if (kv_cache)
    {
        ...
        {
            NCNN_LOGE("MultiHeadAttention_loongarch got foreign kvcache");
            return -1;
        }
    }
    ...
    if (kv_cache)
    {
        ...
        int retv = past_xv_blob.empty() ? storage->create(cached_xv_blob, dst_seqlen, num_heads, embed_dim_per_head, current_value.elemsize, 1) : storage->expand(past_xv_blob, cached_xv_blob, dst_seqlen);
        if (retv != 0)
        {
