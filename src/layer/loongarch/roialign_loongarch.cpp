...
#include "roialign_loongarch.h"

namespace ncnn {

ROIAlign_loongarch::ROIAlign_loongarch()
{
...
int ROIAlign_loongarch::forward(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const
{
    const Mat& bottom_blob = bottom_blobs[0];
    const Mat& roi_blob = bottom_blobs[1];

    if (bottom_blob.elempack == 1 && roi_blob.elempack == 1 && bottom_blob.elembits() == 32 && roi_blob.elembits() == 32)
        return ROIAlign::forward(bottom_blobs, top_blobs, opt);
    ...
    Mat roi_blob_fp32;
    if (unpack_or_cast_to_float32(roi_blob, roi_blob_fp32, opt) != 0)
        return -100;

    std::vector<Mat> bottom_blobs_fp32(2);
    bottom_blobs_fp32[0] = bottom_blob_fp32;
    bottom_blobs_fp32[1] = roi_blob_fp32;

    std::vector<Mat> top_blobs_fp32(1);
    int ret = ROIAlign::forward(bottom_blobs_fp32, top_blobs_fp32, opt_fp32);
    if (ret != 0)
        return ret;
