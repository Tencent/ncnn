...
static void groupnorm_loongarch_bf16(unsigned short* ptr, const float* gamma_ptr, const float* beta_ptr, float eps, int channels, int size, int elempack, size_t cstep)
{
...
#endif // __loongarch_asx
        __m128i _zero_bf16_mean = __lsx_vreplgr2vr_w(0);
        for (; i + 7 < size; i += 8)
        {
            __m128i _p01 = __lsx_vld(ptr0, 0);
            __m128 _p0 = (__m128)__lsx_vilvl_h(_p01, _zero_bf16_mean);
            __m128 _p1 = (__m128)__lsx_vilvh_h(_p01, _zero_bf16_mean);
            _mean4 = __lsx_vfadd_s(_mean4, _p0);
            _mean4 = __lsx_vfadd_s(_mean4, _p1);
            ptr0 += 8;
        }
        for (; i + 3 < size; i += 4)
        {
...
#endif // __loongarch_asx
        __m128i _zero_bf16_var = __lsx_vreplgr2vr_w(0);
        for (; i + 7 < size; i += 8)
        {
            __m128i _p01 = __lsx_vld(ptr0, 0);
            __m128 _p0 = (__m128)__lsx_vilvl_h(_p01, _zero_bf16_var);
            __m128 _p1 = (__m128)__lsx_vilvh_h(_p01, _zero_bf16_var);
            _p0 = __lsx_vfsub_s(_p0, _mean4);
            _p1 = __lsx_vfsub_s(_p1, _mean4);
            _var4 = __lsx_vfmadd_s(_p0, _p0, _var4);
            _var4 = __lsx_vfmadd_s(_p1, _p1, _var4);
            ptr0 += 8;
        }
        for (; i + 3 < size; i += 4)
        {
    ...
    if (gamma_ptr && beta_ptr)
    {
        for (int q = 0; q < channels; q++)
        {
            ...
            __m128 _a4 = (__m128)__lsx_vreplfr2vr_s(0.f);
            __m128 _b4 = (__m128)__lsx_vreplfr2vr_s(0.f);
            __m128 _a4h = (__m128)__lsx_vreplfr2vr_s(0.f);
            __m128 _b4h = (__m128)__lsx_vreplfr2vr_s(0.f);
#endif // __loongarch_sx
            float a = 0.f;
            float b = 0.f;
            ...
            if (elempack == 8)
            {
                __m128 _gamma0 = (__m128)__lsx_vld(gamma_ptr + q * elempack, 0);
                __m128 _gamma1 = (__m128)__lsx_vld(gamma_ptr + q * elempack + 4, 0);
                __m128 _beta0 = (__m128)__lsx_vld(beta_ptr + q * elempack, 0);
                __m128 _beta1 = (__m128)__lsx_vld(beta_ptr + q * elempack + 4, 0);

                _a4 = __lsx_vfmul_s(_var4, _gamma0);
                _a4h = __lsx_vfmul_s(_var4, _gamma1);
                _b4 = __lsx_vfmadd_s(_mean4, _gamma0, _beta0);
                _b4h = __lsx_vfmadd_s(_mean4, _gamma1, _beta1);
            }
#endif // !__loongarch_asx
            if (elempack == 4)
            {
            ...
            for (; i + 7 < size; i += 8)
            {
                ...
                _p0 = __lsx_vfmadd_s(_p0, _a4, _b4);
                _p1 = __lsx_vfmadd_s(_p1, _a4h, _b4h);
                __lsx_vst(float2bfloat_lsx(_p0, _p1), ptr0, 0);
                ptr0 += 8;
            }
            for (; i + 3 < size; i += 4)
            {
    ...
    else
    {
        for (int q = 0; q < channels; q++)
        {
            ...
            for (; i + 7 < size; i += 8)
            {
                ...
                __m128 _p0 = (__m128)__lsx_vilvl_h(_p01, _zero_bf16);
                __m128 _p1 = (__m128)__lsx_vilvh_h(_p01, _zero_bf16);
                _p0 = __lsx_vfmadd_s(_p0, _var4, _mean4);
                _p1 = __lsx_vfmadd_s(_p1, _var4, _mean4);
                __lsx_vst(float2bfloat_lsx(_p0, _p1), ptr0, 0);
                ptr0 += 8;
            }
            for (; i + 3 < size; i += 4)
...
int GroupNorm_loongarch::forward_inplace_bf16s(Mat& bottom_top_blob, const Option& opt) const
{
    ...
    if (dims == 1)
    {
        ...
        for (int g = 0; g < group; g++)
        {
            ...
            const float* gamma_ptr = affine ? (const float*)gamma_data + g * channels_g : 0;
            const float* beta_ptr = affine ? (const float*)beta_data + g * channels_g : 0;
            groupnorm_loongarch_bf16(bottom_top_blob_g, gamma_ptr, beta_ptr, eps, channels_g / g_elempack, g_elempack, g_elempack, 1);
        }
    }
    ...
    if (dims == 2)
    {
        ...
        for (int g = 0; g < group; g++)
        {
            unsigned short* ptr_g = (unsigned short*)bottom_top_blob_packed.row(g * channels_g / g_elempack);
            const float* gamma_ptr = affine ? (const float*)gamma_data + g * channels_g : 0;
            const float* beta_ptr = affine ? (const float*)beta_data + g * channels_g : 0;
            groupnorm_loongarch_bf16(ptr_g, gamma_ptr, beta_ptr, eps, channels_g / g_elempack, w * g_elempack, g_elempack, w);
        }
    }
    ...
    if (dims == 3 || dims == 4)
    {
        ...
        for (int g = 0; g < group; g++)
        {
            unsigned short* ptr_g = (unsigned short*)bottom_top_blob_packed.channel(g * channels_g / g_elempack);
            const float* gamma_ptr = affine ? (const float*)gamma_data + g * channels_g : 0;
            const float* beta_ptr = affine ? (const float*)beta_data + g * channels_g : 0;
            groupnorm_loongarch_bf16(ptr_g, gamma_ptr, beta_ptr, eps, channels_g / g_elempack, size * g_elempack, g_elempack, cstep);
        }
    }
    ...
    if (g_elempack != elempack)
    {
        ...
        convert_packing(bottom_top_blob_packed, bottom_top_blob_unpacked, elempack, opt_pack);
