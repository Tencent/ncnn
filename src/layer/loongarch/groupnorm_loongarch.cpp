    ...
    if (gamma_ptr && beta_ptr)
    {
        for (int q = 0; q < channels; q++)
        {
            ...
            for (; i + 7 < size; i += 8)
            {
                ...
                _p1 = __lsx_vfmadd_s(_p1, _a4, _b4);
...
int GroupNorm_loongarch::forward_inplace_bf16s(Mat& bottom_top_blob, const Option& opt) const
{
    ...
    if (dims == 1)
    {
        ...
        for (int g = 0; g < group; g++)
        {
            unsigned short* ptr_g = (unsigned short*)bottom_top_blob_packed + g * channels_g / g_elempack * (g_elempack == 8 ? 8 : (g_elempack == 4 ? 4 : 1));
            const float* gamma_ptr = affine ? (const float*)gamma_data + g * channels_g : 0;
            const float* beta_ptr = affine ? (const float*)beta_data + g * channels_g : 0;
            groupnorm_loongarch_bf16(ptr_g, gamma_ptr, beta_ptr, eps, channels_g / g_elempack, g_elempack, g_elempack, 1);
        }
    }
