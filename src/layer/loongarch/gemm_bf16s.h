...
static void pack_A_tile_bf16(const Mat& A, Mat& AT, int i, int max_ii, int k, int max_kk)
{
    ...
    for (; ii + 7 < max_ii; ii += 8)
    {
        ...
        if (elempack == 4)
        {
            ...
            {
                pp[0] = p0[0]; pp[1] = p0[1]; pp[2] = p0[2]; pp[3] = p0[3];
                pp[4] = p1[0]; pp[5] = p1[1]; pp[6] = p1[2]; pp[7] = p1[3];
                pp += 8; p0 += 4; p1 += 4;
            }
        }
        if (elempack == 1)
...
static void unpack_output_tile_fp32_to_bf16(const Mat& topT, const Mat& C, Mat& top_blob, int broadcast_type_C, int i, int max_ii, int j, int max_jj, float alpha, float beta, int output_transpose, int output_elemtype)
{
    ...
    for (; ii + 3 < max_ii; ii += 4)
    {
        ...
        for (; jj + 7 < max_jj; jj += 8)
        {
            ...
            if (pCi)
            {
                ...
                if (broadcast_type_C == 0)
                {
                    ...
                    _sum0 = __lsx_vfadd_s(_sum0, _c); _sum1 = __lsx_vfadd_s(_sum1, _c); _sum2 = __lsx_vfadd_s(_sum2, _c); _sum3 = __lsx_vfadd_s(_sum3, _c);
                    _sum4 = __lsx_vfadd_s(_sum4, _c); _sum5 = __lsx_vfadd_s(_sum5, _c); _sum6 = __lsx_vfadd_s(_sum6, _c); _sum7 = __lsx_vfadd_s(_sum7, _c);
                }
                if (broadcast_type_C == 1 || broadcast_type_C == 2)
                {
            ...
            _sum0 = __lsx_vfmul_s(_sum0, _valpha); _sum1 = __lsx_vfmul_s(_sum1, _valpha); _sum2 = __lsx_vfmul_s(_sum2, _valpha); _sum3 = __lsx_vfmul_s(_sum3, _valpha);
            _sum4 = __lsx_vfmul_s(_sum4, _valpha); _sum5 = __lsx_vfmul_s(_sum5, _valpha); _sum6 = __lsx_vfmul_s(_sum6, _valpha); _sum7 = __lsx_vfmul_s(_sum7, _valpha);

            if (output_transpose)
            {
                float tmp0[4], tmp1[4], tmp2[4], tmp3[4], tmp4[4], tmp5[4], tmp6[4], tmp7[4];
                __lsx_vst((__m128i)_sum0, tmp0, 0); __lsx_vst((__m128i)_sum1, tmp1, 0); __lsx_vst((__m128i)_sum2, tmp2, 0); __lsx_vst((__m128i)_sum3, tmp3, 0);
                __lsx_vst((__m128i)_sum4, tmp4, 0); __lsx_vst((__m128i)_sum5, tmp5, 0); __lsx_vst((__m128i)_sum6, tmp6, 0); __lsx_vst((__m128i)_sum7, tmp7, 0);
            ...
            else
            {
                if (out_elempack == 4)
                {
                    if (output_elemtype == 1) { __lsx_vst((__m128i)_sum0, p0f32, 0); __lsx_vst((__m128i)_sum1, p0f32+4, 0); __lsx_vst((__m128i)_sum2, p0f32+8, 0); __lsx_vst((__m128i)_sum3, p0f32+12, 0); __lsx_vst((__m128i)_sum4, p0f32+16, 0); __lsx_vst((__m128i)_sum5, p0f32+20, 0); __lsx_vst((__m128i)_sum6, p0f32+24, 0); __lsx_vst((__m128i)_sum7, p0f32+28, 0); p0f32 += 32; }
                    else { float2bfloat_lsx_store(_sum0, p0); float2bfloat_lsx_store(_sum1, p0+4); float2bfloat_lsx_store(_sum2, p0+8); float2bfloat_lsx_store(_sum3, p0+12); float2bfloat_lsx_store(_sum4, p0+16); float2bfloat_lsx_store(_sum5, p0+20); float2bfloat_lsx_store(_sum6, p0+24); float2bfloat_lsx_store(_sum7, p0+28); p0 += 32; }
                }
                if (out_elempack == 1)
                {
                    transpose4x4_ps(_sum0, _sum1, _sum2, _sum3);
                    transpose4x4_ps(_sum4, _sum5, _sum6, _sum7);
                    if (output_elemtype == 1) { __lsx_vst((__m128i)_sum0, p0f32, 0); __lsx_vst((__m128i)_sum4, p0f32+4, 0); __lsx_vst((__m128i)_sum1, p0f32+out_hstep, 0); __lsx_vst((__m128i)_sum5, p0f32+out_hstep+4, 0); __lsx_vst((__m128i)_sum2, p0f32+out_hstep*2, 0); __lsx_vst((__m128i)_sum6, p0f32+out_hstep*2+4, 0); __lsx_vst((__m128i)_sum3, p0f32+out_hstep*3, 0); __lsx_vst((__m128i)_sum7, p0f32+out_hstep*3+4, 0); p0f32 += 8; }
                    else { float2bfloat_lsx_store(_sum0, p0); float2bfloat_lsx_store(_sum4, p0+4); float2bfloat_lsx_store(_sum1, p0+out_hstep); float2bfloat_lsx_store(_sum5, p0+out_hstep+4); float2bfloat_lsx_store(_sum2, p0+out_hstep*2); float2bfloat_lsx_store(_sum6, p0+out_hstep*2+4); float2bfloat_lsx_store(_sum3, p0+out_hstep*3); float2bfloat_lsx_store(_sum7, p0+out_hstep*3+4); p0 += 8; }
                }
            }
        }
        for (; jj + 3 < max_jj; jj += 4)
        {
            ...
            {
                __m128 _beta = __lsx_vreplfr2vr_s(beta);
                if (broadcast_type_C == 0) { __m128 _c = __lsx_vfmul_s(__lsx_vreplfr2vr_s(pCi[0]), _beta); _sum0 = __lsx_vfadd_s(_sum0, _c); _sum1 = __lsx_vfadd_s(_sum1, _c); _sum2 = __lsx_vfadd_s(_sum2, _c); _sum3 = __lsx_vfadd_s(_sum3, _c); }
                if (broadcast_type_C == 1 || broadcast_type_C == 2) { __m128 _c = __lsx_vfmul_s((__m128)__lsx_vld(pCi, 0), _beta); _sum0 = __lsx_vfadd_s(_sum0, _c); _sum1 = __lsx_vfadd_s(_sum1, _c); _sum2 = __lsx_vfadd_s(_sum2, _c); _sum3 = __lsx_vfadd_s(_sum3, _c); }
                if (broadcast_type_C == 3)
                {
                    if (c_elempack == 4)
                    {
            ...
            else
            {
                if (out_elempack == 4)
                {
                    if (output_elemtype == 1) { __lsx_vst((__m128i)_sum0, p0f32, 0); __lsx_vst((__m128i)_sum1, p0f32+4, 0); __lsx_vst((__m128i)_sum2, p0f32+8, 0); __lsx_vst((__m128i)_sum3, p0f32+12, 0); p0f32 += 16; }
                    else { float2bfloat_lsx_store(_sum0, p0); float2bfloat_lsx_store(_sum1, p0+4); float2bfloat_lsx_store(_sum2, p0+8); float2bfloat_lsx_store(_sum3, p0+12); p0 += 16; }
                }
                if (out_elempack == 1)
                {
                    transpose4x4_ps(_sum0, _sum1, _sum2, _sum3);
                    if (output_elemtype == 1) { __lsx_vst((__m128i)_sum0, p0f32, 0); __lsx_vst((__m128i)_sum1, p0f32+out_hstep, 0); __lsx_vst((__m128i)_sum2, p0f32+out_hstep*2, 0); __lsx_vst((__m128i)_sum3, p0f32+out_hstep*3, 0); p0f32 += 4; }
                    else { float2bfloat_lsx_store(_sum0, p0); float2bfloat_lsx_store(_sum1, p0+out_hstep); float2bfloat_lsx_store(_sum2, p0+out_hstep*2); float2bfloat_lsx_store(_sum3, p0+out_hstep*3); p0 += 4; }
                }
            }
        }
        for (; jj + 1 < max_jj; jj += 2)
        {
            ...
            {
                __m128 _beta = __lsx_vreplfr2vr_s(beta);
                if (broadcast_type_C == 0) { __m128 _c = __lsx_vfmul_s(__lsx_vreplfr2vr_s(pCi[0]), _beta); _sum0 = __lsx_vfadd_s(_sum0, _c); _sum1 = __lsx_vfadd_s(_sum1, _c); }
                if (broadcast_type_C == 1 || broadcast_type_C == 2) { __m128 _c = __lsx_vfmul_s((__m128)__lsx_vld(pCi, 0), _beta); _sum0 = __lsx_vfadd_s(_sum0, _c); _sum1 = __lsx_vfadd_s(_sum1, _c); }
                if (broadcast_type_C == 3)
                {
                    if (c_elempack == 4)
                    {
                ...
                if (broadcast_type_C == 4) { _sum0 = __lsx_vfmadd_s(_beta, __lsx_vreplfr2vr_s(pCi[0]), _sum0); _sum1 = __lsx_vfmadd_s(_beta, __lsx_vreplfr2vr_s(pCi[1]), _sum1); pCi += 2; }
            }
            ...
            {
                float tmp0[4], tmp1[4];
                __lsx_vst((__m128i)_sum0, tmp0, 0); __lsx_vst((__m128i)_sum1, tmp1, 0);
                for (int c = 0; c < 2; c++)
                {
            ...
            else
            {
                if (out_elempack == 4)
                {
                    if (output_elemtype == 1) { __lsx_vst((__m128i)_sum0, p0f32, 0); __lsx_vst((__m128i)_sum1, p0f32+4, 0); p0f32 += 8; }
                    else { float2bfloat_lsx_store(_sum0, p0); float2bfloat_lsx_store(_sum1, p0+4); p0 += 8; }
                }
                if (out_elempack == 1)
                {
                    float tmp0[4], tmp1[4];
        ...
        for (; jj < max_jj; jj += 1)
        {
            ...
            {
                __m128 _beta = __lsx_vreplfr2vr_s(beta);
                if (broadcast_type_C == 0) { _sum0 = __lsx_vfadd_s(_sum0, __lsx_vfmul_s(__lsx_vreplfr2vr_s(pCi[0]), _beta)); }
                if (broadcast_type_C == 1 || broadcast_type_C == 2) { _sum0 = __lsx_vfadd_s(_sum0, __lsx_vfmul_s((__m128)__lsx_vld(pCi, 0), _beta)); }
                if (broadcast_type_C == 3)
                {
                    if (c_elempack == 4)
                    {
                ...
                if (broadcast_type_C == 4) { _sum0 = __lsx_vfmadd_s(_beta, __lsx_vreplfr2vr_s(pCi[0]), _sum0); pCi += 1; }
            }
            ...
            else
            {
                if (out_elempack == 4)
                {
                    if (output_elemtype == 1) { __lsx_vst((__m128i)_sum0, p0f32, 0); p0f32 += 4; }
                    else { float2bfloat_lsx_store(_sum0, p0); p0 += 4; }
                }
                if (out_elempack == 1)
                {
                    float tmp[4];
