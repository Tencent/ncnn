...
#ifndef HARDSWISH_LOONGARCH_BF16S_H
#define HARDSWISH_LOONGARCH_BF16S_H

static void hardswish_bf16s(Mat& a, float alpha, float beta, const Option& opt)
{
    int w = a.w;
    int h = a.h;
    int d = a.d;
    ...
    for (int q = 0; q < channels; q++)
    {
        ...
        for (; i + 7 < size; i += 8)
        {
            ...
            __m256 _hs = __lasx_xvfmadd_s(_alpha_lasx, _p, _beta_lasx);
            _hs = __lasx_xvfmax_s(_hs, _zero_lasx);
            _hs = __lasx_xvfmin_s(_hs, _one_lasx);
            _p = __lasx_xvfmul_s(_p, _hs);
        ...
        for (; i + 3 < size; i += 4)
        {
            ...
            __m128 _hs = __lsx_vfmadd_s(_alpha, _p, _beta);
            _hs = __lsx_vfmax_s(_hs, _zero);
            _hs = __lsx_vfmin_s(_hs, _one);
            _p = __lsx_vfmul_s(_p, _hs);
        ...
        for (; i < size; i++)
        {
            ...
            v = v * std::min(1.f, std::max(0.f, v * alpha + beta));
...
#endif // HARDSWISH_LOONGARCH_BF16S_H
