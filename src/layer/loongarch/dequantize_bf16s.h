...
#ifndef DEQUANTIZE_LOONGARCH_BF16S_H
#define DEQUANTIZE_LOONGARCH_BF16S_H
...
static void dequantize_bf16(const int* intptr, unsigned short* ptr, const Mat& scale_data, const Mat& bias_data, int elemcount, int elempack)
{
    ...
    else
    {
...
#if __loongarch_asx
        __m256 _bias_avx = (__m256)__lasx_xvreplfr2vr_s(bias);
#else  // __loongarch_asx
        ...
        if (bias_data_size > 1)
        {
            ...
            {
#if __loongarch_asx
                _bias_avx = (__m256)__lasx_xvld((const float*)bias_data, 0);
#else  // __loongarch_asx
...
#endif // DEQUANTIZE_LOONGARCH_BF16S_H
