...
#ifndef SWISH_LOONGARCH_BF16S_H
#define SWISH_LOONGARCH_BF16S_H
...
static void swish_bf16s(Mat& a, const Option& opt)
{
    ...
    for (int q = 0; q < channels; q++)
    {
        ...
        for (; i + 7 < size; i += 8)
        {
            ...
            _p = __lasx_xvfdiv_s(_p, __lasx_xvfadd_s(_one_lasx, exp256_ps(__lasx_xvfsub_s(_zero_lasx, _p))));
        ...
        for (; i + 3 < size; i += 4)
        {
            ...
            _p = __lsx_vfdiv_s(_p, __lsx_vfadd_s(_one, exp_ps(__lsx_vfsub_s(_zero, _p))));
