...
static int eltwise_fp16s(const std::vector<Mat>& bottom_blobs, Mat& top_blob, int op_type, const Mat& coeffs, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return eltwise_fp16s_asimdhp(bottom_blobs, top_blob, op_type, coeffs, opt);
#endif
    ...
    if (bottom_blobs.size() == 2)
    {
        ...
        if (op_type == Eltwise::Operation_SUM)
        {
            if (coeffs.w == 0)
            {
                const Mat& bottom_blob1 = bottom_blobs[1];
        ...
        if (op_type == Eltwise::Operation_MAX)
        {
            // first blob
            const Mat& bottom_blob1 = bottom_blobs[1];
            #pragma omp parallel for num_threads(opt.num_threads)
...
static int eltwise_fp16sa(const std::vector<Mat>& bottom_blobs, Mat& top_blob, int op_type, const Mat& coeffs, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return eltwise_fp16sa_asimdhp(bottom_blobs, top_blob, op_type, coeffs, opt);
#endif
#if __ARM_FEATURE_FP16_VECTOR_ARITHMETIC
