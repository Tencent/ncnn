...
static int layernorm_inplace_fp16s(Mat& bottom_top_blob, int affine_size, float eps, const Mat& gamma_data, const Mat& beta_data, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return layernorm_inplace_fp16s_asimdhp(bottom_top_blob, affine_size, eps, gamma_data, beta_data, opt);
#endif
