// Copyright 2026 Futz12 <pchar.cn>
// SPDX-License-Identifier: BSD-3-Clause

#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
int elu_fp16s_asimdhp(Mat& bottom_top_blob, float alpha, const Option& opt);
int elu_fp16sa_asimdhp(Mat& bottom_top_blob, float alpha, const Option& opt);
#endif

static int elu_fp16s(Mat& bottom_top_blob, float alpha, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return elu_fp16s_asimdhp(bottom_top_blob, alpha, opt);
#endif

#if __ARM_FEATURE_FP16_VECTOR_ARITHMETIC

    int w = bottom_top_blob.w;
    int h = bottom_top_blob.h;
    int d = bottom_top_blob.d;
    int channels = bottom_top_blob.c;
    int elempack = bottom_top_blob.elempack;
    int size = w * h * d * elempack;

    #pragma omp parallel for num_threads(opt.num_threads)
    for (int q = 0; q < channels; q++)
    {
        __fp16* ptr = bottom_top_blob.channel(q);

        int i = 0;

        float32x4_t _alpha_f32 = vdupq_n_f32(alpha);

        for (; i + 7 < size; i += 8)
        {
            float16x8_t _p = vld1q_f16(ptr);

            float32x4_t _p_low = elu_ps(vcvt_f32_f16(vget_low_f16(_p)), _alpha_f32);
            float32x4_t _p_high = elu_ps(vcvt_f32_f16(vget_high_f16(_p)), _alpha_f32);

            vst1q_f16(ptr, vcombine_f16(vcvt_f16_f32(_p_low), vcvt_f16_f32(_p_high)));
            ptr += 8;
        }
        for (; i + 3 < size; i += 4)
        {
            float16x4_t _p = vld1_f16(ptr);

            float32x4_t _p_f32 = elu_ps(vcvt_f32_f16(_p), _alpha_f32);

            vst1_f16(ptr, vcvt_f16_f32(_p_f32));
            ptr += 4;
        }
        for (; i < size; i++)
        {
            __fp16 v = ptr[0];
            if (v < (__fp16)0.f)
                ptr[0] = (__fp16)(alpha * (expf((float)v) - 1.f));

            ptr += 1;
        }
    }

    return 0;

#else
    (void)bottom_top_blob;
    (void)alpha;
    (void)opt;
    return 0;
#endif
}

static int elu_fp16sa(Mat& bottom_top_blob, float alpha, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return elu_fp16sa_asimdhp(bottom_top_blob, alpha, opt);
#endif

#if __ARM_FEATURE_FP16_VECTOR_ARITHMETIC

    int w = bottom_top_blob.w;
    int h = bottom_top_blob.h;
    int d = bottom_top_blob.d;
    int channels = bottom_top_blob.c;
    int elempack = bottom_top_blob.elempack;
    int size = w * h * d * elempack;

    #pragma omp parallel for num_threads(opt.num_threads)
    for (int q = 0; q < channels; q++)
    {
        __fp16* ptr = bottom_top_blob.channel(q);

        int i = 0;

        float16x8_t _alpha = vdupq_n_f16((__fp16)alpha);

        for (; i + 15 < size; i += 16)
        {
            float16x8_t _p0 = vld1q_f16(ptr);
            float16x8_t _p1 = vld1q_f16(ptr + 8);

            _p0 = elu_ps_f16(_p0, _alpha);
            _p1 = elu_ps_f16(_p1, _alpha);

            vst1q_f16(ptr, _p0);
            vst1q_f16(ptr + 8, _p1);
            ptr += 16;
        }
        for (; i + 7 < size; i += 8)
        {
            float16x8_t _p = vld1q_f16(ptr);

            _p = elu_ps_f16(_p, _alpha);

            vst1q_f16(ptr, _p);
            ptr += 8;
        }
        for (; i + 3 < size; i += 4)
        {
            float16x4_t _p = vld1_f16(ptr);

            _p = elu_ps_f16(_p, vget_low_f16(_alpha));

            vst1_f16(ptr, _p);
            ptr += 4;
        }
        for (; i < size; i++)
        {
            __fp16 v = ptr[0];
            if (v < (__fp16)0.f)
                ptr[0] = (__fp16)(alpha * (expf((float)v) - 1.f));

            ptr += 1;
        }
    }

    return 0;

#else
    (void)bottom_top_blob;
    (void)alpha;
    (void)opt;
    return 0;
#endif
}

