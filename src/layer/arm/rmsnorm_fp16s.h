...
int rmsnorm_inplace_fp16s_asimdhp(Mat& bottom_top_blob, int affine_size, float eps, const Mat& gamma_data, const Option& opt);
#endif

static int rmsnorm_inplace_fp16s(Mat& bottom_top_blob, int affine_size, float eps, const Mat& gamma_data, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return rmsnorm_inplace_fp16s_asimdhp(bottom_top_blob, affine_size, eps, gamma_data, opt);
#endif
    ...
    if (dims == 4)
    {
        ...
        else // if (affine_size == w * h * d)
        {
            ...
            {
                __fp16* ptr = bottom_top_blob.channel(q);
                rmsnorm_fp16s(ptr, gamma_data, eps, w * h * d, elempack);
            }
        }
    }
