...
static int prelu_fp16s(Mat& bottom_top_blob, int num_slope, const Mat& slope_data, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return prelu_fp16s_asimdhp(bottom_top_blob, num_slope, slope_data, opt);
#endif
...
static int prelu_fp16sa(Mat& bottom_top_blob, int num_slope, const Mat& slope_data, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return prelu_fp16sa_asimdhp(bottom_top_blob, num_slope, slope_data, opt);
#endif
