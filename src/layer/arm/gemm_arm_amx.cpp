...
#include "gemm_bf16s_fp16s_amx.h"
#include "gemm_fp16s_amx.h"

#include <amx_usability.h>

static void gemm_transB_packed_tile_fp16sa_amx(const Mat& AT_tile, const Mat& BT_tile, const Mat& CT_tile, Mat& topT_tile, Mat& top_blob, int broadcast_type_C, int i, int max_ii, int j, int max_jj, int k, int max_kk, bool k_end)
{
    amx_set();
    ...
    for (; ii + 7 < max_ii; ii += 8)
    {
        ...
        for (; jj + 31 < max_jj; jj += 32)
        {
            ...
            for (; kk < max_kk; kk += 1)
            {
                amx_ldx(false, 0, pB);
                amx_ldy(false, 0, pA);
                amx_fma16_masked(false, 0, 0, 0, 0x2, 32, 0x2, 8, false, false, (k == 0 && pC == nullptr && kk == 0));
                // amx_ldx(false, 0, pA);
                // amx_ldy(false, 0, pB);
                // amx_fma16_masked(false, 0, 0, 0, 0x2, 8, 0x2, 12, false, false, (k == 0 && pC == nullptr && kk == 0));
            ...
            if (k_end)
            {
                if (out_elempack == 1)
                {
                    ...
                    for (int r = 0; r < 8; r++)
                    {
                        amx_stz(false, 2 * r, outptr0 + out_hstep * r);
                        // amx_stz(false, 2 * r, tmp);
                        // memcpy(outptr0 + out_hstep * r, tmp, 32 * sizeof(__fp16));
                    }
        ...
        for (; jj + 11 < max_jj; jj += 12)
        {
            ...
            for (; kk < max_kk; kk += 1)
            {
                amx_ldx(false, 0, pB);
                amx_ldy(false, 0, pA);
                amx_fma16_masked(false, 0, 0, 0, 0x2, 12, 0x2, 8, false, false, (k == 0 && pC == nullptr && kk == 0));
                // amx_ldx(false, 0, pA);
                // amx_ldy(false, 0, pB);
                // amx_fma16_masked(false, 0, 0, 0, 0x2, 8, 0x2, 12, false, false, (k == 0 && pC == nullptr && kk == 0));
            ...
            if (k_end)
            {
                if (out_elempack == 1)
                {
                    ...
                    for (int r = 0; r < 8; r++)
                    {
                        // amx_stz(false, 2 * r, outptr + 8 * r);
                        amx_stz(false, 2 * r, tmp);
                        memcpy(outptr0 + out_hstep * r, tmp, 12 * sizeof(__fp16));
                    }
    ...
    for (; ii + 3 < max_ii; ii += 4)
    {
        ...
        for (; jj + 31 < max_jj; jj += 32)
        {
            ...
            for (; kk < max_kk; kk += 1)
            {
                amx_ldx(false, 0, pB);
                amx_ldy(false, 0, pA);
                amx_fma16_masked(false, 0, 0, 0, 0x2, 32, 0x2, 4, false, false, (k == 0 && pC == nullptr && kk == 0));
                // amx_ldx(false, 0, pA);
                // amx_ldy(false, 0, pB);
                // amx_fma16_masked(false, 0, 0, 0, 0x2, 8, 0x2, 12, false, false, (k == 0 && pC == nullptr && kk == 0));
    ...
    for (; ii + 1 < max_ii; ii += 2)
    {
        ...
        for (; jj + 31 < max_jj; jj += 32)
        {
            ...
            for (; kk < max_kk; kk += 1)
            {
                amx_ldx(false, 0, pB);
                amx_ldy(false, 0, pA);
                amx_fma16_masked(false, 0, 0, 0, 0x2, 32, 0x2, 2, false, false, (k == 0 && pC == nullptr && kk == 0));
                // amx_ldx(false, 0, pA);
                // amx_ldy(false, 0, pB);
                // amx_fma16_masked(false, 0, 0, 0, 0x2, 8, 0x2, 12, false, false, (k == 0 && pC == nullptr && kk == 0));
    ...
    for (; ii < max_ii; ii += 1)
    {
        ...
        for (; jj + 31 < max_jj; jj += 32)
        {
            ...
            for (; kk < max_kk; kk += 1)
            {
                amx_ldx(false, 0, pB);
                amx_ldy(false, 0, pA);
                amx_fma16_masked(false, 0, 0, 0, 0x2, 32, 0x2, 1, false, false, (k == 0 && pC == nullptr && kk == 0));
                // amx_ldx(false, 0, pA);
                // amx_ldy(false, 0, pB);
                // amx_fma16_masked(false, 0, 0, 0, 0x2, 8, 0x2, 12, false, false, (k == 0 && pC == nullptr && kk == 0));
...
static void get_optimal_tile_mnk_fp16sa_amx(int M, int N, int K, int constant_TILE_M, int constant_TILE_N, int constant_TILE_K, int& TILE_M, int& TILE_N, int& TILE_K, int nT)
{
    ...
    TILE_M = std::max(32, tile_size / 32 * 32);
    TILE_N = std::max(32, tile_size / 32 * 32);
    TILE_K = std::max(32, tile_size / 32 * 32);

    if (K > 0)
    {
        int nn_K = (K + TILE_K - 1) / TILE_K;
        TILE_K = std::min(TILE_K, ((K + nn_K - 1) / nn_K + 31) / 32 * 32);
    ...
    if (M > 0)
    {
        ...
        TILE_M = std::min(TILE_M, ((M + nn_M - 1) / nn_M + 31) / 32 * 32);
    }

    if (N > 0)
    {
        int nn_N = (N + TILE_N - 1) / TILE_N;
        TILE_N = std::min(TILE_N, ((N + nn_N - 1) / nn_N + 31) / 32 * 32);
    }

    if (nT > 1)
    {
        TILE_M = std::min(TILE_M, (std::max(1, TILE_M / nT) + 31) / 32 * 32);
    }

    // always take constant TILE_M/N/K value when provided
    if (constant_TILE_M > 0)
    {
        TILE_M = (constant_TILE_M + 31) / 32 * 32;
    }

    if (constant_TILE_N > 0)
    {
        TILE_N = (constant_TILE_N + 31) / 32 * 32;
    }

    if (constant_TILE_K > 0)
    {
        TILE_K = (constant_TILE_K + 31) / 32 * 32;
    }
}

static int gemm_arm_fp16sa_amx(const Mat& A, const Mat& B, const Mat& C, Mat& top_blob, int broadcast_type_C, int transA, int transB, int output_transpose, int constant_TILE_M, int constant_TILE_N, int constant_TILE_K, int nT, const Option& opt)
{
    ...
    get_optimal_tile_mnk_fp16sa_amx(M, N, K, constant_TILE_M, constant_TILE_N, constant_TILE_K, TILE_M, TILE_N, TILE_K, nT);
    ...
    for (int ppjk = 0; ppjk < nn_NK; ppjk++)
    {
        ...
        if (transB)
        {
            pack_B_tile_bf16_fp16_amx(B, BT_tile, j, max_jj, k, max_kk);
        }
        else
        {
            transpose_pack_B_tile_bf16_fp16_amx(B, BT_tile, j, max_jj, k, max_kk);
        }
    }
    ...
    for (int ppi = 0; ppi < nn_M; ppi++)
    {
        ...
        for (int j = 0; j < N; j += TILE_N)
        {
            ...
            if (broadcast_type_C == 3)
            {
                pack_A_tile_bf16_fp16_amx(C, topT_tile, i, max_ii, j, max_jj);
            }
            ...
            for (int k = 0; k < K; k += TILE_K)
            {
                ...
                if (j == 0)
                {
                    if (transA)
                    {
                        transpose_pack_A_tile_bf16_fp16_amx(A, AT_tile, i, max_ii, k, max_kk);
                    }
                    else
                    {
                        pack_A_tile_bf16_fp16_amx(A, AT_tile, i, max_ii, k, max_kk);
                    }
                }

                bool k_end = !output_transpose && k + TILE_K >= K;

                gemm_transB_packed_tile_fp16sa_amx(AT_tile, BT_tile, CT_tile, topT_tile, top_blob, broadcast_type_C, i, max_ii, j, max_jj, k, max_kk, k_end);
            }

            if (output_transpose)
            {
                transpose_unpack_output_tile_bf16_fp16_amx(topT_tile, top_blob, i, max_ii, j, max_jj);
            }
        }
    }
...
static int gemm_AT_arm_fp16sa_amx(const Mat& AT, const Mat& B, const Mat& C, Mat& top_blob, int broadcast_type_C, int M, int K, int transB, int output_transpose, int constant_TILE_M, int constant_TILE_N, int constant_TILE_K, int nT, const Option& opt)
{
    const int N = transB ? (B.dims == 3 ? B.c : B.h) * B.elempack : B.w;
...
static int gemm_BT_arm_fp16sa_amx(const Mat& A, const Mat& BT, const Mat& C, Mat& top_blob, int broadcast_type_C, int N, int K, int transA, int output_transpose, int constant_TILE_M, int constant_TILE_N, int constant_TILE_K, int nT, const Option& opt)
{
    const int M = transA ? A.w : (A.dims == 3 ? A.c : A.h) * A.elempack;
...
static int gemm_AT_BT_arm_fp16sa_amx(const Mat& AT, const Mat& BT, const Mat& C, Mat& top_blob, int broadcast_type_C, int M, int N, int K, int output_transpose, int constant_TILE_M, int constant_TILE_N, int constant_TILE_K, int nT, const Option& opt)
{
    // NCNN_LOGE("M/N/K = %d %d %d", M, N, K);
...
int Gemm_arm::create_pipeline_fp16sa_amx(const Option& opt)
{
    if (constantA)
    {
        ...
        int TILE_M, TILE_N, TILE_K;
        get_optimal_tile_mnk_fp16sa_amx(M, 0, K, constant_TILE_M, constant_TILE_N, constant_TILE_K, TILE_M, TILE_N, TILE_K, opt.num_threads);
        ...
        for (int ppj = 0; ppj < nn_M; ppj++)
        {
            ...
            for (int k = 0; k < K; k += TILE_K)
            {
                ...
                if (transA)
                {
                    transpose_pack_A_tile_fp32_to_fp16_amx(A_data, AT_tile, i, max_ii, k, max_kk);
                }
                else
                {
                    pack_A_tile_fp32_to_fp16_amx(A_data, AT_tile, i, max_ii, k, max_kk);
                }
            }
        }
    ...
    if (constantB)
    {
        ...
        int TILE_M, TILE_N, TILE_K;
        get_optimal_tile_mnk_fp16sa_amx(0, N, K, constant_TILE_M, constant_TILE_N, constant_TILE_K, TILE_M, TILE_N, TILE_K, opt.num_threads);
        ...
        for (int ppj = 0; ppj < nn_N; ppj++)
        {
            ...
            for (int k = 0; k < K; k += TILE_K)
            {
                ...
                if (transB)
                {
                    pack_B_tile_fp32_to_fp16_amx(B_data, BT_tile, j, max_jj, k, max_kk);
                }
                else
                {
                    transpose_pack_B_tile_fp32_to_fp16_amx(B_data, BT_tile, j, max_jj, k, max_kk);
                }
            }
        }
...
int Gemm_arm::forward_fp16sa_amx(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const
{
    int M;
    int N;
    ...
    if (constantA && constantB)
    {
        ret = gemm_AT_BT_arm_fp16sa_amx(AT_data, BT_data, C, top_blob, broadcast_type_C, constantM, constantN, constantK, output_transpose, constant_TILE_M, constant_TILE_N, constant_TILE_K, _nT, opt);
    }
    else if (constantA)
    {
        const Mat& B = bottom_blobs[0];
        ret = gemm_AT_arm_fp16sa_amx(AT_data, B, C, top_blob, broadcast_type_C, constantM, constantK, transB, output_transpose, constant_TILE_M, constant_TILE_N, constant_TILE_K, _nT, opt);
    }
    else if (constantB)
    {
        const Mat& A = bottom_blobs[0];
        ret = gemm_BT_arm_fp16sa_amx(A, BT_data, C, top_blob, broadcast_type_C, constantN, constantK, transA, output_transpose, constant_TILE_M, constant_TILE_N, constant_TILE_K, _nT, opt);
    }
    else
    {
        const Mat& A = bottom_blobs[0];
        const Mat& B = bottom_blobs[1];
        ret = gemm_arm_fp16sa_amx(A, B, C, top_blob, broadcast_type_C, transA, transB, output_transpose, constant_TILE_M, constant_TILE_N, constant_TILE_K, _nT, opt);
    }
    if (ret != 0)
        return ret;
