...
static void rotaryembed_fp16s(const Mat& bottom_blob, const Mat& cos_cache, const Mat& sin_cache, Mat& top_blob, int interleaved, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
    {
        rotaryembed_fp16s_asimdhp(bottom_blob, cos_cache, sin_cache, top_blob, interleaved, opt);
        return;
    }
#endif
...
static void rotaryembed_fp16sa(const Mat& bottom_blob, const Mat& cos_cache, const Mat& sin_cache, Mat& top_blob, int interleaved, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
    {
        rotaryembed_fp16sa_asimdhp(bottom_blob, cos_cache, sin_cache, top_blob, interleaved, opt);
        return;
    }
#endif
#if __ARM_FEATURE_FP16_VECTOR_ARITHMETIC
