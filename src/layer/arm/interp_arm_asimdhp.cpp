...
int Interp_arm::forward_fp16sa(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const
{
    ...
    if (dims == 2)
    {
        ...
        if (elempack == 8)
        {
            ...
            if (resize_type == 2) // bilinear
            {
                ...
                linear_coeffs_fp16sa_impl(w, outw, xofs, alpha, align_corner, (output_width || dynamic_target_size || !size_expr.empty()) ? w / (float)outw : 1.f / width_scale);
            ...
            if (resize_type == 3) // bicubic
            {
                ...
                cubic_coeffs_fp16sa_impl(w, outw, xofs, alpha, align_corner, (output_width || dynamic_target_size || !size_expr.empty()) ? w / (float)outw : 1.f / width_scale);
    ...
    if (elempack == 8)
    {
        ...
        if (resize_type == 2) // bilinear
        {
            ...
            linear_coeffs_fp16sa_impl(w, outw, xofs, alpha, align_corner, (output_width || dynamic_target_size || !size_expr.empty()) ? w / (float)outw : 1.f / width_scale);
            linear_coeffs_fp16sa_impl(h, outh, yofs, beta, align_corner, (output_height || dynamic_target_size || !size_expr.empty()) ? h / (float)outh : 1.f / height_scale);
        ...
        if (resize_type == 3) // bicubic
        {
            ...
            cubic_coeffs_fp16sa_impl(w, outw, xofs, alpha, align_corner, (output_width || dynamic_target_size || !size_expr.empty()) ? w / (float)outw : 1.f / width_scale);
            cubic_coeffs_fp16sa_impl(h, outh, yofs, beta, align_corner, (output_height || dynamic_target_size || !size_expr.empty()) ? h / (float)outh : 1.f / height_scale);
