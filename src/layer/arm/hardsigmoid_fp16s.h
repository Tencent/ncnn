...
static int hardsigmoid_fp16s(Mat& bottom_top_blob, float alpha, float beta, float lower, float upper, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return hardsigmoid_fp16s_asimdhp(bottom_top_blob, alpha, beta, lower, upper, opt);
#endif
...
static int hardsigmoid_fp16sa(Mat& bottom_top_blob, float alpha, float beta, float lower, float upper, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return hardsigmoid_fp16sa_asimdhp(bottom_top_blob, alpha, beta, lower, upper, opt);
#endif
