...
static int instancenorm_fp16s(Mat& bottom_top_blob, float eps, int affine, const Mat& gamma_data, const Mat& beta_data, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return instancenorm_fp16s_asimdhp(bottom_top_blob, eps, affine, gamma_data, beta_data, opt);
#endif
