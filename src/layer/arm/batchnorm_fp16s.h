...
static int batchnorm_fp16s(Mat& bottom_top_blob, const Mat& a_data, const Mat& b_data, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return batchnorm_fp16s_asimdhp(bottom_top_blob, a_data, b_data, opt);
#endif
...
static int batchnorm_fp16sa(Mat& bottom_top_blob, const Mat& a_data, const Mat& b_data, const Option& opt)
{
#if NCNN_RUNTIME_CPU && NCNN_ARM82 && __aarch64__ && !__ARM_FEATURE_FP16_VECTOR_ARITHMETIC
    if (ncnn::cpu_support_arm_asimdhp())
        return batchnorm_fp16sa_asimdhp(bottom_top_blob, a_data, b_data, opt);
#endif
