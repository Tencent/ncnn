...
int MultiHeadAttention_arm::create_pipeline(const Option& _opt)
{
...

    {
        ...
        pd.set(11, 0);                                             // output_N1M
        pd.set(12, 1);                                             // output_elempack
        pd.set(13, kv_cache ? 1 : (opt.use_fp16_storage ? 0 : 1)); // output_elemtype = auto/fp32
#if NCNN_INT8
        pd.set(18, int8_scale_term);
#endif
        int ret = qk_gemm->load_param(pd);
...
int MultiHeadAttention_arm::forward(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& _opt) const
{
    ...
    if (kv_cache)
    {
        ...
        {
            NCNN_LOGE("MultiHeadAttention_arm got foreign kvcache");
            return -1;
        }
    }
    ...
    size_t workspace_elemsize = kv_cache ? 4u : (weight_block_quantize || opt.use_bf16_storage ? 4u : elemsize);
