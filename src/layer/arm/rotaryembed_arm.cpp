        ...
        for (int i = 0; i < seqlen; i++)
        {
            ...
            if (interleaved)
            {
                ...
                for (; j + 7 < half; j += 8)
                {
                    ...
                    float32x4_t _y1l = vmlaq_f32(vmulq_f32(_x1l, _cl), _x0l, _sl); // x0*s + x1*c
                    float32x4_t _y1h = vmlaq_f32(vmulq_f32(_x1h, _ch), _x0h, _sh);

                    uint16x8x2_t _out;
                    _out.val[0] = vcombine_u16(float2bfloat(_y0l), float2bfloat(_y0h));
                    _out.val[1] = vcombine_u16(float2bfloat(_y1l), float2bfloat(_y1h));
