...
static int test_unaryop_load_param_case(const ncnn::ParamDict& pd, bool valid)
{
    ncnn::Layer* layer = ncnn::create_layer_naive(ncnn::LayerType::UnaryOp);
    if (!layer)
        return -1;
    ...
    if (ret != (valid ? 0 : -1))
    {
        fprintf(stderr, "test_unaryop_load_param failed ret=%d expected=%d op_type=%d\n", ret, valid ? 0 : -1, pd.get(0, 0));
        return -1;
    }
...
{
    ncnn::ParamDict base;
    if (test_unaryop_load_param_case(base, true) != 0)
        return -1;
    ...
    for (int i = 0; i <= 27; i++)
    {
        ...
        pd.set(0, i);
        if (test_unaryop_load_param_case(pd, true) != 0)
            return -1;
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ncnn::ParamDict pd = base;
        pd.set(0, invalid[i]);
        if (test_unaryop_load_param_case(pd, false) != 0)
            return -1;
    }
