...
static int test_normalize(const ncnn::Mat& a, int across_spatial, int across_channel, int channel_shared, float eps, int eps_mode)
{
    ...
    if (ret != 0)
    {
        fprintf(stderr, "test_normalize failed a.dims=%d a=(%d %d %d %d) across_spatial=%d across_channel=%d channel_shared=%d eps=%f eps_mode=%d\n", a.dims, a.w, a.h, a.d, a.c, across_spatial, across_channel, channel_shared, eps, eps_mode);
    }

    return ret;
}

static int test_packing_normalize(const ncnn::Mat& a, int across_spatial, int across_channel, int channel_shared, float eps, int eps_mode, int flag = 0)
{
    int scale_data_size = channel_shared ? 1 : a.c;
    ...
    int ret = test_layer_opt("Normalize", pd, weights, opt, a, 0.001f, flag | TEST_LAYER_DISABLE_GPU_TESTING);
    if (ret != 0)
    {
        fprintf(stderr, "test_packing_normalize failed a.dims=%d a=(%d %d %d %d) across_spatial=%d across_channel=%d channel_shared=%d eps=%f eps_mode=%d flag=%d\n", a.dims, a.w, a.h, a.d, a.c, across_spatial, across_channel, channel_shared, eps, eps_mode, flag);
    }
...
static int test_normalize_4()
{
    ...
    return 0
           || test_packing_normalize(a, 1, 0, 0, 0.01f, 0)
           || test_packing_normalize(a, 1, 0, 1, 0.001f, 1)
           || test_packing_normalize(a, 0, 1, 0, 0.002f, 2)
           || test_packing_normalize(a, 1, 1, 1, 0.01f, 0)
           || test_packing_normalize(b, 1, 0, 0, 0.001f, 1)
           || test_packing_normalize(b, 0, 1, 1, 0.002f, 2)
           || test_packing_normalize(b, 1, 1, 0, 0.01f, 0);
}
...
static int test_normalize_5()
{
    ...
    return 0
           || test_packing_normalize(a, 1, 0, 0, 0.0001f, 0)
           || test_packing_normalize(a, 0, 1, 1, 0.0001f, 1)
           || test_packing_normalize(a, 1, 1, 0, 0.0001f, 2)
           || test_packing_normalize(b, 1, 0, 0, 0.0001f, 0)
           || test_packing_normalize(b, 0, 1, 1, 0.0001f, 1)
           || test_packing_normalize(b, 1, 1, 0, 0.0001f, 2);
}
