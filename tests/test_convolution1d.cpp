...
static int test_convolution1d_load_param()
{
    ...
    base.set(6, 216);
    base.set(7, 2);
    if (test_convolution1d_load_param_case(base, true)) return -1;
    for (int activation = 0; activation <= 7; activation++)
    {
        ncnn::ParamDict pd = base;
        pd.set(9, activation);
        bool need_params = activation == 2 || activation == 3 || activation == 6;
        if (test_convolution1d_load_param_case(pd, !need_params && activation != 7)) return -1;
        if (activation == 7) continue;
        ncnn::Mat params(2);
        params[0] = 0.1f;
        params[1] = 0.5f;
        pd.set(10, params);
        if (test_convolution1d_load_param_case(pd, true)) return -1;
        pd.set(10, params.range(0, 1));
        if (test_convolution1d_load_param_case(pd, activation != 3 && activation != 6)) return -1;
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ncnn::ParamDict pd = base;
        pd.set(10, bad[i]);
        if (test_convolution1d_load_param_case(pd, false)) return -1;
    }
...
int main()
{
    if (test_convolution1d_load_param() != 0)
        return -1;
