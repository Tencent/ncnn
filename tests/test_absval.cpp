...
static int test_absval(const ncnn::Mat& a, int flag = 0)
{
    ncnn::ParamDict pd;

    std::vector<ncnn::Mat> weights(0);

    int ret = test_layer("AbsVal", pd, weights, a, 0.001, flag);
    if (ret != 0)
    {
        fprintf(stderr, "test_absval failed a.dims=%d a=(%d %d %d %d)\n", a.dims, a.w, a.h, a.d, a.c);
    }
...
{
    return 0
           || test_absval(RandomMat(5, 6, 7, 24), TEST_LAYER_DISABLE_GPU_TESTING)
           || test_absval(RandomMat(7, 8, 9, 12))
           || test_absval(RandomMat(3, 4, 5, 13))
           || test_absval(RandomMat(1, 1, 13))
           || test_absval(RandomMat(5, 7, 24), TEST_LAYER_DISABLE_GPU_TESTING)
...
static int test_absval_1()
{
    return 0
           ...
           || test_absval(RandomMat(15, 24), TEST_LAYER_DISABLE_GPU_TESTING)
