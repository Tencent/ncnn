...
static int test_gru_load_param_case(const ncnn::ParamDict& pd, bool valid)
{
    ...
    {
        ncnn::Layer* layer = backend == 0 ? ncnn::create_layer_naive(ncnn::LayerType::GRU) : ncnn::create_layer_cpu(ncnn::LayerType::GRU);
        if (!layer) return -1;
        int ret = layer->load_param(pd);
        delete layer;
        if ((ret == 0) != valid)
        {
            fprintf(stderr, "GRU load_param backend=%d returned %d, expected %s\n", backend, ret, valid ? "success" : "failure");
            return -1;
        }
    }
    return 0;
...
static int test_gru_load_param()
{
    ...
    base.set(0, 8);
    base.set(1, 192);
    if (test_gru_load_param_case(base, true)) return -1;
    const int invalid[] = {0, -1, INT_MIN, INT_MAX};
    for (int i = 0; i < 4; i++)
    {
        ncnn::ParamDict pd = base;
        pd.set(0, invalid[i]);
        if (test_gru_load_param_case(pd, false)) return -1;
    }
    ncnn::ParamDict pd = base;
    pd.set(2, 3);
    if (test_gru_load_param_case(pd, false)) return -1;
