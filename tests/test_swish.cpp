...
static int test_swish(const ncnn::Mat& a, int flag = 0)
{
    ncnn::ParamDict pd;

    std::vector<ncnn::Mat> weights(0);

    int ret = test_layer("Swish", pd, weights, a, 0.001, flag);
    if (ret != 0)
    {
...
{
    return 0
           || test_swish(RandomMat(5, 6, 7, 24), TEST_LAYER_DISABLE_GPU_TESTING)
...
{
    return 0
           || test_swish(RandomMat(5, 7, 24), TEST_LAYER_DISABLE_GPU_TESTING)
...
{
    return 0
           || test_swish(RandomMat(15, 24), TEST_LAYER_DISABLE_GPU_TESTING)
