...
static int test_spectrogram_load_param()
{
    ncnn::ParamDict base;
    base.set(0, 16);
    if (test_spectrogram_load_param_case(base, true)) return -1;
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        if (test_spectrogram_load_param_case(pd, false)) return -1;
    }
    ncnn::ParamDict pd = base;
    pd.set(0, INT_MAX);
    pd.set(7, 2);
    ...
    if (test_spectrogram_load_param_case(pd, true)) return -1;
    return 0;
}

int main()
{
    if (test_spectrogram_load_param() != 0)
        return -1;
