...
static int test_tile_load_param_case(const ncnn::ParamDict& pd, bool valid)
{
    ...
    {
        ncnn::Layer* layer = backend == 0 ? ncnn::create_layer_naive(ncnn::LayerType::Tile) : ncnn::create_layer_cpu(ncnn::LayerType::Tile);
        if (!layer) return -1;
        int ret = layer->load_param(pd);
        delete layer;
        if ((ret == 0) != valid)
        {
            fprintf(stderr, "Tile load_param backend=%d returned %d, expected %s\n", backend, ret, valid ? "success" : "failure");
            return -1;
        }
    }
    return 0;
...
static int test_tile_load_param()
{
    ...
    if (test_tile_load_param_case(base, true)) return -1;
    ncnn::ParamDict pd = base;
    pd.set(2, ncnn::Mat(1, (size_t)1u));
    if (test_tile_load_param_case(pd, false)) return -1;
