...
static int test_deconvolution_load_param_case(const ncnn::ParamDict& pd, bool valid)
{
    ncnn::Layer* layer = ncnn::create_layer_naive(ncnn::LayerType::Deconvolution);
    if (!layer)
        return -1;
    ...
    if (ret != (valid ? 0 : -1))
    {
        ...
        fprintf(stderr, "test_deconvolution_load_param failed ret=%d expected=%d num_output=%d kernel_w=%d kernel_h=%d dilation_w=%d dilation_h=%d stride_w=%d stride_h=%d weight_data_size=%d activation_type=%d dynamic_weight=%d\n", ret, valid ? 0 : -1, num_output, kernel_w, kernel_h, dilation_w, dilation_h, stride_w, stride_h, weight_data_size, activation_type, dynamic_weight);
...
static int test_deconvolution_load_param_case(const ncnn::ParamDict& base, int id, int value, bool valid)
{
    ncnn::ParamDict pd = base;
    pd.set(id, value);

    int ret = test_deconvolution_load_param_case(pd, valid);
    if (ret != 0)
    {
        fprintf(stderr, "test_deconvolution_load_param failed id=%d value=%d\n", id, value);
    }

    return ret;
}

static int test_deconvolution_load_param_activation(const ncnn::ParamDict& base, int activation_type, const ncnn::Mat& activation_params, bool valid)
{
    ncnn::ParamDict pd = base;
    pd.set(9, activation_type);
    pd.set(10, activation_params);

    return test_deconvolution_load_param_case(pd, valid);
}
...
static int test_deconvolution_load_param()
{
    ...
    base.set(1, 3);
    base.set(6, 216);
    if (test_deconvolution_load_param_case(base, true) != 0)
        return -1;
    ...
    int ret = 0
              || test_deconvolution_load_param_activation(base, 0, ncnn::Mat(), true)
              || test_deconvolution_load_param_activation(base, 0, ncnn::Mat(0), true)
              || test_deconvolution_load_param_activation(base, 0, params, true)
              || test_deconvolution_load_param_activation(base, 0, params.range(0, 1), true)
              || test_deconvolution_load_param_activation(base, 1, ncnn::Mat(), true)
              || test_deconvolution_load_param_activation(base, 1, ncnn::Mat(0), true)
              || test_deconvolution_load_param_activation(base, 1, params, true)
              || test_deconvolution_load_param_activation(base, 1, params.range(0, 1), true)
              || test_deconvolution_load_param_activation(base, 2, ncnn::Mat(), false)
              || test_deconvolution_load_param_activation(base, 2, ncnn::Mat(0), false)
              || test_deconvolution_load_param_activation(base, 2, params, true)
              || test_deconvolution_load_param_activation(base, 2, params.range(0, 1), true)
              || test_deconvolution_load_param_activation(base, 3, ncnn::Mat(), false)
              || test_deconvolution_load_param_activation(base, 3, ncnn::Mat(0), false)
              || test_deconvolution_load_param_activation(base, 3, params, true)
              || test_deconvolution_load_param_activation(base, 3, params.range(0, 1), false)
              || test_deconvolution_load_param_activation(base, 4, ncnn::Mat(), true)
              || test_deconvolution_load_param_activation(base, 4, ncnn::Mat(0), true)
              || test_deconvolution_load_param_activation(base, 4, params, true)
              || test_deconvolution_load_param_activation(base, 4, params.range(0, 1), true)
              || test_deconvolution_load_param_activation(base, 5, ncnn::Mat(), true)
              || test_deconvolution_load_param_activation(base, 5, ncnn::Mat(0), true)
              || test_deconvolution_load_param_activation(base, 5, params, true)
              || test_deconvolution_load_param_activation(base, 5, params.range(0, 1), true)
              || test_deconvolution_load_param_activation(base, 6, ncnn::Mat(), false)
              || test_deconvolution_load_param_activation(base, 6, ncnn::Mat(0), false)
              || test_deconvolution_load_param_activation(base, 6, params, true)
              || test_deconvolution_load_param_activation(base, 6, params.range(0, 1), false)
              || test_deconvolution_load_param_activation(base, 7, ncnn::Mat(), false);
    if (ret != 0)
        return ret;
    ...
    for (int i = 0; i < 5; i++)
    {
        ncnn::ParamDict pd = base;
        pd.set(10, bad[i]);
        if (test_deconvolution_load_param_case(pd, false) != 0)
            return -1;
    }

    ret = 0
          || test_deconvolution_load_param_case(base, 0, 0, false)
          || test_deconvolution_load_param_case(base, 1, 0, false)
          || test_deconvolution_load_param_case(base, 2, 0, false)
          || test_deconvolution_load_param_case(base, 3, 0, false)
          || test_deconvolution_load_param_case(base, 1, INT_MAX, false)
          || test_deconvolution_load_param_case(base, 6, 217, false);
    if (ret != 0)
        return ret;
...
int main()
{
    ...
    return test_deconvolution_0() || test_deconvolution_1() || test_deconvolution_load_param();
}
