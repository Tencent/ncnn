...
static int test_gemm_block_quant(const ncnn::Mat& A, const ncnn::Mat& B, const ncnn::Mat& C, const ncnn::ParamDict& pd, int bits, int block_size, int has_input_scale, int constantC)
{
    ...
    return test_layer("Gemm", pd, weights, a, 1, 0.001f, TEST_LAYER_DISABLE_GPU_TESTING | TEST_LAYER_ENABLE_THREADING);
}
...
static int test_gemm_bias(int M, int N, int K, int bits, int block_size, const ncnn::Mat& C, float alpha, float beta, int has_input_scale, int transA, int output_transpose, int constantC, int output_N1M = 0, int output_elemtype = 0, int TILE_M = 0, int TILE_N = 0, int TILE_K = 0)
{
...
static int test_gemm_3()
{
    return 0
           ...
           || test_gemm(8, 7, 34, 8, 32, 1)
           || test_gemm(7, 9, 34, 8, 32, 1, 1, 1)
           ...
           || test_gemm_wq_int8_tile(13, 19, 80, 32, 7, 5, 48)
           ...
           || test_gemm(2, 1, 49, 8, 32, 0, 1, 1)
           || test_gemm(1, 2, 49, 8, 32, 0, 1, 1)
           || test_gemm(2, 5, 67, 8, 32, 0, 0, 1)
