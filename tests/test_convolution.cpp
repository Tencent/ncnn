...
static int test_convolution_load_param_text()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if (ret != 0)
        {
            fprintf(stderr, "test_convolution_load_param_text failed params=%s ret=%d\n", params[i], ret);
            return ret;
        }
        ...
        {
            fprintf(stderr, "test_convolution_load_param_text modified params=%s\n", params[i]);
            return -1;
        }
    }
#endif
...
int main()
{
    ...
    return test_convolution_0() || test_convolution_load_param() || test_convolution_load_param_text();
}
