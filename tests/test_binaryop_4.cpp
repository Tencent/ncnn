...
static int test_binaryop_2(int w, int h, int flag = 0)
{
    ...
    for (int j = 0; j < 4; j++)
    {
        for (int k = 0; k < 4; k++)
        {
            ...
            int ret;
            if (j != k && (j & k) > 1)
                ret = test_binaryop(a[j], b[k], flag | TEST_LAYER_DISABLE_GPU_TESTING);
            else
                ret = test_binaryop(a[j], b[k], flag);
            if (ret != 0)
                return ret;
        }
        ...
        int ret;
        if (j != 0 && j != 3)
            ret = test_binaryop(a[j], 0.7f, flag | TEST_LAYER_DISABLE_GPU_TESTING);
        else
            ret = test_binaryop(a[j], 0.7f, flag);
        if (ret != 0)
            return ret;
    }
