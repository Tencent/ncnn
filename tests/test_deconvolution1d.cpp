...
static int test_deconvolution1d_activation_params_text()
{
#if NCNN_STRING
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if (ret != 0)
        {
            fprintf(stderr, "test_deconvolution1d_activation_params_text failed params=%s ret=%d\n", params[i], ret);
            return ret;
        }
        ...
        {
            fprintf(stderr, "test_deconvolution1d_activation_params_text modified params=%s\n", params[i]);
            return -1;
        }
    }
#endif
...
int main()
{
    ...
    return 0
           || test_deconvolution1d_0()
           || test_deconvolution1d_1()
           || test_deconvolution1d_activation_params_text()
#if NCNN_VALIDATION
           || test_deconvolution1d_load_param()
           || test_deconvolution1d_load_param_text()
#endif // NCNN_VALIDATION
           ;
}
