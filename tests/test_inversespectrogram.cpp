...
static int test_inversespectrogram_load_param_case(const ncnn::ParamDict& pd, bool valid)
{
    ...
    {
        ncnn::Layer* layer = backend == 0 ? ncnn::create_layer_naive(ncnn::LayerType::InverseSpectrogram) : ncnn::create_layer_cpu(ncnn::LayerType::InverseSpectrogram);
        if (!layer) return -1;
        int ret = layer->load_param(pd);
        delete layer;
        if ((ret == 0) != valid)
        {
            fprintf(stderr, "InverseSpectrogram load_param backend=%d returned %d, expected %s\n", backend, ret, valid ? "success" : "failure");
            return -1;
        }
    }
    return 0;
...
static int test_inversespectrogram_load_param()
{
    ncnn::ParamDict base;
    base.set(0, 16);
    if (test_inversespectrogram_load_param_case(base, true)) return -1;
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        if (test_inversespectrogram_load_param_case(pd, false)) return -1;
    }
    ncnn::ParamDict pd = base;
    pd.set(0, INT_MAX);
    pd.set(7, 2);
    ...
    if (test_inversespectrogram_load_param_case(pd, true)) return -1;
    return 0;
}

int main()
{
    if (test_inversespectrogram_load_param() != 0)
        return -1;
