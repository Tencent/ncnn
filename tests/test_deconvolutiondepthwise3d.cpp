...
static int test_deconvolutiondepthwise3d_load_param_case(const ncnn::ParamDict& pd, bool valid)
{
    ncnn::Layer* layer = ncnn::create_layer_naive(ncnn::LayerType::DeconvolutionDepthWise3D);
    if (!layer)
        return -1;
    ...
    if (ret != (valid ? 0 : -1))
    {
        ...
        fprintf(stderr, "test_deconvolutiondepthwise3d_load_param failed ret=%d expected=%d num_output=%d kernel_w=%d kernel_h=%d kernel_d=%d dilation_w=%d dilation_h=%d dilation_d=%d stride_w=%d stride_h=%d stride_d=%d weight_data_size=%d group=%d activation_type=%d\n", ret, valid ? 0 : -1, num_output, kernel_w, kernel_h, kernel_d, dilation_w, dilation_h, dilation_d, stride_w, stride_h, stride_d, weight_data_size, group, activation_type);
...
static int test_deconvolutiondepthwise3d_load_param_case(const ncnn::ParamDict& base, int id, int value, bool valid)
{
    ncnn::ParamDict pd = base;
    pd.set(id, value);

    int ret = test_deconvolutiondepthwise3d_load_param_case(pd, valid);
    if (ret != 0)
    {
        fprintf(stderr, "test_deconvolutiondepthwise3d_load_param failed id=%d value=%d\n", id, value);
    }

    return ret;
}

static int test_deconvolutiondepthwise3d_load_param_activation(const ncnn::ParamDict& base, int activation_type, const ncnn::Mat& activation_params, bool valid)
{
    ncnn::ParamDict pd = base;
    pd.set(9, activation_type);
    pd.set(10, activation_params);

    return test_deconvolutiondepthwise3d_load_param_case(pd, valid);
}
...
static int test_deconvolutiondepthwise3d_load_param()
{
    ...
    base.set(6, 216);
    base.set(7, 2);
    if (test_deconvolutiondepthwise3d_load_param_case(base, true) != 0)
        return -1;
    ...
    for (int i = 0; i < 4; i++)
    {
        ncnn::ParamDict pd = base;
        pd.set(10, bad[i]);
        if (test_deconvolutiondepthwise3d_load_param_case(pd, false) != 0)
            return -1;
    }

    ret = 0
          || test_deconvolutiondepthwise3d_load_param_case(base, 0, 0, false)
          || test_deconvolutiondepthwise3d_load_param_case(base, 1, 0, false)
          || test_deconvolutiondepthwise3d_load_param_case(base, 2, 0, false)
          || test_deconvolutiondepthwise3d_load_param_case(base, 3, 0, false)
          || test_deconvolutiondepthwise3d_load_param_case(base, 1, INT_MAX, false)
          || test_deconvolutiondepthwise3d_load_param_case(base, 6, 217, false);
    if (ret != 0)
        return ret;
...
int main()
{
    ...
    return test_deconvolutiondepthwise3d_0() || test_deconvolutiondepthwise3d_load_param();
}
